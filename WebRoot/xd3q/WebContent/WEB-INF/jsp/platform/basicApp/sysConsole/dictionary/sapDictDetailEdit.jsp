...
<title> 编辑页面</title>
<script language="javascript" type="text/javascript" defer="defer" src="<%= request.getContextPath() %>/js/platform/basicApp/sysConsole/dictionary/sapDictDetailEdit.js"></script>
</head>
<body>
...
function _save()
{
	var domValue_l = $('SapDictDetail.domValue_l').value;
	var ddText =$('SapDictDetail.ddText').value;
	if(!domValue_l)
	{
        _getMainFrame().showInfo("[域值],不允许为空！");
        return ;
	}
	if(!ddText)
	{
		_getMainFrame().showInfo("[文本],不允许为空！");
		return ;
	}
