<%-- 
  ...
  - Date: 2010年08月02日 13点55分26秒
  - Copyright Notice:版权声明 福建讯盟软件有限公司, 版权所有 违者必究
  - @(#)
  - Description:  
  - <功能>主对象内贸付款(HomePayment)增加页面
--%>
...
<fisc:attachement businessId="${businessId}" boIdField="HomePayment.paymentid" allowDelete="true" allowUpload="true" divId="div_attachement" boName="HomePayment" boProperty="attachement" gridPageSize="10" gridHeight="285" ></fisc:attachement>
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	...
	<!-- 邱杰烜 2010-09-29 新增"是否纯代理付款div_isrepresentpay_dict"、"纯代理付款客户div_representpaycust_sh" 
	<tr>
		<td align="right" width="15%"  <fisc:authentication sourceName="HomePayment.isrepresentpay" nodeId="${workflowNodeDefId}"/> >${vt.property.isrepresentpay}：</td>
		<td  width="40%">
			<div id="div_isrepresentpay_dict"></div>
			<fisc:dictionary boName="HomePayment" boProperty="isrepresentpay" dictionaryName="YDPAYMENTYESORNO" divId="div_isrepresentpay_dict" isNeedAuth="false"  value="${homePayment.isrepresentpay}"  ></fisc:dictionary>
		</td>
		<td align="right"  width="15%" <fisc:authentication sourceName="HomePayment.representpaycust" nodeId="${workflowNodeDefId}"/> >${vt.property.representpaycust}：</td>
		<td  width="30%" >
			<div id="div_representpaycust_sh"></div>
			<fisc:searchHelp divId="div_representpaycust_sh" boName="HomePayment" boProperty="representpaycust" value="${homePayment.representpaycust}"></fisc:searchHelp>
		</td>
	</tr>
	-->
	<tr>
	...
	<tr>
		...
		<td   width="40%" >
		    <input type="text" class="inputText" id="HomePayment.createtime" name="HomePayment.createtime" value="${homePayment.createtime}"  readonly="readonly" <fisc:authentication sourceName="HomePayment.createtime" nodeId="${workflowNodeDefId}"/>>
		</td>
	</tr>
	<tr>
...
<script language="javascript" type="text/javascript" defer="defer" src="<%= request.getContextPath() %>/js/xdss3/payment/homePaymentAdd.js"></script>
<script language="javascript" type="text/javascript" defer="defer" src="<%= request.getContextPath() %>/js/xdss3/payment/homePaymentAddGen.js"></script>
...
if(Boolean.parseBoolean(haveBindWorkFlow.toString())){%>
					{id:'_submitProcess',text:'${vt.mSubmitProcess}',handler:_submitProcessHomePayment,disabled:true,iconCls:'task'},'-',	
				     {id:'_submitForReassign',text:'重分配提交',handler:_submitForReassignHomePayment,disabled:true,iconCls:'task'},'-',   				
<%}%>		
          
					{id:'_saveOrUpdateHomePayment',text:'${vt.mSaveOrUpdate}',handler:_saveOrUpdateHomePayment,iconCls:'icon-table-save'},'-',
