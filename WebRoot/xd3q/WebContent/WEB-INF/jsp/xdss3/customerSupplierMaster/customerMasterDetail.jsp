...
<table width="900" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <tr>
    	...
    	<td width="450" colspan="2"><font color="green">●</font>财务增加必填;<font color="red">▲</font>业务增加必填</td>
    </tr>
       <tr>
      ...
      <tr>
        ...
        <td><div id="updateDiv"><input type="text" name="guestCode" value="${main.guestCode}"/></div></td>
        <td><div align="right">联系人:</div></td>
        <td><input type="text" name="contactMan" value="${main.contactMan}"/></td>
      </tr>
      <tr>
        <td><div align="right">客户名称<font color="red">▲</font></div></td>
        <td colspan="3"><input type="text" name="guestName" value="${main.guestName}" size="100"/></td>
        </tr>
      <tr>
        <td><div align="right">中间客户名称</div></td>
        <td colspan="3"><input type="text" name="longText" value="${main.longText}" size="100"/></td>
        </tr>
      <tr>
        <td><div align="right">通讯地址<font color="red">▲</font></div></td>
        <td colspan="3"><input type="text" name="address" value="${main.address}"  size="100"/></td>
        </tr>
      <tr>
      ...
      <tr>
      <td><div align="right">是否为信达系统内客户:<font color="red">▲</font></div></td>
      <td>
      ...
      <tr>
        <td><div align="right">对应供应商编码:</div></td>
        <td><input type="text" name="sapSuppliersCode" value="${main.sapSuppliersCode}"/><input type="button" name="btn1" value="..." onclick="selectSupply()"/></td>
        <td><div align="right">账户分配组<font color="green">●</font></div></td>
        <td><div id="div_accountAllot"></div></td>
      </tr>
      <tr>
        <td ><div align="right">账户组<font color="green">●</font></div></td>
        <td ><div id="div_AccountGroup"></div></td>
        <td ><div align="right">公司代码<font color="red">▲</font></div></td>
        <td ><div id="div_companyCode"></div></td>
      </tr>
      <tr>
        <td><div align="right">销售组织<font color="red">▲</font></div></td>
        <td><div id="div_SaleGroup"></div></td>
        <td><div align="right">标题</div></td>
        <td><input type="text" name="title" value="${main.title}"/></td>
      </tr>
      <tr>
        <td><div align="right">税分类:</div></td>
        <td><div id="div_taxSort"></div></td>
      ...
      <tr>
        ...
        <td><div align="right">付款方式<font color="red">▲</font></div></td><td><input type="text" name="payWay" value="${main.payWay}"/><input type="button" name="btn2" value="..." onclick="selectPayTerm()"/></td>
      </tr>
      <tr>
        <td><div align="right">付款条件<font color="red">▲</font></td>
      ...
      <tr>
       ...
       <tr>
        <td width="900" colspan="4" align="center"><div align="center" ><font  size=5><b>系统信息</b></font></div></td>        
     </tr>
     <tr>
        <td><div align="right">状态:</div></td>
        <td>
        	<select id="isAvailable" name="isAvailable" tabindex="27">
				<option value="1">请选择</option>
				<option value="0"
					<c:if test="${main.isAvailable=='0'}"> selected </c:if>>作废</option>
				<option value="1"
					<c:if test="${main.isAvailable=='1'}"> selected </c:if>>有效</option>
					<option value="2"
					<c:if test="${main.isAvailable=='2'}"> selected </c:if>>冻结</option>
			</select>
        </td>
         <td><div align="right">创建时间:</div></td>
        <td><input type="text" name="creatorTime" value="${main.creatorTime}" readonly="true"/></td>
        </tr>
        <tr>
        <td><div align="right">更改时间:</div></td>
        <td><input type="text" name="updateTime" value="${main.updateTime}" readonly="true"/></td>
         <td><div align="right">创建部门:</div></td>
        <td><input type="text" name="createDept" value="${main.createDept}" readonly="true"/></td>
        </tr>
        <tr>
        <td><div align="right">创建人:</div></td>
        <td><input type="text" name="createor" value="${main.createor}" readonly="true"/></td>  
        <td/>
       	<td/>     
        </tr>
         <tr>
       <td colspan="4" align="center">
        ...
        <td><div id="close"><input type="button" name="close" value="关闭" onclick="closeForm()"/></div></td>
          ...
          <input type="hidden" name="guestId" value="${main.guestId}"/>
...
<script type="text/javascript">
  ...
  uploadform = new Ext.form.FormPanel({
				...
				if(uploadform.form.isValid()){
					...
					uploadform.getForm().submit({    
						...
						success: function(form, action){						
						    new AjaxEngine(encodeURI('<%=Constants.FILE_LOCATION_URL%>/attachementController.spr?action=getFilePath&id=' + Ext.getDom('customerId').value + '&boname=' + boname),
	...
	$('updateDiv').innerHTML='<input type="text" name="guestCode" value="${main.guestCode}" readOnly="readOnly"/><input type="button" id="btn3" value="..." onclick="selectGuest()"></input>';
	...
	var operateType='';
	  ...
	  operateType = 'save';
			...
			new AjaxEngine('customerMasterController.spr',{method:"post", parameters: parm,onComplete: callBackHandle2});
    }
    ...
    function saveForm()
    {
    	...
    	//if(Ext.getDom('totalMoney').value == "")Ext.getDom('totalMoney').value='0';
    	if(!checkData()){
    		Ext.Msg.alert('提示','请填写必填项！包括上传附件');
    		return;
    	}
    	//if(rule_ns_ds.getTotalCount()>4){
	    	operateType = 'save';
	        var parm="?action=save&"+Form.serialize('mainForm')+'&update=${modify}';
			new AjaxEngine('customerMasterController.spr',{method:"post", parameters: parm,onComplete: callBackHandle});
	//	}else{
	//		Ext.Msg.alert('提示','请上传附件,附件内容为：企业法人营业执照、组织机构代码证、税务登记证、资信调查报告、财务报表、其他');
	//	}
    }    
    ...
    function selectSupply(){
		top.ExtModalWindowUtil.show('查找供应商','masterQueryController.spr?action=toFindSupplier','',supplyCallback,{width:755,height:450});
    }
    function supplyCallback(){
	var cb = top.ExtModalWindowUtil.getReturnValue();
	Ext.getDom('sapSuppliersCode').value=cb.LIFNR;
    }
    function closeForm()
    {
	    Ext.Msg.show({
    ...
    function guestCallback(){
    	var cb = top.ExtModalWindowUtil.getReturnValue();
    	$('guestCode').value=cb.KUNNR;//客户编码
    	$('guestName').value=cb.NAME1;//客户名称
    	$('country').value=cb.LAND1;//国家
    	$('area').value=cb.ORT01;//地区
    	$('vatNum').value=cb.PSTLZ;//邮政编码/城市
    	//$('').value=cb.STRAS;//街道/门牌号
    	$('phone').value=cb.TELF1;//电话
    	$('phoneExt').value=cb.TELF2;//电话分机
    	$('fax').value=cb.TELFX;//传真号
    	$('email').value=cb.TELBX;//EMAIL
    	$('contactMan').value=cb.ZSABE;//联系人
    	//$('').value=cb.ZTERM;//付款条件
    	$('payWay').value=cb.ZWELS;//付款方式
    	//$('').value=cb.BZIRK;//销售地区
    	dict_div_currency.setComboValue(cb.WAERS);//货币
    	//$('').value=cb.INCO1;//国际贸易条款1
    	//$('').value=cb.INCO2;//国际贸易条款2
    	//$('').value=cb.KUNN4;//付款方
    	dict_div_SaleGroup.setComboValue(cb.VKORG);//销售组织
    }
	...
	function checkData(){
		if(Ext.getDom('guestName').value == "") return false ;
		if(Ext.getDom('address').value == "") return false ;
		...
		if(dict_div_SaleGroup.getComboValue == "") return false ;
	...
	//	if(dict_div_accountAllot.getComboValue == "") return false ;	
	...
	//	if(Ext.getDom('code').value == "") return false ;
		...
		function upload(id){
		boname = id;			
		if(Ext.getDom('customerId').value == ""){
			top.ExtModalWindowUtil.alert('提示',"请先保存主数据在上传附件！");	
		}else{
			var url ='<%= request.getContextPath() %>/attachementController.spr?action=uploadManger&id=' + Ext.getDom('customerId').value + '&userId=' + Ext.getDom('creator').value + '&boname=' + boname;
			...
			if(fileUrl.indexOf(sername) != -1 && fileUrl.indexOf(port) != -1){			
				win.show();
				uploadform.url=encodeURI('<%=Constants.FILE_LOCATION_URL%>/attachementController.spr?action=upload3&cburl=http://<%= request.getServerName() %>:<%= request.getServerPort() %><%= request.getContextPath() %>/attachementController.spr&callback=upcallBackHandle&id=' + Ext.getDom('customerId').value + '&userId=' + Ext.getDom('creator').value + "&boname=" + boname);
			}else{
	...
	function delFile(id){
		...
		var customerId = Ext.getDom('customerId').value;
		var fileName = Ext.getDom(id).value;
		var path = id+'Path';
		if(fileName !=''){
			if(creator ==''){		
				...
				if(fileUrl.indexOf(sername) != -1 && fileUrl.indexOf(port) != -1){
					Ext.Ajax.request({
						url: encodeURI('<%=Constants.FILE_LOCATION_URL%>/attachementController.spr?action=proceeFileDelete&path='+id+'&fileName=' + fileName + '&creator=' + creator +'&masterid=') + customerId,
						params:'',
						scriptTag: true,
						success: function(response){
							new AjaxEngine('customerMasterController.spr?action=delAtt&id=' + Ext.getDom('customerId').value + "&boname=" + id,{method:"post", parameters: '',onComplete: delcallBackHandle});					
						}	
					});
				}else{
					Ext.getDom(id).value = '';
	...
	function disableDropSelection(bool){
		...
		dict_div_SaleGroup.disable(bool);
	    ...
	    dict_div_taxSort.disable(bool);
	    dict_div_currency.disable(bool);
	    dict_div_accountAllot.disable(bool);
	    $('btn1').disabled=bool;
	    $('btn2').disabled=bool;
	    if(bool){
	   	 	combo.disable();
...
<fiscxd:dictionary divId="div_SaleGroup" fieldName="salesOrg" dictionaryName="BM_SALES_ORG" selectedValue="${main.salesOrg}" width="153" disable="${Type}" needDisplayCode="true"></fiscxd:dictionary>
...
<fiscxd:dictionary divId="div_taxSort" fieldName="taxSort" dictionaryName="BM_TAX_SORT" selectedValue="${main.taxSort}" width="153" disable="${Type}" needDisplayCode="true"></fiscxd:dictionary>
<fiscxd:dictionary divId="div_currency" fieldName="currency" dictionaryName="BM_CURRENCY" selectedValue="${main.currency}" width="153" disable="${Type}" needDisplayCode="true"></fiscxd:dictionary>
<fiscxd:dictionary divId="div_accountAllot" fieldName="accountAllot" dictionaryName="BM_ACCOUNT_ALLOT" selectedValue="${main.accountAllot}" width="153" disable="${Type}" needDisplayCode="true"></fiscxd:dictionary>
