...
<table width="900" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <tr>    	
    	<td width="450" colspan="2">注:<font color="green">●</font>财务增加必填;<font color="red">▲</font>业务增加必填</td>
    </tr>
     <tr>
      ...
      <tr>
        <td width="150"><div align="right">账户组<font color="green">●</font></div></td>
      ...
      <tr>
        ...
        <td><div id="close"><input type="button" name="close" value="关闭" onclick="closeForm()"/></div></td>
          ...
          <input type="hidden" name="suppliersId" value="${main.suppliersId}"/>
           <input type="hidden" name="oldSupplierid" value="${main.oldSupplierid}"/>
...
var operateType='';
	  ...
	  operateType = 'save';
			...
			new AjaxEngine('supplierMasterController.spr',{method:"post", parameters: parm,onComplete: callBackHandle2});
    }
    ...
    function saveForm()
    {
    	...
    	//if(rule_ns_ds.getTotalCount()>4){
	    	operateType='save';
	        var parm="?action=save&"+Form.serialize('mainForm')+'&update=${modify}';
			new AjaxEngine('supplierMasterController.spr',{method:"post", parameters: parm,onComplete: callBackHandle});
	//	}else{
	...
	var cb = top.ExtModalWindowUtil.getReturnValue();
	Ext.getDom('sapGuestCode').value=cb.LIFNR;
    }
    function closeForm()
    {
	    Ext.Msg.show({
				...
				if(fileUrl.indexOf(sername) != -1 && fileUrl.indexOf(port) != -1){
					Ext.Ajax.request({
						...
						success: function(response){
							new AjaxEngine('supplierMasterController.spr?action=delAtt&id=' + Ext.getDom('suppliersId').value + "&boname=" + id,{method:"post", parameters: '',onComplete: delcallBackHandle});					
						}	
					});
				}else{
					Ext.getDom(id).value = '';
