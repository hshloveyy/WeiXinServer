...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
    <tr >
      ...
      <td width="25%" colspan="3"><input name="SUPPLIERS_CODE" type="text" size="20"/></td>
      <td width="8%" align="right">供应商名称:</td>
      <td width="25%" colspan="3"><input name="SUPPLIERS_Name" type="text" size="20"/></td>
...
Ext.onReady(function(){
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'supplierMasterController.spr?action=find&state=3'}),
        		reader: new Ext.data.JsonReader({
            		root: 'root',
            		totalProperty: 'totalProperty'
            	},[
            		{name:'SUPPLIERMASTER_ID'},
            		{name:'ACCOUNT_GROUP'},
            		{name:'COMPANY_CODE'},
            		{name:'PURCHASE_ORG'},
            		{name:'TITLE'},
    ...
    var addmaterialInfo = new Ext.Toolbar.Button({
		...
		handler:function(){
			...
			'supplierMasterController.spr?action=createSupplerMaster',
			'',
						...
						top.ExtModalWindowUtil.alert('提示','只能选择一条记录！');
					     ...
					     top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
   									callbackFlag='del';
 									if(buttonId=='yes'){
 	 									var parm ='?action=delete&id='+records[0].json.SUPPLIERMASTER_ID; 
 										new AjaxEngine('supplierMasterController.spr',
  ...
  //data range
    ...
    function supplierCallback(){
		...
		new AjaxEngine('supplierMasterController.spr?action=updateSupplerMaster&lifnr=' + lifnr,
    ...
    function updateMastercallBackHandle(trans){
		...
		top.ExtModalWindowUtil.show('修证数据供应商主数据申请',
			'supplierMasterController.spr?action=forModify&id=' + customField.id,
			'',
			funAddreceiptsCallBack,
			{width:900,height:600});
		}else{
...
//	alert(records[0].data.RECEIPT_ID);
	top.ExtModalWindowUtil.show('供应商主数据审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.SUPPLIERMASTER_ID,
		'',	'',	{width:880,height:400});
}
function modify(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('修改','supplierMasterController.spr?action=forModify&id='+record[0].json.SUPPLIERMASTER_ID,'',find,{width:900,height:600});
}
function view(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('查看','supplierMasterController.spr?action=forView&id='+record[0].json.SUPPLIERMASTER_ID,'',find,{width:900,height:600});
}
function modifyCallback(){
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'supplierMasterController.spr?action=find&'+Form.serialize('findCondictionFR');
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}

function exportToExcel(){
	var url = 'supplierMasterController.spr?action=exportToExcel&'+Form.serialize('findCondictionFR');
