...
<body>
<fisc:grid divId="div_voucherItem" boName="VoucherItem" needCheckBox="true" editable="true" defaultCondition=" YVOUCHERITEM.VOUCHERID='${voucher.voucherid}'" orderBySql="rownumber,depid" pageSize="10000" title="借贷" needAutoLoad="true" height="285" nameSapceSupport="true" ></fisc:grid>
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	...
	<tr>
		...
		<td  width="40%">
			<div id="div_kostl_sh"></div>
			<fisc:searchHelp divId="div_kostl_sh" boName="Voucher" boProperty="kostl"  value="${voucher.kostl}" ></fisc:searchHelp>
		</td>
	</tr>
	<tr>
...
{id:'_confirm',text:'确认并关闭',handler:_confirmVoucher,iconCls:'icon-method'},'-',
' '
],
...
Ext.onReady(function(){
	...
	VoucherItem_grid.getStore().on('load',function(e,recs,o){
		...
		for(var i=0;i<recs.length;i++){
			...
			if(r.get('rownumber')=='1'){
				if(r.get('amount')!='0'){
					Ext.get('Voucher.exchangerate').dom.value = round(parseFloat(r.get('amount2'))/parseFloat(r.get('amount')),5);
				}else{
					Ext.get('Voucher.exchangerate').dom.value = '0';
				}
				
			}
			//判断是否是需要业务范围，2100在模拟凭证后的凭证行项目不用业务范围，如果以后要加公司，如果跟2100一样就加，不一样就不用
			var companycode = Ext.get('Voucher.companycode').dom.value ;
			if('2100' == companycode || '2700' == companycode || '2800' == companycode || '2900' == companycode ||  '2400' == companycode){
