<%-- 
  ...
  - Date: 2010年08月16日 16点58分45秒
  - Copyright Notice:版权声明 福建讯盟软件有限公司, 版权所有 违者必究
  - @(#)
  - Description:  
  - <功能>主对象其他代垫金额变更记录(OtherChangeRecord)管理页面
--%>
...
<script language="javascript" type="text/javascript" defer="defer" src="<%= request.getContextPath() %>/js/xdss3/ceditValueControl/custOtheChanRecoManage.js"></script>
<script language="javascript" type="text/javascript" defer="defer" src="<%= request.getContextPath() %>/js/xdss3/ceditValueControl/custOtheChanRecoManageGen.js"></script>
</head>
<body>
<fisc:grid divId="div_southForm" 
		   boName="CustOtheChanReco"  
		   editable="false" 
		   needCheckBox="true" 
		   needToolbar="true" 
		   needAutoLoad="true" 
		   orderBySql=" YOTHECHANRECO.Createtime DESC "
		   defaultCondition=" (YOTHECHANRECO.USERTYPE = '1' OR YOTHECHANRECO.USERTYPE = '2') and EXISTS (SELECT 'x' FROM T_PROJECT_INFO PI WHERE PI.DEPT_ID  in (SELECT dept_id FROM t_sys_dept_user WHERE user_id = '${userId}' UNION SELECT DEPT_ID FROM T_SYS_USER_MANAGER_DEPT WHERE USER_ID = '${userId}')  AND PI.PROJECT_ID = PROJECTID) "
		   >
</fisc:grid>
<!-- dpPageFtlLib.ftl -->
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	<tr>
		...
		<td  width="30%" >
			...
			<fisc:searchHelp divId="div_userid_sh" boName="CustOtheChanReco" boProperty="userid" searchType="field" hiddenName="userid.fieldValue" valueField="KUNNR" displayField="NAME1"></fisc:searchHelp>
		</td>
		...
		<td  width="40%" >
			...
			<fisc:searchHelp divId="div_projectid_sh" boName="CustOtheChanReco" boProperty="projectid" searchType="field" hiddenName="projectid.fieldValue" valueField="PROJECT_ID" displayField="PROJECT_NAME"></fisc:searchHelp>
		</td>
	</tr>
	<tr>
...
var Txt={
	// 采购订单
	otherChangeRecord:'${vt.boText}',
	// 您选择了【其他代垫金额变更记录删除】操作，是否确定继续该操作？
	otherChangeRecord_Delete_ConfirmOperation:'${vt.delete_ConfirmOperation}',
	// 提示
	cue:'${vt.sCue}',
	// 确定
	ok:'${vt.sOk}',
