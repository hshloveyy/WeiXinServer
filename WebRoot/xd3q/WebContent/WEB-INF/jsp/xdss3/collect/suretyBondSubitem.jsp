...
<body>
	<fisc:grid 
		   ...
		   pageSize="10" 
		   tableName="ycollect "
		   handlerClass="com.infolion.xdss3.collect.domain.CollectSuretyBondGrid2"
 		   whereSql="" 
 		   needCheckBox="true"
	...
	<!-- style="display: none" -->
		<table width="100%" border="0" cellpadding="4" cellspacing="1"  style="display: none">
			<tr>
				<td width="" align="right">收款单号：</td>
				<td width="" colspan="3">
					<input type="text" class="inputText" id="collectno" name="collectno" value="${collectno}">
				</td>
				<td></td>
...
<script language="javascript" type="text/javascript" defer="defer" src="<%= request.getContextPath() %>/js/xdss3/collect/suretyBondSubitem.js"></script>
...
Ext.onReady(function(){
    ...
    viewport.doLayout();
     ...
     var contIndex = Collect_grid.getColumnModel().findColumnIndex('businessstate');
    Collect_grid.getColumnModel().setRenderer(contIndex,function(contNo){		
		if(contNo == '2'){
			return '过机要室';
		}
		if(contNo == '3'){
...
function _manager(value,metadata,record){
	...
	var hrefStr = '';
	if(category == '2'){// 若为退款
		hrefStr = '<a href="#" style="color: green;" onclick="_commonMethodOperation(\'1\',\'客户退款查看\',Collect_grid,\'xdss3/customerDrawback/customerRefundmentController.spr?action=_view&refundmentid='+value+'\',\'_viewCustomerRefundmentpCallBack\',\'_view\',\'false\')">退款查看</a>  ';
		hrefStr +='<a href="#" style="color: green;" onclick="_viewProcessState(\''+value+'\',\'xdss3/customerDrawback/customerRefundmentController.spr?action=_viewProcessState\')">查看流程</a>  ';
	}else{				// 若为收款
		hrefStr = '<a href="#" style="color:green;" onclick="_commonMethodOperation(\'1\',\'收款查看\',Collect_grid,\'xdss3/collect/collectController.spr?action=_view&collectid='+value+'\',\'_viewCollectpCallBack\',\'_view\',\'false\')">收款查看</a>    ';
		hrefStr +='<a href="#" style="color: green;" onclick="_viewProcessState(\''+value+'\',\'xdss3/collect/collectController.spr?action=_viewProcessState\')">查看流程</a>';
	}
	return hrefStr;
}

function _signJudge(value,metadata,record){
	var category = record.get('category');
	if(category == '2'){// 若为退款
		return '-'+value;
	}else{				// 若为收款
