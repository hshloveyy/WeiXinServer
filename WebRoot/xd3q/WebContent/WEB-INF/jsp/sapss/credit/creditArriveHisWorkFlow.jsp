...
Ext.onReady(function(){
  ...
  //装期
	...
	if(taskType=="6") //信用证收证修改 节点。
    {
        ...
        customCreateDate1.setDisabled(false);
        ...
        */
        ...
        Ext.getDom('btnselectContractInfo').disabled = false;
        Ext.getDom('btnselectProjectInfo').disabled = false;
        Ext.getDom('btnselectDept').disabled = false;

        Ext.getDom("billConditions").readOnly = false;
        Ext.getDom("creditNo").readOnly = false;
        Ext.getDom("createBank").readOnly = false;
        Ext.getDom("electricShip").readOnly = false;

        var creditRecDate1=Ext.getCmp("creditRecDate");
        creditRecDate1.setDisabled(false);
	...
	<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
	    <tr>
			...
			<td width="15%" align="left"><input name="projectNo"
				...
				<input type="button" value="..." id="btnselectProjectInfo" name="btnselectProjectInfo" onclick="selectProjectInfo()" disabled="disabled"></td>
      ...
      <td width="11%" align="right">审证人：</td>
        <td width="22%" align="left">
			<input name="applyer" type="text" size="14" tabindex="1" value="${main.applyer}" readonly="readonly"/>
        </td>
        <td width="11%" align="right">信用证号：</td>
        <td width="22%" align="left">
        	<input name="creditNo" type="text" size="14" tabindex="1" value="${main.creditNo}" readonly="readonly"/>
        </td>
      ...
      <tr>
        ...
        <td width="22%" align="left">
			<input name="country" type="text" size="14" tabindex="1" value="${main.country}" readonly="readonly"/>
        </td>
        <td width="11%" align="right">付款方式：</td>
        <td width="22%" align="left">
			<input name="paymentType" type="text" size="14" tabindex="1" value="${main.paymentType}" readonly="readonly"/>
        </td>
        <td width="11%" align="right">开证行：</td>
		<td width="22%" align="left">
			<input name="createBank" type="text" size="14" tabindex="2" value="${main.createBank}" readonly="readonly"/>
		</td>
     ...
     <tr>
        ...
        <td width="22%" align="left">
			<input name="amount" type="text" size="14" tabindex="1" value="${main.amount}" readonly="readonly"/>
        </td>
        ...
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
            <input name="rate" type="text" size="14" tabindex="1" value="${main.rate}" readonly="readonly"/>
        </td>
        <td width="11%" align="right"><font color="red">*</font>受益人：</td>
        <td width="22%" align="left">
			<input name="benefit" type="text" size="14" tabindex="2" value="${main.benefit}" readonly="readonly"/><input type="button" value="..." name="btnselectDept" title="选择部门" onclick="selectDeptInfo()" disabled="disabled">
        </td>
      </tr>
      <tr>
        <td width="11%" align="right">开证申请人：</td>
        <td width="22%" align="left">
			<input name="request" type="text" size="14" tabindex="1" value="${main.request}" readonly="readonly"/>
        </td>
        <td width="11%" align="right">货物品名：</td>
        <td width="22%" align="left">
			<input name="goods" type="text" size="14" tabindex="1" value="${main.goods}"  readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>规格：</td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="specification" type="text" size="14" tabindex="1" value="${main.specification}" readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>唛头：</td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
        	<input name="mark" type="text" size="14" tabindex="1" value="${main.mark}" readonly="readonly"/>
        </td>
      </tr>
      <tr>
        <td width="11%" align="right">发票：</td>
        <td width="22%" align="left">
			<input name="invoice" type="text" size="14" tabindex="1" value="${main.invoice}" readonly="readonly"/>
        </td>
        <td width="11%" align="right"><c:if test="${saleTradeType=='2'}">运输单据/货物收据：</c:if><c:if test="${saleTradeType=='3'}">提单：</c:if></td>
        <td width="22%" align="left">
			<input name="billOfLading" type="text" size="14" tabindex="1" value="${main.billOfLading}" readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>保险单：</td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="billOfInsurance" type="text" size="14" tabindex="1" value="${main.billOfInsurance}" readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>><nobr>品质(分析证)：</nobr></td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="billOfQuality" type="text" size="14" tabindex="1" value="${main.billOfQuality}"  readonly="readonly"/>
        </td>
      </tr>
      <tr>
        ...
        <td width="22%" align="left">
			<input name="benefitCertification" type="text" size="14" tabindex="1" value="${main.benefitCertification}" readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>产地证：</td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="certificateOfOrigin" type="text" size="14" tabindex="1" value="${main.certificateOfOrigin}" readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>装箱单：</td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="packingSlip" type="text" size="14" tabindex="1" value="${main.packingSlip}" readonly="readonly"/>
        </td>
        <td width="11%" align="right" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>装船电：</td>
        <td width="22%" align="left" <c:if test="${saleTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="electricShip" type="text" size="14" tabindex="1" value="${main.electricShip}" readonly="readonly"/>
        </td>
      </tr>
      <tr>
        <td width="11%" align="right">其他单据：</td>
        <td width="22%" align="left" colspan="5">
			<input name="otherDocuments" type="text" size="89" tabindex="1" value="${main.otherDocuments}" readonly="readonly"/>
        </td>
      ...
      <tr>
        ...
        <td width="22%" align="left">
			<input name="loadingPeriod" type="text" size="14" tabindex="1" value="${main.loadingPeriod}"  readonly="readonly"/>
        </td>
        <td width="11%" align="right"><c:if test="${saleTradeType=='2'}">装运地</c:if><c:if test="${saleTradeType=='3'}">装运港</c:if><font color="red">▲</font></td>
        <td width="22%" align="left">
			<input name="portOfShipment" type="text" size="14" tabindex="1" value="${main.portOfShipment}" readonly="readonly"/>
        </td>
        <td width="11%" align="right"><c:if test="${saleTradeType=='2'}">目的地</c:if><c:if test="${saleTradeType=='3'}">目的港</c:if></td>
        <td width="22%" align="left">
			<input name="portOfDestination"	type="text" size="14" tabindex="1" value="${main.portOfDestination}" readonly="readonly"/>
        </td>
      ...
      <tr>
        ...
        <td width="11%" align="right">到期地点：</td>
	      <td width="22%" align="left">
			 <input name="place" type="text" size="14" tabindex="1" value="${main.place}" readonly="readonly"/>
	      </td>
      </tr>
     <tr>
        <td width="11%" align="right" valign="top">特别条款：</td>
        <td width="88%" align="left" colspan="7">
        	<textarea cols="97" rows="10" id="specialConditions" name="specialConditions" readonly="readonly">${main.specialConditions}</textarea>
        </td>
     </tr>
    <tr>
		<td width="11%" align="right" valign="top"><nobr>应修改事项：</nobr></td>
		<td width="88%" align="left" colspan="7">
        	<textarea cols="97" rows="3" id="billConditions" name="billConditions" readonly="readonly">${main.billConditions}</textarea>
		</td>
    </tr>
...
<fiscxd:workflow-taskHistory divId="history" businessRecordId="${main.creditHisId}" width="750"></fiscxd:workflow-taskHistory>
