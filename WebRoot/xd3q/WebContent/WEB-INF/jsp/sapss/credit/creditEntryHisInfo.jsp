...
Ext.onReady(function(){
   ...
   var creditEntryHisInfobbar = new Ext.PagingToolbar({
			...
			items:[{
		       ...
		       buttons:[{
	            	...
	            	handler:function(){
	    			    ...
	    			    var modifyInfo =Ext.getDom("modifyInfo").value; 
	    			    if(modifyInfo==null || modifyInfo == ''  )
	    			    {
	    			    	top.ExtModalWindowUtil.alert('错误','[应修改事项]必填！');
		    			    return;
	    			    }
	            		var param1 = Form.serialize('mainForm');
						var param = param1 + "&versionNoOld=" + Ext.getDom("versionNo").value +"&action=saveCreditEntryHisInfo";
						
						new AjaxEngine('creditEntryHisController.spr', 
	...
	{
		creditEntryHisInfogrid.addListener('rowclick', creditEntryHisInfogridrowclick1);
	}
    ...
    function creditEntryHisInfogridrowclick(grid, rowIndex, e){
      ...
      if (grid.selModel.hasSelection()){
			...
			else
			{
		    	...
		    	Ext.getDom("bailDate").value = (records.get('BAIL_DATE')==null) ? "" : records.get('BAIL_DATE'); 
...
function operaRD(value,metadata,record){
	...
	var versionNo = record.data.VERSION_NO;
...
function prePrint()
{
   if(creditEntryHisInfogrid.selModel.hasSelection())
   {
	   var records = creditEntryHisInfogrid.selModel.getSelections();
	   var creditHisId = records[0].json.CREDIT_HIS_ID;
       window.open('creditEntryHisController.spr?action=dealPrint&creditHisId='+creditHisId,'_blank','location=no,resizable=yes');
   }
}
//信用证改证，流程跟踪
...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
	<tr>
		...
		<td width="15%" align="left"><nobr><input name="contractNo"
			type="text" size="12" value="${main.contractNo}" readonly="readonly" />
			 <input type="button" tabindex="1"  value="..." id="btnselectContractInfo" name="btnselectContractInfo" onclick="selectContractInfo()">
		...
		<td width="15%" align="left"><input name="projectNo"
			type="text" size="15" tabindex="3" value="${main.projectNo}"
			readonly="readonly" /></td>
		<td width="10%" align="right"><nobr>立项名称：</nobr></td>
		<td width="15%" align="left"><input
			name="projectName" type="text" size="15" tabindex="4"
			value="${main.projectName}" readonly="readonly" /></td>
	</tr>
	<tr>
	    <td width="11%" align="right">申请人：</td>
		<td width="22%" align="left">
			<input name="request" type="text" size="15" tabindex="5" value="${main.request}" readonly="readonly"/>
		</td>
		<td width="11%" align="right">信用证号：</td>
		<td width="22%" align="left"><input name="creditNo" type="text"
			size="15" tabindex="6" value="${main.creditNo}" title="信用证号要等最后才输入！" readonly="readonly"/></td>
		<td width="11%" align="right"><nobr>开证日期：</nobr></td>
		<td width="22%" align="left">
		<input name="createDate" type="text" size="15" tabindex="7"
	...
	<tr>
		...
		<td width="22%" align="left">
			<input name="country" type="text" size="15" tabindex="9" value="${main.country}" />
		</td>
		<td width="11%" align="right">付款方式：</td>
		<td width="22%" align="left">
			<input name="paymentType" type="text" size="15" tabindex="10" value="${main.paymentType}" />
		</td>
		<td width="11%" align="right">受益人：</td>
		<td width="22%" align="left" colspan="3">
			<input name="benefit" type="text" size="53" tabindex="11" value="${main.benefit}" />
		</td>
	</tr>
	...
	<tr>
		...
		<td width="22%" align="left">
			<input name="amount" type="text" size="15" tabindex="12" value="${main.amount}" />
		</td>
		...
		<td width="22%" align="left" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>>
		 	<input name="rate" type="text" size="15" tabindex="13" value="${main.rate}" />
		</td>
		...
		<td width="22%" align="left" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>><input name="writeOffSingleNo"
			type="text" size="15" tabindex="14" value="${main.writeOffSingleNo}" readonly="readonly"/>
		</td>
	...
	<tr>
		...
		<td width="22%" align="left">
			<input name="goods" type="text" size="15" tabindex="15" value="${main.goods}" />
		</td>
		<td width="11%" align="right">规格：</td>
		<td width="22%" align="left">
			<input name="specification" type="text" size="15" tabindex="16" value="${main.specification}" />
		</td>
		...
		<td width="22%" align="left" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>><input name="mark" type="text"
			size="15" tabindex="17" value="${main.mark}" /></td>    
	...
	<tr>
		...
		<td width="22%" align="left">
			<input name="invoice" type="text" size="15" tabindex="19" value="${main.invoice}" />
		</td>
		<td width="11%" align="right"><c:if test="${purTradeType=='1'}">提单：</c:if><c:if test="${purTradeType=='2'}">运输单据或货物收据：</c:if></td>
		<td width="22%" align="left">
			<input name="billOfLading" type="text" size="15" tabindex="10" value="${main.billOfLading}" />
		</td>
		<td width="11%" align="right">保险单：</td>
		<td width="22%" align="left">
			<input name="billOfInsurance" type="text" size="15" tabindex="20" value="${main.billOfInsurance}" />
		</td>
		<td width="11%" align="right"><nobr>品质(分析证)：</nobr></td>
		<td width="22%" align="left">
			<input name="billOfQuality" type="text" size="15" tabindex="21" value="${main.billOfQuality}" />
		</td>
	</tr>
	<tr>
		<td width="11%" align="right"><nobr>受益人证明：</nobr></td>
		<td width="22%" align="left">
			<input name="benefitCertification" type="text" size="15" tabindex="22" value="${main.benefitCertification}" />
		</td>
		<td width="11%" align="right" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>>产地证:</td>
		<td width="22%" align="left" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="certificateOfOrigin" type="text" size="15" tabindex="23" value="${main.certificateOfOrigin}" />
		</td>
		<td width="11%" align="right">装箱单：</td>
		<td width="22%" align="left">
			<input name="packingSlip" type="text" size="15" tabindex="24" value="${main.packingSlip}" />
		</td>
		<td width="11%" align="right" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>>起运电：</td>
		<td width="22%" align="left" <c:if test="${purTradeType=='2'}">style='visibility: hidden'</c:if>>
			<input name="dispatchElectric" type="text" size="15" tabindex="25" value="${main.dispatchElectric}" />
		</td>
	</tr>
	<tr>
	    <td width="11%" align="right" valign="top">其他单据:</td>
		<td width="22%" align="left" colspan="7">
		    <textarea cols="96" rows="5" name="otherDocuments" tabindex="26">${main.otherDocuments}</textarea>
		</td>
	...
	<tr>
		...
		<td width="22%" align="left">
			<input name="portOfShipment" type="text" size="15" tabindex="27" value="${main.portOfShipment}" />
		</td>
		<td width="11%" align="right"><c:if test="${purTradeType=='1'}">目的港：</c:if><c:if test="${purTradeType=='2'}">目的地：</c:if></td>
		<td width="22%" align="left">
			<input name="portOfDestination"	type="text" size="15" tabindex="28" value="${main.portOfDestination}" />
		</td>
		<td width="11%" align="right">装期：</td>
		<td width="22%" align="left">
			<input name="loadingPeriod" type="text" size="15" tabindex="29" value="${main.loadingPeriod}" />
		</td>
		<td width="11%" align="right">有效期：</td>
        <td width="22%" align="left">
            <input name="validDate" type="text" size="15" tabindex="30" value="${main.validDate}" readonly="readonly" />
        </td>
	...
	<tr>
		...
		<td width="22%" align="left">
			<select id="transShipment" name="transShipment" tabindex="31">
		...
		<td width="22%" align="left">
			<select id="canBatches" name="canBatches" tabindex="32">
				<option value="">请选择</option>
		...
		<td width="22%" align="left">
			<select id="preSecurity" name="preSecurity" tabindex="33">
				<option value="">请选择</option>
				<option value="Y"
		...
		<td width="22%" align="left">
			<input name="place" type="text" size="15" tabindex="34" value="${main.place}" />
	   </td>
	...
	<tr>
		...
		<td width="88%" align="left" colspan="7">
			<textarea cols="96" rows="4" id="specialConditions" name="specialConditions" tabindex="35">${main.specialConditions}</textarea>
		</td>
	</tr>
	<tr>
		<td width="11%" align="right" valign="top">修改事项：</td>
		<td width="88%" align="left" colspan="7">
			<textarea cols="96" rows="4" id="modifyInfo" name="modifyInfo" tabindex="35">${main.modifyInfo}</textarea>
		</td>
	</tr>
	<tr>
		<td width="11%" align="right" valign="top">保证金情况：</td>
		<td width="88%" align="left" colspan="3">
			<input type="text" size="30"  name="creditInfo" value="${main.creditInfo}" tabindex="36"/>
		</td>
	    <td width="11%" align="right"><nobr>保证金日期：</nobr></td>
		<td width="22%" align="left" >
			<input name="bailDate" type="text" size="15" tabindex="37" value="${main.bailDate}" readonly="readonly" />
		</td>  	
