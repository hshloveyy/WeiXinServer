...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
    <tr >
      ...
      <td ><input name="creator" type="text" size="20"/></td>
    </tr>
    <tr align="center">
...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('增加','scrappedController.spr?action=toScrappedInfo','',find,{width:800,height:500});
		}
   	});
						...
						top.ExtModalWindowUtil.alert('提示','只能选择一条记录！');
							...
							top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
   									callbackFlag = 'del';
 									if(buttonId=='yes'){
 	 									var parm ='?action=delScrapped&scrappedId='+records[0].json.SCRAPPED_ID; 
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'scrappedController.spr?action=find'}),
    ...
    var itemCm = new Ext.grid.ColumnModel([
		...
		itemSm,
		{
            ...
            sortable: false,
            hidden:true,
            dataIndex: 'SCRAPPED_ID'
        },{
			header: '报废单编号',
            sortable: false,
            hidden:true,
            dataIndex: 'SCRAPPED_NO'
        },{
			header: '申请时间',
           	width: 100,
    ...
    function operaRD(value,metadata,record){
        ...
        if(record.data.EXAMINE_STATE_D_EXAMINE_STATE=='新增'){
        	return "<a href='#' onclick='modify()'>修改</a> ";
        }else
        	return "<a href='#' onclick='history()'>查看</a> <a href='#' onclick='viewHistory()'>流程查看</a>";
     }
...
function viewHistory(){
	...
	top.ExtModalWindowUtil.show('审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.SCRAPPED_ID,
		'',	'',	{width:880,height:400});
}
function modify(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('修改','scrappedController.spr?action=modify&scrappedId='+record[0].json.SCRAPPED_ID,'',find,{width:800,height:500});
}

function history(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('查看','scrappedController.spr?action=view&scrappedId='+record[0].json.SCRAPPED_ID,'',find,{width:800,height:500});
}

function find(){
	var url = 'scrappedController.spr?action=find&'+Form.serialize('findCondictionFR');;
