...
<table width="100%" border="1" cellpadding="4" cellspacing="1"
	...
	<tr>
		...
		<td><table><tr><td><div id="sDateDiv"></div></td><td>到</td><td><div id="eDateDiv"></div></td></tr></table>
		
		</td>
		<td align="right">发票类型</td>
		<td>
		<select name="billType"><option value="1">增值税</option><option value="3">普通发票</option></select>
		</td>
		<td align="center"></td>

		<td><input type="button" value="查找"
			onclick="findBillApply()"></input> <input type="reset" value="清空"></input>
		   <a onbeforeactivate="onbeforClickA()" href="billApplyController.spr?action=dealOutToExcel1" id="aaa" target="_self">下载</a>			
		</td>
	</tr>
</table>
</form>
...
function findBillApply(){
	...
	requestUrl = requestUrl + '&dept=' + selectId_dept;	
...
function onbeforClickA(){
	var param = Form.serialize('findBillApplyFrom');
	var requestUrl = 'billApplyController.spr?action=dealOutToExcel1&' + param;
	requestUrl = requestUrl + "&tradeType=" + tradeType;
	requestUrl = requestUrl + '&dept=' + selectId_dept;	
    //alert(requestUrl);
	$('aaa').href=requestUrl;
...
Ext.onReady(function(){
    ...
    var addBillApply = new Ext.Toolbar.Button({
		...
		handler:function(){
			...
			'billApplyController.spr?action=newBillApplyCreate&isProduct=1&tradeType='+tradeType,
			'',
   				...
   				if (billApplygrid.selModel.hasSelection()){
					...
					if (records.length > 1){
						top.ExtModalWindowUtil.alert('提示','只能选择一条记录！');
					      ...
					      window.open('billApplyController.spr?action=dealPrint&billApplyId='+records[0].json.BILL_APPLY_ID,'_blank','');
					}
				}else{
	...
	billApplyds = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'billApplyController.spr?action=queryBillApply&billType=1&tradeType='+tradeType}),
 ...
 //billApplygrid.addListener('rowclick', billApplygridrowclick);
   	...
   	var viewport = new Ext.Viewport({
		...
		items:[{
			...
			region:"center",
			layout:'fit',
			title: strBillApplyTitle + "开票申请单(增值税)列表",
			items:[billApplygrid]
		}]
	});
...
function viewForm(billApplyId){
	...
	top.ExtModalWindowUtil.show(strBillApplyTitle + '开票申请单信息',
		'billApplyController.spr?action=addBillApplyView&billApplyId='+ billApplyId +"&tradeType="+ tradeType +"&operType=001&from=composite",
		'',
...
function operaForm(billApplyId){
	...
	top.ExtModalWindowUtil.show(strBillApplyTitle + '开票申请单信息',
		'billApplyController.spr?action=addBillApplyView&billApplyId='+ billApplyId +"&tradeType="+ tradeType+ "&operType=101",
		'',
		funBillApplyCallbackFunction,
		{width:900,height:550});
}
...
function submitForm(billApplyId){
	...
	top.ExtModalWindowUtil.show(strBillApplyTitle + '开票申请单信息',
		'billApplyController.spr?action=addBillApplyView&billApplyId='+ billApplyId +"&tradeType="+ tradeType+  "&operType=011",
		'',
		funBillApplyCallbackFunction,
		{width:900,height:550});
}
