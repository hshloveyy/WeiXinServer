...
Ext.onReady(function(){
   	...
   	var del = new Ext.Toolbar.Button({
		...
		handler:function(){
   				if (itemGrid.selModel.hasSelection()){
					...
					}else if(records[0].data.STATUS_D_STORAGEPROTOCOL_STATE=='新增'){
							top.Ext.Msg.show({
   								title:'提示',
   								msg: '是否确定删除记录',
   								buttons: {yes:'是', no:'否'},
    ...
    function operaRD(value,metadata,record){
        var state = record.data.STATUS_D_STORAGEPROTOCOL_STATE;

    	if('${loginer}'== record.data.CREATOR_ID){
    		if (state=='新增'){    		
        		return '<a href="#" onclick="modify();" >修改</a>';
       	 	}else if(state=='审批通过'){
        		return '<a href="#" onclick="view()">查看</a> <a href="#" onclick="toChangeForm()">变更</a> <a href="#" onclick="viewHistory()">流程跟踪</a>';
        	}else if(state=='变更'){
        		return '<a href="#" onclick="view()">查看</a> <a href="#" onclick="viewHistory()">流程跟踪</a>';
        	}else if(state=='变更通过'){
            		return '<a href="#" onclick="toChangeForm()">变更</a> <a href="#" onclick="viewHistory()">流程跟踪</a>';	
            }else{
        		return '<a href="#" onclick="view()">查看</a> <a href="#" onclick="viewHistory()">流程跟踪</a>';
            }		
    	}else{
...
function toChangeForm(){
	if(tempGrid.selModel.hasSelection()){
		var record = tempGrid.selModel.getSelections();
		top.ExtModalWindowUtil.show('仓储协议变更','storageProtocolController.spr?action=changeW&increateFile=true&readable=true&infoId='+record[0].json.INFO_ID,'',find,{width:800,height:500});
	}
}
function find(){
