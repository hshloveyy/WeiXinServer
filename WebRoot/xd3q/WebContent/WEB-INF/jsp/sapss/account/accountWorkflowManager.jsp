...
function submitMainForm(){
	...
	var accountTitle =window.frames['content']['maintainForm'].document.getElementById("accountTitle").value;
	var balanceMaintain =window.frames['content']['maintainForm'].document.getElementById("balanceMaintain").value;
	var cashierAccount =window.frames['content']['maintainForm'].document.getElementById("cashierAccount").value;
	...
	if(mask!=null && mask.trim()!=''){		
		if(accountTitle==''&&taskName=="信息中心录入")
		{showMsg('请确认会计科目并保存数据再提交');		
		return;
		}			
		if(balanceMaintain==''&&taskName=="信息中心录入")
		{showMsg('请确认银行科目期初余额并保存数据再提交');		
		return;
		}
		if(cashierAccount==''&&taskName=="信息中心录入")
		{showMsg('请确认出纳日记账并保存数据再提交');		
		return;
		}
		Ext.Msg.show({
	   		title:'提示',
