...
<form id="findCostFR" method="post">
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
    <tr >
      <td width="12%" height="20" align="right">公司代码:</td>
      <td width="20%"><input name="companyCode" type="text" size="20"/></td>
      <td width="12%" align="right">成本中心名称:</td>
      <td width="20%" colspan="3"><input name="costCenterName" type="text" size="20"/></td>
	  <td width="10%" align="right">成本中心组类型:</td>
      <td width="20%"><input name="costGroupType" type="text" size="20"/></td>
	</tr>
	<tr>
	  <td width="12%" height="20" align="right">上层成本中心组:</td>
      <td width="20%"><input name="upCostGroup" type="text" size="20"/></td>
      ...
      <td width="20%"><div id="costStateDiv"></div></td>
    </tr>
    <tr align="center">
...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('增加成本中心','costCenterController.spr?action=create','',find,{width:800,height:600});
		}
   	});
						...
						top.ExtModalWindowUtil.alert('提示','只能选择一条记录！');
							...
							top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
 									if(buttonId=='yes'){
 											Ext.Ajax.request({
												url: 'costCenterController.spr?action=delete&costID='+records[0].json.COST_ID,
	...
	//extComponentServlet?type=grid&grid_columns=COST_ID,PROJECT_NAME,APPLY_TIME,PROJECT_NO&grid_sql=select * from t_project_info&grid_size=10'}),
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'costCenterController.spr?action=find'}),
    ...
    var itemCm = new Ext.grid.ColumnModel([
		...
		itemSm,{
           ...
           width: 100,
           sortable: true,
           dataIndex: 'COST_CENTER_NO'
           },{
           		header: '上层成本中心组',
          		width: 100,
           		sortable: false,
           		dataIndex: 'UP_COST_GROUP'
           ...
           dataIndex: 'COST_CENTER_TYPE'
           		...
           		width: 100,
           		sortable: true,
           		dataIndex: 'COST_STATE_D_COST_STATE'
           },{
           		header: '审批通过时间',
           		width: 100,
           		sortable: true,
    ...
    function operaRD(value,metadata,record){
    	var state = record.data.COST_STATE_D_COST_STATE;
...
function viewHistory(){
	...
	top.ExtModalWindowUtil.show('审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.COST_ID,
		'',	'',	{width:880,height:400});
}
function operaForm(){
		if(tempGrid.selModel.hasSelection()){
			var record = tempGrid.selModel.getSelections();
			top.ExtModalWindowUtil.show('修改成本中心','costCenterController.spr?action=modify&from=modify&costID='+record[0].json.COST_ID,'',modifyCallback,{width:800,height:600});
		}
}
function submitWorkflowForm(){
		if(tempGrid.selModel.hasSelection()){
			var record = tempGrid.selModel.getSelections();
			top.ExtModalWindowUtil.show('提交成本中心申请','costCenterController.spr?action=modify&from=workflow&costID='+record[0].json.COST_ID,'',modifyCallback,{width:800,height:600});
		}
}
function viewProjectForm(){
		if(tempGrid.selModel.hasSelection()){
			var record = tempGrid.selModel.getSelections();
			top.ExtModalWindowUtil.show('查看成本中心申请','costCenterController.spr?action=modify&from=view&costID='+record[0].json.COST_ID,'',modifyCallback,{width:800,height:600});
		}
}
function modifyCallback(){
	var url = 'costCenterController.spr?action=find';
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'costCenterController.spr?action=find&'+Form.serialize('findCostFR');
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
</script>
<fiscxd:dictionary divId="costStateDiv" fieldName="state" dictionaryName="BM_COST_STATE" width="155"></fiscxd:dictionary>