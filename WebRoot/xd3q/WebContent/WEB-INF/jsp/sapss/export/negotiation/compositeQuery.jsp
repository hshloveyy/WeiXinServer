...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('议付信息','negotiationController.spr?action=toNegotiationInfo','',find,{width:600,height:500});
		}
   	});
						...
						if (records[0].data.EXAMINE_STATE != "1" )
							...
							top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
   									callbackFlag='del';
 									if(buttonId=='yes'){
 	 									var parm ='?action=delete&id='+records[0].json.NEGOTIATION_ID; 
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'negotiationController.spr?action=find'}),
...
//	alert(records[0].data.RECEIPT_ID);
	top.ExtModalWindowUtil.show('议付审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.NEGOTIATION_ID,
		'',	'',	{width:880,height:400});
}
function modify(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('议付信息','negotiationController.spr?action=forModify&id='+record[0].json.NEGOTIATION_ID,'',find,{width:600,height:500});
}

function view(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('议付信息','negotiationController.spr?action=forView&id='+record[0].json.NEGOTIATION_ID,'',find,{width:600,height:500});
}

function modifyCallback(){
	var url = 'negotiationController.spr?action=find&tradeTypeSingle=${tradeType}';
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'negotiationController.spr?action=find&'+Form.serialize('query');
