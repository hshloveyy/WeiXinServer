...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
	...
	<td width="26%" >
      ...
      <td width="10%" height="20" align="right">信用证号:</td>
     <td width="22%"><input name="creditNo" type="text" size="20"/></td>
      <td width="10%" height="20" align="right">开证行:</td>
     <td width="22%"><input name="createBank" type="createBank" size="20"/></td>     
    </tr>
    <tr >
      <td align="right">到证日期:</td>
      <td ><input name="startDate" type="text" size="20"/></td>
      <td align="right">至</td>
      <td ><input name="endDate" type="text" size="20"/></td>
...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('打包贷款信息','baleLoanController.spr?action=toBaleLoanInfo','',find,{width:600,height:500});
		}
   	});
						...
						if (records[0].data.EXAMINE_STATE != "1" )
							...
							top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
   									callbackFlag='del';
 									if(buttonId=='yes'){
 	 									var parm ='?action=delete&id='+records[0].json.BALE_LOAN_ID; 
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'baleLoanController.spr?action=find'}),
...
//	alert(records[0].data.RECEIPT_ID);
	top.ExtModalWindowUtil.show('打包贷款管理审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.BALE_LOAN_ID,
		'',	'',	{width:880,height:400});
}
function modify(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('打包贷款信息','baleLoanController.spr?action=forModify&id='+record[0].json.BALE_LOAN_ID,'',find,{width:600,height:500});
}

function view(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('打包贷款信息','baleLoanController.spr?action=forView&id='+record[0].json.BALE_LOAN_ID,'',find,{width:600,height:500});
}

function modifyCallback(){
	var url = 'baleLoanController.spr?action=find&tradeTypeSingle=${tradeType}';
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'baleLoanController.spr?action=find&'+Form.serialize('query');
