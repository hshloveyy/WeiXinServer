...
Ext.onReady(function(){
    ...
    //增加
    var addpurchaseBill = new Ext.Toolbar.Button({
   		text:'增加',
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('新增开票申请单',
			'purchaseBillController.spr?action=newPurchaseBillCreate',
			'',
			funAddpurchaseBillCallBack,
			{width:900,height:550});
		}
   	});
      	
   	//删除
   	var deletepurchaseBill = new Ext.Toolbar.Button({
   		text:'删除',
	    iconCls:'delete',
		handler:function(){
			if (purchaseBillgrid.selModel.hasSelection()){
						...
						title:'提示',
   						...
   						fn: function(buttonId,text){
   							if(buttonId=='yes'){
								...
								param = param + "&purchaseBillId=" + records[0].data.PURCHASE_BILL_ID;

								new AjaxEngine('purchaseBillController.spr', 
						   			{method:"post", parameters: param, onComplete: DeletepurchaseBillCallbackFunction});
   							}
   						},
    ...
    var purchaseBilltbar = new Ext.Toolbar({
		items:[addpurchaseBill,'-',deletepurchaseBill]
	});
