...
Ext.onReady(function(){
	...
	//extComponentServlet?type=grid&grid_columns=PROJECT_ID,PROJECT_NAME,APPLY_TIME,PROJECT_NO&grid_sql=select * from t_project_info&grid_size=10'}),
	 itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'masterQueryController.spr?action=findSaleContract&person=${person}'}),
...
function modify(){
	...
	top.ExtModalWindowUtil.show('审批',
			'contractController.spr?action=seniorExamine&businessRecordId='+record[0].json.CONTRACT_SALES_ID+"&person=${person}",
