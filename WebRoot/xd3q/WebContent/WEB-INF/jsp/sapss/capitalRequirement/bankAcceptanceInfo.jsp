...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'PAYMENTID'},{name:'DRAFTDATE'},{name:'PAYMENTNO'},{name:'APPLYAMOUNT'},{name:'CURRENCY'}];
   	var column=[new Ext.grid.RowNumberer()
	            		,{header:'操作',sortable:true,dataIndex:'PAYMENTID',renderer:paymentIdRender}
	            		,{header:'到期日',sortable:true,dataIndex:'DRAFTDATE',width:80}
	...
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'fundsRequireQueryController.spr?action=rtnBankAcceptancInfo&date=${date}'}),
...
function colClick(paymentId){
	top.ExtModalWindowUtil.show("银行承兑汇票详细","xdss3/payment/homePaymentController.spr?action=_view&paymentid="+paymentId,
