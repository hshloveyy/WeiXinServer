...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
    <tr >
      ...
      <td width="12%" align="right">出票行:</td>
      <td width="20%" colspan="3"><input name="bank" type="text" size="20"/></td>
	  <td width="10%" align="right">收款人名称:</td>
	...
	<tr>
      ...
      <td width="12%" align="right">出票日:</td>
...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('增加银行承兑汇票','creditController.spr?action=create&type=1','',find,{width:800,height:300});
		}
   	});
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'creditController.spr?action=find&type=1'}),
...
function operaForm(){
		if(tempGrid.selModel.hasSelection()){
			...
			top.ExtModalWindowUtil.show('修改银行承兑汇票','creditController.spr?action=modify&from=modify&creditID='+record[0].json.CREDIT_ID,'',modifyCallback,{width:800,height:300});
		}
}
function viewProjectForm(){
		if(tempGrid.selModel.hasSelection()){
			var record = tempGrid.selModel.getSelections();
			top.ExtModalWindowUtil.show('查看银行承兑汇票','creditController.spr?action=modify&from=view&creditID='+record[0].json.CREDIT_ID,'',modifyCallback,{width:800,height:300});
		}
}
function modifyCallback(){
	var url = 'creditController.spr?action=find&type=1';
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'creditController.spr?action=find&type=1&'+Form.serialize('findCreditFR');
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
</script>