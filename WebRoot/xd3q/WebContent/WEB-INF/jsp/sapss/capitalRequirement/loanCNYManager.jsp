...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('增加'+str,'loanCNYController.spr?action=create&type='+type,'',find,{width:800,height:500});
		}
   	});
						...
						top.ExtModalWindowUtil.alert('提示','只能选择一条记录！');
							...
							top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
 									if(buttonId=='yes'){
 											Ext.Ajax.request({
												url: 'loanCNYController.spr?action=delete&loanID='+records[0].json.LOAN_ID,
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'loanCNYController.spr?action=find&type='+type}),
...
function operaForm(){
		if(tempGrid.selModel.hasSelection()){
			...
			top.ExtModalWindowUtil.show('修改'+str,'loanCNYController.spr?action=modify&type='+type+'&from=modify&loanID='+record[0].json.LOAN_ID,'',modifyCallback,{width:800,height:500});
		}
}
function viewProjectForm(){
		if(tempGrid.selModel.hasSelection()){
			var record = tempGrid.selModel.getSelections();
			top.ExtModalWindowUtil.show('查看'+str,'loanCNYController.spr?action=modify&type='+type+'&from=view&loanID='+record[0].json.LOAN_ID,'',modifyCallback,{width:800,height:500});
		}
}
function modifyCallback(){
	var url = 'loanCNYController.spr?action=find&type='+type;
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'loanCNYController.spr?action=find&type='+type+'&'+Form.serialize('findCreditFR');
	ds.proxy= new Ext.data.HttpProxy({url:url});
	ds.load({params:{start:0, limit:10}});
}
</script>