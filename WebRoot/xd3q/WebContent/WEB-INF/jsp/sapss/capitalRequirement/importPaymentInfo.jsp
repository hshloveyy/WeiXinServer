...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'PAYDATE'},{name:'PAYMENTID'},{name:'PAYMENTNO'},{name:'APPLYAMOUNT'},{name:'CURRENCY'}];
   	var column=[new Ext.grid.RowNumberer()
	            		,{header:'操作',sortable:true,dataIndex:'PAYMENTID',renderer:paymentIdRender}
	            		,{header:'到期日',sortable:true,dataIndex:'PAYDATE',width:80}
	...
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'fundsRequireQueryController.spr?action=rtnImportPaymentInfo&date=${date}'}),
...
function colClick(paymentId){
	top.ExtModalWindowUtil.show("信用证付款详细","xdss3/payment/importPaymentController.spr?action=_view&paymentid="+paymentId,
