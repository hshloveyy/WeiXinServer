...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'DEPT_NAME'},{name:'TRADE_TYPE'},{name:'PICK_LIST_NO'},{name:'CONTRACT_NO'},{name:'EKKO_IHREZ'},{name:'GOODS_NAME'}
   			,{name:'BENEFIT'},{name:'CREATE_BANK'},{name:'CREDIT_NO'},{name:'VALID_DATE'},{name:'AMOUNT'},{name:'PICK_LIST_REC_DATE'}
   			,{name:'EKPO_MENGE'},{name:'EKPO_MEINS'},{name:'EKPO_BPRME'},{name:'TOTAL_VALUE'},{name:'PAY_DATE'},{name:'NPAY_DATE'},{name:'PICK_STATE'}];
   	var column=[new Ext.grid.RowNumberer(),
	            		...
	            		,{header:'供应商名称',sortable:true,dataIndex:'BENEFIT',width:80}
	            		,{header:'开证行',sortable:true,dataIndex:'CREATE_BANK',width:80}
	            		,{header:'信用证号',sortable:true,dataIndex:'CREDIT_NO',width:80}
	            		,{header:'信用证期限',sortable:true,dataIndex:'VALID_DATE',width:80}
	...
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'importController.spr?action=rtnCreditPicklistView&'+serv_param}),
...
function doExport(){
	...
	window.location.href('importController.spr?action=creditPicklistViewToExcel&'+param);
