...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'DEPT_NAME'},{name:'TRADE_TYPE'},{name:'PICK_LIST_NO'},{name:'CONTRACT_NO'},{name:'EKKO_IHREZ'},{name:'GOODS_NAME'}
		,{name:'EKKO_LIFNR_NAME'},{name:'TOTAL'},{name:'EKKO_ZTERM_NAME'},{name:'BILL_NO'},{name:'PICK_LIST_REC_DATE'}
		,{name:'EKPO_MENGE'},{name:'EKPO_MEINS'},{name:'EKPO_BPRME'},{name:'TOTAL_VALUE'},{name:'PAY_DATE'},{name:'PICK_STATE'}];
	var column=[new Ext.grid.RowNumberer(),
    		...
    		,{header:'T/T期限',sortable:true,dataIndex:'EKKO_ZTERM_NAME',width:80}
    		...
    		,{header:'付款日',sortable:true,dataIndex:'PAY_DATE',width:80}
    		,{header:'审批状态',sortable:true,dataIndex:'PICK_STATE',width:80}
		];
 		
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'importController.spr?action=rtnTTPicklistView&'+serv_param}),
...
function doExport(){
	...
	window.location.href('importController.spr?action=TTPicklistViewToExcel&'+param);
