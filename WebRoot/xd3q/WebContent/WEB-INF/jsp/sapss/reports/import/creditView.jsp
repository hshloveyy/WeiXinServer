...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'DEPT_NAME'},{name:'TRADE_TYPE'},{name:'PROJECT_NO'},{name:'CONTRACT_NO'},{name:'EKKO_IHREZ'},{name:'EKPO_TXZ01'},{name:'EKKO_LIFNR_NAME'}
   			,{name:'CUSTOMER_LINK_MAN'},{name:'CREDIT_INFO'},{name:'CREATE_DATE'},{name:'CREATE_BANK'},{name:'CREDIT_NO'},
   			{name:'AVAIL_DATE'},{name:'CURRENCY'},{name:'AMOUNT'},{name:'CREDIT_BALANCE'},{name:'LOADING_PERIOD'},{name:'VALID_DATE'},{name:'CREDIT_STATE'}];
   	var column=[new Ext.grid.RowNumberer(),
	            		...
	            		,{header:'货物品名',sortable:true,dataIndex:'EKPO_TXZ01',width:160}
	            		,{header:'供应商名称',sortable:true,dataIndex:'EKKO_LIFNR_NAME',width:80}
	            		,{header:'国内委托方',sortable:true,dataIndex:'CUSTOMER_LINK_MAN',width:80}
	            		,{header:'人民币保证金',sortable:true,dataIndex:'CREDIT_INFO',width:80}
	            		...
	            		,{header:'信用证期限',sortable:true,dataIndex:'AVAIL_DATE',width:80}
	...
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'importController.spr?action=rtnCreditView&'+serv_param}),
...
function doExport(){
	...
	window.location.href('importController.spr?action=creditViewToExcel&'+param);
