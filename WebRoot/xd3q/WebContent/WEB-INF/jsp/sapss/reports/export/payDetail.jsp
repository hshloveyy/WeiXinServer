...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var column=[new Ext.grid.RowNumberer(),
	            		...
	            		,{header:'付款日期',sortable:true,dataIndex:'PAY_REAL_TIME',width:80}
	            		,{header:'币别',sortable:true,dataIndex:'CURRENCY',width:80}
	            		,{header:'付款金额',sortable:true,dataIndex:'PAY_VALUE',width:80}
	            		,{header:'用途',sortable:true,dataIndex:'PAY_USE',width:80}
					];
   		
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'exportRController.spr?action=rtnPayDetail&'+serv_param}),
...
function doExport(){
	...
	window.location.href('exportRController.spr?action=payDetailToExcel&'+param);
