...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'DEPT_NAME'},{name:'TRADE_TYPE'},{name:'SALES_NO'},{name:'CONTRACT_PAPER_NO'},{name:'CUSTOMER'}
   			...
   			,{name:'REAL_RECEIPT_DATE'},{name:'REAL_RECEIPT_TOTAL'},{name:'MARK'}];
   	var column=[new Ext.grid.RowNumberer(),
			...
			,{header:'L/C NO.',sortable:true,dataIndex:'LCDPDA_NO',width:80}
			,{header:'L/C金额',sortable:true,dataIndex:'AMOUNT',width:80}
			,{header:'L/C期限',sortable:true,dataIndex:'VALID_DATE',width:80}
			...
			,{header:'备注',sortable:true,dataIndex:'MARK',width:80}
		];
   		
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'exportRController.spr?action=rtnLCView&'+serv_param}),
...
function doExport(){
	...
	window.location.href('exportRController.spr?action=LCViewToExcel&'+param);
