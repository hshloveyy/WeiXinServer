...
Ext.onReady(function(){
	Ext.QuickTips.init();
   	...
   	var field=[{name:'DEPT_NAME'},{name:'TRADE_TYPE'},{name:'CONTRACT_NO'},{name:'VBKD_IHREZ'},{name:'KUAGV_KUNNR_NAME'}
   				,{name:'OPEN_DATE'},{name:'BILL_NO'},{name:'CURRENCY'},{name:'BILL_TOTAL'},{name:'RECEIPT_DATE'}];
   	var column=[new Ext.grid.RowNumberer(),
				...
				,{header:'开票日期',sortable:true,dataIndex:'OPEN_DATE',width:80}
				...
				,{header:'应收日期',sortable:true,dataIndex:'RECEIPT_DATE',width:80}
			];
   		
	store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({url:'homeTradeController.spr?action=rtnOpenBill&'+serv_param}),
...
function doExport(){
	...
	window.location.href('homeTradeController.spr?action=openBillToExcel&'+param);
