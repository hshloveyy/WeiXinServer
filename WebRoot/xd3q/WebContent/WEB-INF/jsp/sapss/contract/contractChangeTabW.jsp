...
Ext.onReady(function(){
	...
	changeDS.load({params:{start:0,limit:10}});
	...
	itemGrid.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
		...
		Ext.getDom('changeTime').value=r.data.CHANGE_TIME;
		...
		Ext.getDom('amount').value=r.data.AMOUNT!=null?r.data.AMOUNT:'';
...
function saveChange(){
	...
	if(amount!=''&&!Utils.isFloatValue(amount)){
...
function submitChange(){
	if(!outGrid.selModel.hasSelection()){
		top.Ext.Msg.alert('提示','请选择一条记录进行提交');
		return;
	}
	var records = outGrid.selModel.getSelections();
	if(records[0].json.CHANGE_STATE_D_CHANGE_STATE=='变更申请'){
	  ...
	  top.Ext.Msg.show({
   		...
   		fn: function(buttonId,text){
   			if(buttonId=='yes'){
   				submitAction = true;
			//	new AjaxEngine('${controller}',
			// 		{method:"post", parameters:'?action=submitWorkflow&'+Form.serialize('changeInfoform'), onComplete: callBackHandle});
				Ext.Ajax.request({
					url: '${controller}',
					params:'action=submitWorkflow&shortSAP=${shortSAP}&'+Form.serialize('changeInfoform'),
					success: function(response, options){
					   var json = Ext.util.JSON.decode(response.responseText);
						Ext.Msg.alert('提示',json['infolion-json']['coustom'].returnMessage);
						setTimeout(function(){
...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF">
  <tr>
    ...
    <td width="10%">变更时间:</td>
    <td width="10%"><input type="text" name="changeTime" value="${bo.changeTime}" readonly="readonly"/></td>
    <td width="10%">变更批次:</td>
    <td width="10%"><input type="text" name="changeNo" value="${bo.changeNo}" readonly="readonly"/></td>
  </tr>
  <tr>
    <td width="10%">描述:</td>
    <td colspan="5" width="50%"><textarea name="changeDesc" id="changeDesc" style="width:596px;overflow-y:visible;word-break:break-all;">${bo.changeDesc}</textarea></td>
  </tr>
  <tr>
  	
  	<td colspan="1">变更后总合同金额:</td>
  	<td>
  	...
  	<td colspan="2"><font color="red">如涉及变更金额、物料数量、单价等，该栏必填！</font></td>
  </tr>
  <tr>
