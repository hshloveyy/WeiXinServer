...
Ext.onReady(function(){
    ...
    var addOther = new Ext.Toolbar.Button({
   		...
   		iconCls:'add',
		handler:function(){
			var requestUrl = 'contractController.spr?action=addPurchaseCaseInfo&purchaserowsid='+strPurchaseRowsId + '&saprowno=' + strSapRowNo;
			Ext.Ajax.request({
				url: requestUrl,
				success: function(response, options){
					...
					var othertp = new otherPlant({
							...
							KONV_KSCHL_D_CONDITION_TYPE_PU: responseArray.konvKschl,
							KONV_WEARS_D_CURRENCY: responseArray.konvWears,
							KONV_KBETR: responseArray.konvKbetr,
							KONV_LIFNR: responseArray.konvLifnr,
							KONV_LIFNR_NAME: responseArray.column10,
				...
				top.Ext.Msg.show({
					title:'提示',
  						...
  						fn: function(buttonId,text){
  							if(buttonId=='yes'){
  								var param = "?action=deletePurchaseMaterielCase";
    ...
    function afterEdit(obj){
        ...
        if (colname == 'KONV_KSCHL_D_CONDITION_TYPE_PU'){   
        	row.set(colname,getConditionTitle(colvalue));
        	colname = 'KONV_KSCHL';
        }
        if (colname == 'KONV_WEARS_D_CURRENCY'){
        	row.set(colname,getCurrencyTitle(colvalue));
        	colname = 'KONV_WEARS';
        }
        
        var requestUrl = 'contractController.spr?action=updateMaterielCase';
    ...
    function callkonvLifnr(){
		...
		requestUrl = 'contractController.spr?action=updateMaterielCase';
		requestUrl = requestUrl + '&materielcaseid=' + matreielcaseid;
