...
function submitMainForm(ti){
	if (Ext.getDom('workflowExamine').value.trim() != ''){
	    ...
	    var taskId = ${taskId};
	    if(ti>0) taskId =ti;
		var param = '?action=submitPurchaseInfo&workflowTaskId='+taskId+'&workflowCurrentTaskName='+workflowCurrentTaskName+workflowInfo + '&' + param1 + '&' + param2;
		if(ti==null){
		Ext.Msg.show({
	   		title:'提示',
...
function viewTradeMonint(){
	_getMainFrame().maintabs.addPanel('贸易监控','', 'tradeMonitoringController.spr?action=_conditionManage&isdefault=05&queryValue='+window.frames['content']['contractForm'].contractNo.value);
}
