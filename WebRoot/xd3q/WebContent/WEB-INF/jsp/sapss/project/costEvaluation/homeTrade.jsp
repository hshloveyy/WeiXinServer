...
<html>
  ...
  <table width="550" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" >
    ...
    <tr>
      ...
      <td>15</td>
      <td colspan="3">货款利息（年利率${empty main.interestRate?INTEREST_RATE:main.interestRate}%）</td>
    ...
    <tr>
      <td rowspan="6" align="center">税<br>收</td>
      <td height="35">21</td>
      <td colspan="3">增值税进销差（销项-进项）</td>
  ...
  function init(){
 	if(sa != '') {  	
  	...
  	for(var i=1;i<60;i++){
		if(i==1 || i == 2 || i==3 || i == 40 || i == 45 || i == 29 || i == 27 || i==28 || i==52)continue;
	...
	for(var i=1;i<60;i++){	
		if(i==1 || i == 2 || i==3 || i == 4 || i == 27 || i==28 || i==52)continue;
...
/***商品购进成本货款总额**/
function hkzr(){
	$('rmb3').value = Utils.roundoff($('rmb1').value*$('rmb2').value,2);
}
addListener($('rmb1'),"propertychange",hkzr); 
addListener($('rmb2'),"propertychange",hkzr);
/***计算不含税货款和进项税金**/
function bhshkJxsj(){
...
/***进货成本小计**/
function jhcbxj(){
$('rmb6').value = Utils.roundoff(getNumValue('rmb41') + getNumValue('rmb4'),2);
}
addListener($('rmb41'),"propertychange",jhcbxj);
...
/***商品销售货款总额***/
function spxszr(){
$('rmb29').value = Utils.roundoff(getNumValue('rmb27')*getNumValue('rmb28'),2);
}
addListener($('rmb27'),"propertychange",spxszr);
addListener($('rmb28'),"propertychange",spxszr);
/***不含税货款***/
function bhshk(){
...
/***增值税及附加费用及增值税进销差***/
function zzsjfjfy(){
var c = getNumValue('rmb47') - getNumValue('rmb42');
$('rmb57').value = Utils.roundoff(c,2);
var v = 0.12*c ;
if(v<0) v = 0;
...
$('rmb31').value = Utils.roundoff(getNumValue('rmb46') - getNumValue('rmb6') - getNumValue('rmb14') - getNumValue('rmb44') - getNumValue('rmb49'),2);
}
...
/**合同预计利润率（百分比或单位利润）自动计算=预计利润总额/不含税货款**/
function htyjlrl(){
