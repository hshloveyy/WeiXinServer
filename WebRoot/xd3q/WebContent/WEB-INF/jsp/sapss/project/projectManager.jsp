...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
	...
	<tr align="center">
      <td width="12%" height="20" align="right">客户/供应商不包含:</td>
      <td width="20%" align="left"><input name="unitNameN" type="text" size="20" /></td>
      <td colspan="6"><input type="button" value="查找" onclick="find()"/> <input type="reset" value="清空"/></td>
	</tr>
</table>
</form>
</div>
...
Ext.onReady(function(){
	...
	var print = new Ext.Toolbar.Button({
		...
		handler:function(){
   				if (itemGrid.selModel.hasSelection()){
					...
					for (i=0; i<num ;i++) {
					    projectIds +="&projectId="+records[i].json.PROJECT_ID;
					}
					window.open('projectController.spr?action=dealPrintV1'+projectIds,'_blank','');
							
				}else{
					top.ExtModalWindowUtil.alert('提示','请选择记录');
				}
				//selectWindow.hide();
		}
   	});
		...
		handler:function(){
   				if (itemGrid.selModel.hasSelection()){
					...
					for (var i=0;i<records.length;i++){
						idList = idList + records[i].data.PROJECT_ID + '|';
					}
					top.Ext.Msg.show({
						title:'提示',
	  						msg: '是否归档所选记录',
	  						...
	  						fn: function(buttonId,text){
	  							if(buttonId=='yes'){
	  								var param = "?action=fileProject";
									param = param + "&idList=" + idList;

								new AjaxEngine('projectController.spr', 
   	...
   	var close = new Ext.Toolbar.Button({
   		text:'立项关闭',
	    iconCls:'find',
	    disabled:${closeDisable=='false'?false:true},
		handler:function(){
   				if (itemGrid.selModel.hasSelection()){
					var records = itemGrid.selModel.getSelections();
					var idList = '';
    ...
    var itemTbar = new Ext.Toolbar({
		items:[print,'-',print1,'-',file,'-',close]
	});
   	
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'projectController.spr?action=find&state=3'}),
...
function viewProjectForm(){
		if(tempGrid.selModel.hasSelection()){
			...
			top.ExtModalWindowUtil.show('查看'+record[0].json.TRADE_TYPE_D_BUSINESS_TYPE+record[0].json.PROJECT_STATE_D_PROJECT_STATE,'projectController.spr?action=modify&from=view&projectId='+record[0].json.PROJECT_ID,'',modifyCallback,{width:800,height:500});
		}
}
...
function find(){
	...
	var url = 'projectController.spr?action=find&deptId=' + selectId_deptDiv+'&'+Form.serialize('findCondictionFR');
