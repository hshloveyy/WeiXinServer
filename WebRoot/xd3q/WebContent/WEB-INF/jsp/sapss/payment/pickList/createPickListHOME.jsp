...
function selectPurchaseWin(){
	top.ExtModalWindowUtil.show('查询信用证信息',
	'pickListInfoController.spr?action=selectLcNoInfo',
	'',
	selectPurchaseInfoCallBack,
	{width:900,height:450});
}
var temp_WRITE_OFF_SINGLE_NO;
function selectPurchaseInfoCallBack(){
	...
	//Ext.getDom('writeListNo').value = returnvalue.WRITE_OFF_SINGLE_NO;	
	Ext.getDom('issuingBank').value = returnvalue.CREATE_BANK;	
	Ext.getDom('issuingDate').value = returnvalue.CREATE_DATE;	
	Ext.getDom('provider').value = returnvalue.EKKO_LIFNR_NAME;
	Ext.getDom('goods').value = returnvalue.GOODS;
	//temp_WRITE_OFF_SINGLE_NO = returnvalue.WRITE_OFF_SINGLE_NO;	
	Ext.getDom('lcNo').value = returnvalue.CREDIT_NO==null?'':returnvalue.CREDIT_NO;		
	dict_div_currencyId.setComboValue(returnvalue.EKKO_WAERS);
	dict_div_ymatGroup.setComboValue(returnvalue.YMAT_GROUP);		
	Ext.getDom('timeLimit').value = returnvalue.AVAIL_DATE;
		...
		Ext.Ajax.request({
			...
			success: function(response, options){
				...
				var p = new plant({
					    ...
					    EKPO_MEINS: responseArray.ekpoMeins,                             
...
<form id="mainForm" action="" name="mainForm">
	<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" >
      <tr>
        <td align="right">单号<font color="red">▲</font></td>
        <td align="left">
        	<input name="pickListNo" type="text" size="20" tabindex="1" value="${tPickListInfo.pickListNo}"/>
        </td>
        <td width="80" align="right">到单类型<font color="red">▲</font></td>
        <td align="left">
           <div id="div_pickListType"></div>
        </td>
		<td width="90" align="right">部门<font color="red">▲</font></td>
		<td>
			<div id="dept"></div>  创建者：${creatorName}
		</td>
      </tr>
      <tr>
        ...
        <td align="right">L/C No.<font color="red">▲</font></td>
        <td align="left">
			<input name="lcNo" type="text" size="20" tabindex="1" value="${tPickListInfo.lcNo}" readOnly="true"/>
        	<input type="button" value="..." onclick="selectPurchaseWin()"></input>
        	<input type="hidden" name="provider" value="${tPickListInfo.provider}"></input>"
        </td>
      ...
      <tr>
        ...
        <td align="right">单到日<font color="red">▲</font>：</td>
        <td align="left">
      ...
      <tr>
        <td align="right">贸管意见<font color="red">▲</font></td>
...
<div id="div_grid"></div>
<div id="div_accessory"></div>
<fiscxd:fileUpload divId="div_accessory" resourceId="2222" resourceName="33333" recordIdHiddenInputName="444" recordId="${tPickListInfo.pickListId}"></fiscxd:fileUpload>
</body>
</html>
...
<fiscxd:dictionary divId="div_pickListType" fieldName="pickListType" dictionaryName="BM_PICK_LIST_TYPE_LC" width="150" selectedValue="${tPickListInfo.pickListType}"></fiscxd:dictionary>
...
function checkInput(){
	...
	var issuingBank = Ext.getDom('issuingBank').value;
	var lcNo = Ext.getDom('lcNo').value;
	var issuingDate = Ext.getDom('issuingDate').value;
	/**
