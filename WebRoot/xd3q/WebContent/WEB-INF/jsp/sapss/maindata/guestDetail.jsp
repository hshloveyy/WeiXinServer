...
<table width="900" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <tr>
     ...
     <tr>
        <td ><div align="right">账户组<font color="red">▲</font></div></td>
      ...
      <td ><input type="text" name="periodEnd" value="${main.periodEnd}"/></td></tr></table>
        ...
        <td><input type="text"   name="annualInspection" value="${main.annualInspection}"/>
      ...
      <tr>
        ...
        <td colspan="4">
        <textarea name="otherinfo" rows="10" cols="100" style="overflow-y:visible;word-break:break-all;" >${main.otherinfo}</textarea></td>
        </tr>
				...
				if(uploadform.form.isValid()){
					...
					uploadform.getForm().submit({    
						...
						success: function(form, action){						   
						   ...
						   new AjaxEngine(encodeURI('<%=Constants.FILE_LOCATION_URL%>/attachementController.spr?action=getFilePath&id=' + Ext.getDom('guestId').value + '&boname=' + boname),
 ...
 var operateType='';	   
    function upcallBackHandle(xhr){
	   ...
	   operateType = 'save';
		...
		new AjaxEngine('guestController.spr',{method:"post", parameters: parm,onComplete: callBackHandle2});
    }
    ...
    {
    	if($('guestCode').value !=''){
    		var guestCode =$('guestCode').value;
    		if(guestCode.length != 10){
    			Ext.Msg.alert('提示','客户编号不足十位数请在前面补0，');
    			return;
    		}
    	}
    	if(!checkData()){
    	...
    	operateType = 'save';
        var parm="?action=save&"+Form.serialize('mainForm')+'&update=${modify}';
		new AjaxEngine('guestController.spr',{method:"post", parameters: parm,onComplete: callBackHandle});
    }
    function commitForm()
    {
    	if(!checkData('commit')){
    ...
    function addRadio(){
        ...
        $('updateDiv').innerHTML='<input type="text" name="guestCode" value="${main.guestCode}"/>';
     }
    function updateRadio(){
        ...
        dict_div_SaleGroup.disable(false);
        dict_div_companyCode.disable(false);
    }
   function updateRadio2(){        
	...
	function checkData(type){
		...
		if(type=='commit' || Ext.getDom('guestId').value != "" ){
			//如果是个人或员工不检查
	...
	function upload(id){
		...
		boname = id;			
		if(Ext.getDom('guestId').value == ""){
			top.ExtModalWindowUtil.alert('提示',"请先保存主数据在上传附件！");	
		}else{
			var url ='<%= request.getContextPath() %>/attachementController.spr?action=uploadManger&id=' + Ext.getDom('guestId').value + '&userId=' + Ext.getDom('creator').value + '&boname=' + boname;
			...
			if(fileUrl.indexOf(sername) != -1 && fileUrl.indexOf(port) != -1){			
				win.show();
				uploadform.url=encodeURI('<%=Constants.FILE_LOCATION_URL%>/attachementController.spr?action=upload3&cburl=http://<%= request.getServerName() %>:<%= request.getServerPort() %><%= request.getContextPath() %>/attachementController.spr&callback=upcallBackHandle&id=' + Ext.getDom('guestId').value + '&userId=' + Ext.getDom('creator').value + "&boname=" + boname);
			}else{
	...
	function delFile(id){
		...
		var guestId = Ext.getDom('guestId').value;
		var fileName = Ext.getDom(id).value;
		var path = id+'Path';
		if(fileName !=''){
			if(creator ==''){		
				...
				if(fileUrl.indexOf(sername) != -1 && fileUrl.indexOf(port) != -1){
					Ext.Ajax.request({
						url: encodeURI('<%=Constants.FILE_LOCATION_URL%>/attachementController.spr?action=proceeFileDelete&path='+id+'&fileName=' + fileName + '&creator=' + creator +'&masterid=') + guestId,
						params:'',
						scriptTag: true,
						success: function(response){
							
							new AjaxEngine('guestController.spr?action=delAtt&id=' + Ext.getDom('guestId').value + "&boname=" + id,{method:"post", parameters: '',onComplete: delcallBackHandle});
...
<fiscxd:workflow-taskHistory divId="div_history" businessRecordId="${main.guestId}" width="750"></fiscxd:workflow-taskHistory>
