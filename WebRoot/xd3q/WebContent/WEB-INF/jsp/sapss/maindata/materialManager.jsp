...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
    <tr >
      <td width="12%" height="20" align="right">物料编号:</td>
      <td width="25%" colspan="3"><input name="material_ID" type="text" size="20"/></td>
      <td width="8%" align="right">物料名称:</td>
      <td width="25%" colspan="3"><input name="material_Name" type="text" size="20"/></td>
      <td width="10%" align="right">物料组:</td>
      <td width="24%"><input name="material_Group" type="text" size="20" /></td>
    </tr>
    <tr align="center">
...
Ext.onReady(function(){
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'materialController.spr?action=find&state=3'}),
        		reader: new Ext.data.JsonReader({
            		...
            		{name:'MATERIAL_SALE_TAX'},
            		{name:'EXP_SALE_TAX'},
            		{name:'IMP_SALE_TAX'},
            		{name:'WITHIN_SALE_TAX'},
            		{name:'SALE_ORG'},
            		{name:'CREATOR'},
					{name:'CREATOR_TIME'},
					{name:'APPLY_TIME'},
    ...
    var addmaterialInfo = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('新增物料主数据申请',
			'materialController.spr?action=createMaterial',
			'',
					...
					if(records[0].json.EXAMINE_STATE == 1)
					{
					     top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
   									callbackFlag='del';
 									if(buttonId=='yes'){
 	 									var parm ='?action=delete&id='+records[0].json.MATERIAL_ID; 
    ...
    var itemCm = new Ext.grid.ColumnModel([
		...
		itemSm,{
           ...
           dataIndex: 'MATERIAL_CODE'
			...
			header: '物料销项税',
           	width: 50,
           	dataIndex: 'MATERIAL_SALE_TAX',
           	hidden:true,
           	sortable: true
           },{
			header: '出口订单销项税',
           	width: 50,
           	dataIndex: 'EXP_SALE_TAX',
           	hidden:true,
           	sortable: true
           },{
			header: '进口订单销项税',
           	width: 50,
           	dataIndex: 'IMP_SALE_TAX',
           	hidden:true,
           	sortable: true
           },{
			header: '内销订单销项税',
    ...
    var itemTbar = new Ext.Toolbar({
		items:[addmaterialInfo,'-',deletematerialInfo]
	});
...
//	alert(records[0].data.RECEIPT_ID);
	top.ExtModalWindowUtil.show('物料主数据审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.MATERIAL_ID,
		'',	'',	{width:880,height:400});
}
function modify(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('修改','materialController.spr?action=forModify&id='+record[0].json.MATERIAL_ID,'',find,{width:900,height:600});
}
function viewForm(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('查看','materialController.spr?action=forView&id='+record[0].json.MATERIAL_ID,'',find,{width:900,height:600});
}
function modifyCallback(){
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'materialController.spr?action=find&'+Form.serialize('findCondictionFR');
