...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
    <tr >
      ...
      <td width="10%" align="right">搜索条件:</td>
      <td width="24%"><input name="SEARCH_TERM" type="text" size="20" /></td>
    </tr>
    <tr align="center">
...
Ext.onReady(function(){
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'guestController.spr?action=find&state=3'}),
    ...
    var addmaterialInfo = new Ext.Toolbar.Button({
		...
		handler:function(){
			...
			'guestController.spr?action=createGuest',
			'',
					...
					if(records[0].json.EXAMINE_STATE == 1)
					{
					     top.Ext.Msg.show({
   								...
   								fn: function(buttonId,text){
   									callbackFlag='del';
 									if(buttonId=='yes'){
 	 									var parm ='?action=delete&id='+records[0].json.GUEST_ID; 
 										new AjaxEngine('guestController.spr',
...
//	alert(records[0].data.RECEIPT_ID);
	top.ExtModalWindowUtil.show('客户主数据审批详情信息','workflowController.spr?action=finishWorkDetailView&businessRecordId='+records[0].json.GUEST_ID,
		'',	'',	{width:880,height:400});
}
function modify(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('修改','guestController.spr?action=forModify&id='+record[0].json.GUEST_ID,'',find,{width:900,height:600});
}
function view(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('查看','guestController.spr?action=forView&id='+record[0].json.GUEST_ID,'',find,{width:900,height:600});
}
function modifyCallback(){
	ds.load({params:{start:0, limit:10}});
}
function find(){
	var url = 'guestController.spr?action=find&'+Form.serialize('findCondictionFR');
...
function guestCallback(){    
		var cb = top.ExtModalWindowUtil.getReturnValue();
	    	...
	    	new AjaxEngine('guestController.spr?action=updateCustomerMaster&kunnr=' + kunnr,
...
function updateMastercallBackHandle(trans){
		...
		top.ExtModalWindowUtil.show('修改',
				'guestController.spr?action=forModify2&id=' + customField.id + '&buttonType=' + buttonType,
				'',
				find,
				{width:900,height:600});
		}else{
