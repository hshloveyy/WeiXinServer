...
Ext.onReady(function(){
 ...
 function funAddShipDetailCallBack(){
    	...
    	var requestUrl = 'pledgeShipController.spr?action=addMaterielInfo';
    	requestUrl = requestUrl + "&pledgeShipsInfoId=" + "${main.pledgeShipsInfoId}";
		...
		Ext.Ajax.request({
			...
			success: function(response, options){
				...
				var p = new assetUserHisPlant({
				        PLEDGESHIPS_ITEM_ID: responseArray.pledgeShipsItemId,
				        PLEDGESHIPS_ID: responseArray.pledgeShipsId,
		...
		handler:function(){
		  if (assetUserHisgrid.selModel.hasSelection()){
				...
				for (var i=0;i<records.length;i++){
						idList = idList + records[i].data.PLEDGESHIPS_ITEM_ID + ',';
					}
					top.Ext.Msg.show({
						title:'提示',
	  						msg: '是否确定删除记录',
	...
	var assetUserHisPlant = Ext.data.Record.create([
		{name:'PLEDGESHIPS_ITEM_ID'},  			//明细ID
		{name:'PLEDGESHIPS_ID'},  				//进仓ID
	...
	assetUserHisds = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'pledgeShipController.spr?action=queryMaterial&pledgeShipsInfoId=' + '${main.pledgeShipsInfoId}'}),
    ...
    var assetUserHiscm = new Ext.grid.ColumnModel([
		...
		assetUserHissm,
           ...
           width: 80,
           sortable: true,
           dataIndex: 'PLEDGESHIPS_ITEM_ID',
           hidden: true
           },
           {header: '出仓ID',
           width: 80,
           sortable: true,
           dataIndex: 'PLEDGESHIPS_ID',
           hidden: true
           },  
		   {header: '物料编码',
           width: 80,
   ...
   assetUserHisds.load({params:{start:0, limit:10},arg:[]});
    ...
    function afterUserHisEdit(obj){
        ...
        var pledgeShipsItemId = row.get("PLEDGESHIPS_ITEM_ID");
        ...
        if (colName == 'QUANTITY'){
		    ...
		    var requestUrl ="pledgeShipController.spr?action=getShipQuality&" + Form.serialize('mainForm');
	        var materialCode = row.get("EKPO_MATNR");
	        var batchNo = row.get("BATCH_NO");
	        var pledgeShipsItemId = row.get("PLEDGESHIPS_ITEM_ID");
			requestUrl += "&materialCode=" +　materialCode;
			requestUrl += "&batchNo=" +　batchNo;
			requestUrl += "&pledgeShipsItemId=" +　pledgeShipsItemId;
		    Ext.Ajax.request({
				url: requestUrl,
				sync:true,
				success: function(response, options){
			...
			if(!isUpdate){
		       updateMaterielValue(row,pledgeShipsItemId,'QUANTITY',hasShipQuqlity);
		       row.set("QUANTITY",hasShipQuqlity);
		    }
		    else updateMaterielValue(row,pledgeShipsItemId,colName,colValue);
		}		
       ...
       var requestUrl ="pledgeShipController.spr?action=queryInventoryNum&" + Form.serialize('mainForm');
	        var materialCode = row.get("EKPO_MATNR");
	        var batchNo = row.get("BATCH_NO");
	        var pledgeShipsItemId = row.get("PLEDGESHIPS_ITEM_ID");
			requestUrl += "&materialCode=" +　materialCode;
       ...
       Ext.Ajax.request({
			...
			success: function(response, options){
					...
					var customField = responseUtil1.getCustomField("coustom");
	  				...
	  				var price = customField.price;
	  				var currency = customField.currency;
	  				var tiaojiandingjiadanwei = customField.tiaojiandingjiadanwei;
	  				var hasShipQuqlity = customField.hasShipQuqlity;
	  				queryInventoryNum(row,price,currency,tiaojiandingjiadanwei,hasShipQuqlity);
	  				}
				},
				failure:function(response, options){}
			});	

        if(!isUpdate) updateMaterielValue(row,pledgeShipsItemId,'BATCH_NO','');
			else updateMaterielValue(row,pledgeShipsItemId,colName,colValue);
        }
        //数量或单价
        if (colName == 'QUANTITY' || colName == 'PRICE'||colName=='EKPO_PEINH'){
        	setMaterielTotal(row);
        	updateMaterielValue(row,pledgeShipsItemId,colName,colValue);
        }
        else updateMaterielValue(row,pledgeShipsItemId,colName,colValue);
    } 
    
     //更新物料信息
    function updateMaterielValue(row,pledgeShipsItemId,colName,colValue){
    	var requestUrl = 'pledgeShipController.spr?action=updateShipMateriel';
		requestUrl = requestUrl + '&pledgeShipsItemId=' + pledgeShipsItemId;
     ...
     function setMaterielTotal(row){   
        var pledgeShipsItemId = row.get("PLEDGESHIPS_ITEM_ID");
     	...
     	updateMaterielValue(row,pledgeShipsItemId,"TOTAL",Utils.roundoff(total,2));
     }   
    //刷新库存数量
     function　queryInventoryNum(row,price,currency,tiaojiandingjiadanwei,hasShipQuqlity){
         row.set("PRICE",price);
         row.set("CURRENCY",currency);
		 row.set("EKPO_PEINH",tiaojiandingjiadanwei);
		 row.set("QUANTITY",hasShipQuqlity);
		 var total = parseFloat(hasShipQuqlity) *  parseFloat(price)/parseFloat(tiaojiandingjiadanwei);
		 row.set("TOTAL",formatnumber(total,2));		 
     }
	...
	//申报日期
   	...
   	//出仓时间
   	var shipsTime = new Ext.form.DateField({
   		format:'Y-m-d',
		id:'shipsTime',
	    name:'shipsTime',
		width: 160,
...
function selectProjectInfoCallBack(){
	...
	document.mainForm.unitName.value = returnvalue.CUSTOMER_LINK_MAN;
}
...
<form id="mainForm" action="" name="mainForm">
     ...
     <tr>
        ...
        <td width="22%" align="left">
            <input type="hidden" id="pledgeShipsInfoId" name="pledgeShipsInfoId" value="${main.pledgeShipsInfoId}"></input>
        ...
        <td width="11%" align="right">出仓单编号：</td>
        <td width="22%" align="left">
			<input name="pledgeShipsInfoNo" readonly="readonly" type="text" size="20" tabindex="1" value="${main.pledgeShipsInfoNo}"/> 
        </td>
      ...
      <tr>
        ...
        <td width="11%" align="right">出仓时间：</td>
        <td width="22%" align="left">
			<input id="shipsTime" name="shipsTime" type="text"  value="${main.shipsTime}"/>        
        </td>      
      </tr>     
      <tr>
...
<fiscxd:workflow-taskHistory divId="div_history" businessRecordId="${main.pledgeShipsInfoId}" width="750"></fiscxd:workflow-taskHistory>
<fiscxd:fileUpload divId="rule" recordId="${main.pledgeShipsInfoId}" erasable="${!save}"  increasable="${!save}" 
