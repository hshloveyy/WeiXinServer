...
<table width="100%" border="1" cellpadding="4" cellspacing="1"
	...
	<tr>
		...
		<td><input type="button" value="查找" onclick="findShipNotifyInfo()"><input type="reset" value="清空"></td>
		<td></td>
	</tr>
...
function operaForm(exportApplyId){
	...
	top.ExtModalWindowUtil.show(strshipNotifyTitle + '出口货物通知单信息',
		'shipNotifyController.spr?action=addShipNotifyView&exportApplyId='+ exportApplyId + "&tradeType=" + tradeType + "&operType=11",
		'',
		ShipNotifycallbackFunction,
		{width:900,height:550});
}

function subimWorkflow(exportApplyId){
	if (exportApplyId == null || exportApplyId == ''){
		var records = shipNotifyInfogrid.selModel.getSelections();		
		exportApplyId = records[0].data.EXPORT_APPLY_ID;
	}
	...
	top.ExtModalWindowUtil.show(strshipNotifyTitle + '出口货物通知单信息',
		'shipNotifyController.spr?action=addShipNotifyView&exportApplyId='+ exportApplyId + "&tradeType=" + tradeType + "&operType=01",
		'',
		ShipNotifycallbackFunction,
		{width:900,height:550});
}
...
function viewForm(exportApplyId){
	...
	top.ExtModalWindowUtil.show(strshipNotifyTitle + '出口货物通知单信息',
		'shipNotifyController.spr?action=addShipNotifyView&exportApplyId='+ exportApplyId + "&tradeType=" + tradeType + "&operType=00",
		'',
		ShipNotifycallbackFunction,
		{width:900,height:550});
}
