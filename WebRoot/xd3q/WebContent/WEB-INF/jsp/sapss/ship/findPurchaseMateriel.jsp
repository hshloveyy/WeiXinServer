...
function findPurchaseInfo(){
	...
	var requestUrl  = 'shipController.spr?action=queryPurchaseMateriel&' + param;
...
Ext.onReady(function(){
   ...
   var Purchasegrid = new Ext.grid.EditorGridPanel({
    	id:'Purchasegrid',
        ds: Purchaseds,
        cm: Purchasecm,
        sm: Purchasesm,
			...
			buttons:[{
				...
				handler:function(){
					if (Purchasegrid.selModel.hasSelection()){
						...
						if (records[0].data.LINES<1)
							top.ExtModalWindowUtil.alert('提示','分行数必须大于0！');
						else
						{
							if (records.length > 1){
