...
<table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#6699FF" class="datatable">
	...
	<td width="20%" >
		<div id="dept"></div>
		<input type="hidden" name="returnType" type="text" size="20" value="1"/>
	</td>    
      <td width="10%" height="20" align="right">立项号:</td>
     <td width="10%"><input name="projectNo" type="text" size="15"/></td>
      <td width="10%" height="20" align="right">合同编码:</td>
      <td><input name="contractNo" type="text" size="15"/></td>
      <td width="15%" height="20" align="right">SAP订单号(原单):</td>
    ...
    <tr >
      ...
      <td><input name="oldContractNo" type="text" size="15"/></td>
      <td  align="right">SAP订单号(退货):</td>
      <td><input name="sapOrderNo" type="text" size="15"/></td>      
    </tr>
    <tr align="center">
...
Ext.onReady(function(){
    ...
    var add = new Ext.Toolbar.Button({
	    ...
	    iconCls:'add',
		handler:function(){
			top.ExtModalWindowUtil.show('已开票退货信息','salesReturnController.spr?action=toSalesReturnInfo&returnType=1','',find,{width:800,height:600,closable:false});
		}
   	});
	...
	var itemStore = new Ext.data.Store({
        		proxy : new Ext.data.HttpProxy({url:'salesReturnController.spr?action=find&returnType=1'}),
...
function modify(){
	...
	top.ExtModalWindowUtil.show('已开票退货信息','salesReturnController.spr?action=forModify&id='+record[0].json.RETURN_ID,'',find,{width:800,height:600,closable:false});
}

function view(){
	var record = tempGrid.selModel.getSelections();
	top.ExtModalWindowUtil.show('已开票退货信息','salesReturnController.spr?action=forView&id='+record[0].json.RETURN_ID,'',find,{width:800,height:600,closable:false});
}
