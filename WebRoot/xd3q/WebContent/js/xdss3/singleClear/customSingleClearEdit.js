 ...
 * 
 * @(#) Description: <功能>主对象客户单清帐(CustomSingleClear)编辑页面用户可编程扩展JS文件
 */
	...
	if (transport.responseText)
	{
        ...
        if(result==null || result ==""){
			...
			Ext.getCmp("tabs").on('tabchange',function(t,p){
	     	...
	     	_getMainFrame().maintabs.addPanel('客户单清凭证预览', '', contextPath + '/xdss3/voucher/voucherController.spr?action=_manage&businessid=' + result,closeVoucherCallBack,true);
        }
        **/
        	Ext.get('currentWorkflowTask').mask();
			Ext.get('centercontent').mask();
