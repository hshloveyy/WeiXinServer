/**
  ...
  * Date: 2010年06月11日 03点10分49秒
  * Copyright Notice:版权声明 福建讯盟软件有限公司, 版权所有 违者必究
  * @(#)
  * Description:  
  * <功能>主对象票清预收款(BillClearCollect)增加页面用户可编程扩展JS文件
 */

Ext.onReady(function(){
	Ext.getCmp("_voucherPreview").hide();
	Ext.get("BillClearCollect.cleartype").dom.value = '1';
	Ext.get("BillClearCollect.businessstate").dom.value = '0';
	...
	if(isReassign == 'Y'){
		...
		Ext.getCmp('_submitForReassign').show();
	}
	...
	/*
	div_customer_sh_sh.on('change',function(e,n,o){
		searchCustomerTitleWin.defaultCondition = 'KUNNR='+n;
	});
	
	BillClearItem_grid.on('rowclick',function(grid, rowIndex, e){
		//var fieldName = grid.getColumnModel().getDataIndex(columnIndex); var data = record.get(fieldName);
	    var record = grid.getStore().getAt(rowIndex);  // Get the Record
	    BillInCollect_grid.getStore().filter('billclearitemid',record.get('billclearitemid'));

	    for(var i=0;i<BillInCollect_grid.getStore().getCount();i++){
	    	var rec = BillInCollect_grid.getStore().getAt(i);
	    	var nowclearedamount = BillInCollect_grid.getStore().sumquery('collectitemid',rec.get('collectitemid'),'nowclearamount');
			rec.set('nowclearedamount',nowclearedamount-rec.get('nowclearamount'));
	    }
	    //BillInCollect_grid.getStore().commitChanges();
	});
	*/
...
function winCollectItemCallBack(jsonArrayData)
{
	...
	for(var i = 0;i < jsonArrayData.length;i++){
		...
		if(num == -1){
			collectitemids += jsonArrayData[i].COLLECTITEMID + ",";
			collectids += jsonArrayData[i].COLLECTID + ",";
		}
	}
	...
	if(collectitemids!=''){
		Ext.Ajax.request({
	        ...
	        params : {collectitemids:collectitemids,collectids:collectids,billclearid:billclearid},
	        success : function(xhr){
				if(xhr.responseText){
