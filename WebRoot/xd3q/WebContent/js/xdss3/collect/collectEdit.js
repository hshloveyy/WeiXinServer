...
function _presubmitProcessCollect(){
	...
	if ((ps=='资金部出纳审核' || ps=='出纳审核' || 
		 ps=='资金部出纳确认票据到期收款' || ps=='出纳确认票据到期收款' ||
		 ps=='资金部出纳确认' || ps=='出纳确认'||
	 ...
	 */
	if((ps=='资金部审核票据并登记相关信息'||ps=='审核票据并登记相关信息') && (collectType=='11' || collectType=='12')){
...
function _voucherPreviewCollect(){
	...
	if(ps=='财务会计审核贴现或议付并做帐'){
		if(Ext.getDom('Collect.toldisnegamount').value!=Ext.getDom('Collect.applyamount').value){
			Ext.getDom('workflowLeaveTransitionName').value = '部分议付返回资金部';
		}else{
			Ext.getDom('workflowLeaveTransitionName').value = '确认贴现或议付已做帐';
		}
	}
...
function _submitVoucher(){
	...
	// 邱杰烜 2011-05-19 生成贴现/议付凭证
	if ($('Collect.processstate').value == '财务会计审核贴现或议付并做帐'){
		url = contextPath+'/xdss3/collect/collectController.spr?action=voucherPreview&type=6';
	}
		...
		callback : function(){
			if(transport.responseText){
				...
				for(var j=0;j<CollectBankItem_grid.getStore().getCount();j++){
					...
					var vflag = CollectBankItem_grid.getStore().getAt(j).get('flag');
					if(colbankitemid && vflag!='1'){
				...
				var collectbankaccs = '';
				for(var j=0;j<CollectBankItem_grid.getStore().getCount();j++){
					var collectbankacc = CollectBankItem_grid.getStore().getAt(j).get('collectbankacc');
					if(collectbankacc){
						collectbankaccs += collectbankacc + ',';
					}
				}
				
				//alert("获取journalType");
				Ext.Ajax.request({
    				url : contextPath+'/xdss3/collect/collectController.spr?action=getJournalType',
        			params : {colbankitemids:collectbankaccs},
        			success : function(xhr){
						if(xhr.responseText){
				        	var jsonData = Ext.util.JSON.decode(xhr.responseText);
				        	var journalType = jsonData.journalType;
							//if($('Collect.processstate').value == '综合二部审核')
							//alert(journalType);
...
function mainInit(){
	if (roletype == "1"){
		...
		CollectBankItem_grid_cm.setHidden(CollectBankItem_grid_cm.findColumnIndex('cashflowitem'),false);
...
//		calendar_Collect_accountdate.setEditable(false);		// 设置记账日期只读
	...
	if(isReassign!='Y'){		// 若不为重分配
		if(ps.trim()=='' || ps=='修改'){
			...
			tb.hideTabStripItem('settleSubjectTab');		// 隐藏（页签）结算科目	
			tb.hideTabStripItem('fundFlowTab');				// 隐藏（页签）纯资金往来	
		...
		// 以下这两个节点的只在票据时才有
		if(ps=='资金部审核票据并登记相关信息'|| ps=='审核票据并登记相关信息' || ps=='财务会计审核票据并做帐'){
			tb.hideTabStripItem('collectBankItemTab');		// 隐藏（页签）收款银行页签
		}
		// 邱杰烜 2011-05-18 "累计贴现/议付金额"在以下这个节点时触发累计
		if(ps=='财务会计审核贴现或议付并做帐'){
			// 结算科目金额
			Ext.getDom('SettleSubject.amount1').onchange = calcTotalDisNegAmount;
			Ext.getDom('SettleSubject.amount2').onchange = calcTotalDisNegAmount;
			Ext.getDom('SettleSubject.amount3').onchange = calcTotalDisNegAmount;
			Ext.getDom('SettleSubject.amount4').onchange = calcTotalDisNegAmount;
			// 结算科目借贷
			dict_div_SettleSubjectdebtcredit1_dict.on('select', calcTotalDisNegAmount);
			dict_div_SettleSubjectdebtcredit2_dict.on('select', calcTotalDisNegAmount);
			dict_div_SettleSubjectdebtcredit3_dict.on('select', calcTotalDisNegAmount);
			dict_div_SettleSubjectdebtcredit4_dict.on('select', calcTotalDisNegAmount);
			// 纯资金往来金额
			Ext.getDom('FundFlow.amount1').onchange = calcTotalDisNegAmount;
			Ext.getDom('FundFlow.amount2').onchange = calcTotalDisNegAmount;
			Ext.getDom('FundFlow.amount3').onchange = calcTotalDisNegAmount;
			// 纯资金往来借贷
			dict_div_FundFlowdebtcredit1_dict.on('select', calcTotalDisNegAmount);
			dict_div_FundFlowdebtcredit2_dict.on('select', calcTotalDisNegAmount);
			dict_div_FundFlowdebtcredit3_dict.on('select', calcTotalDisNegAmount);
			// 设置纯资金往来Form与纯资金往来Form下的输入框不可编辑
			var settleSubjectFlag = new Number(Ext.getDom('SettleSubject.flag').value);
			var fundFlowFlag = new Number(Ext.getDom('FundFlow.flag').value);
			setSettleSubjectCmpsUneditable(settleSubjectFlag);
			setFundFlowCmpsUneditable(fundFlowFlag);
			calcTotalDisNegAmount();
		}
...
//			CollectBankItem_grid_cm.setHidden(CollectBankItem_grid_cm.findColumnIndex('cashflowitem_text'),true);
	...
	if(isDeposit == 'Y'){	// 若为保证金转货款
		...
		dict_div_incsuretybond_dict.setEditable(false);
	}
}
...
//		}
	...
	Ext.getDom('Collect.settlerate').onchange = function(){				// 设置结算汇率失去焦点时触发
	 ...
	 * 邱杰烜 2011-05-17 “收款银行行项”下，如果本条数据标识为“1”，则该条数据不让修改或删除
	 */
	/***********************************************************************/
	/*
	 * 收款金额分配页签
	...
	function calcCollectAmount(e){
		...
		// 邱杰烜 2011-05-18 在“财务会计审核贴现或议付并做帐”节点更改金额时重新计算“累计贴现/议付金额”
		if(cField == 'collectamount' && ps=='财务会计审核贴现或议付并做帐'){
			calcTotalDisNegAmount();
		}
	}
	// 如果本条数据标识为“1”，则该条数据不让修改或删除
	CollectBankItem_grid.on('beforeedit', isEditable, CollectBankItem_grid);
	function isEditable(e){
		var cRecord = e.record;
		if(cRecord.get('flag')=='1'){
			return false;
		}
	}
 ...
 * @创建作者：邱杰烜
 * @创建日期：2011-05-18
 * 计算“累计贴现/议付金额” = 结算科目金额 + 纯资金往来金额 + 收款银行金额
 */
function calcTotalDisNegAmount(){
	// 结算科目金额
	var ssAmount1 = Utils.isNumber(Ext.getDom('SettleSubject.amount1').value) ? parseFloat(Ext.getDom('SettleSubject.amount1').value) : 0;
	var ssAmount2 = Utils.isNumber(Ext.getDom('SettleSubject.amount2').value) ? parseFloat(Ext.getDom('SettleSubject.amount2').value) : 0;
	var ssAmount3 = Utils.isNumber(Ext.getDom('SettleSubject.amount3').value) ? parseFloat(Ext.getDom('SettleSubject.amount3').value) : 0;
	var ssAmount4 = Utils.isNumber(Ext.getDom('SettleSubject.amount4').value) ? parseFloat(Ext.getDom('SettleSubject.amount4').value) : 0;
	// 结算科目借贷
	var ssDebcrd1 = dict_div_SettleSubjectdebtcredit1_dict.getValue();
	var ssDebcrd2 = dict_div_SettleSubjectdebtcredit2_dict.getValue();
	var ssDebcrd3 = dict_div_SettleSubjectdebtcredit3_dict.getValue();
	var ssDebcrd4 = dict_div_SettleSubjectdebtcredit4_dict.getValue();
	// 结算科目金额：借为负，贷为正
	if(ssDebcrd1 == 'H'){ ssAmount1 *= -1; }
	if(ssDebcrd2 == 'H'){ ssAmount2 *= -1; }
	if(ssDebcrd3 == 'H'){ ssAmount3 *= -1; }
	if(ssDebcrd4 == 'H'){ ssAmount4 *= -1; }
	// 纯资金往来金额
	var ffAmount1 = Utils.isNumber(Ext.getDom('FundFlow.amount1').value) ? parseFloat(Ext.getDom('FundFlow.amount1').value) : 0;
	var ffAmount2 = Utils.isNumber(Ext.getDom('FundFlow.amount2').value) ? parseFloat(Ext.getDom('FundFlow.amount2').value) : 0;
	var ffAmount3 = Utils.isNumber(Ext.getDom('FundFlow.amount3').value) ? parseFloat(Ext.getDom('FundFlow.amount3').value) : 0;
	// 纯资金往来借贷
	var ffDebcrd1 = dict_div_FundFlowdebtcredit1_dict.getValue();
	var ffDebcrd2 = dict_div_FundFlowdebtcredit2_dict.getValue();
	var ffDebcrd3 = dict_div_FundFlowdebtcredit3_dict.getValue();
	// 纯资金往来金额：借为负，贷为正
	if(ffDebcrd1 == 'H'){ ffAmount1 *= -1; }
	if(ffDebcrd2 == 'H'){ ffAmount2 *= -1; }
	if(ffDebcrd3 == 'H'){ ffAmount3 *= -1; }
	// 收款银行金额
	var collectAmount = 0;
	for(var i=0;i<CollectBankItem_store.getCount();i++){
//		if (CollectBankItem_store.getAt(i).get('flag') == '1') {
			collectAmount += parseFloat(CollectBankItem_store.getAt(i).get('collectamount'));
//		}
	}
	var toldisnegamount = ssAmount1 + ssAmount2 + ssAmount3 + ssAmount4 + ffAmount1 + ffAmount2 + ffAmount3 + collectAmount;
	Ext.getDom('Collect.toldisnegamount').value = round(toldisnegamount, 2);
}
 ...
 * 设置结算科目Form下的输入框不可编辑 
 */
function setSettleSubjectCmpsUneditable(flag){
	for(var i=1; i<=flag; i++){
		eval('dict_div_SettleSubjectdebtcredit' + i + '_dict').setEditable(false);	// 借贷
		Ext.getDom('SettleSubject.amount' + i).readOnly = true;						// 金额
		Ext.getDom('SettleSubject.standardamount' + i).readOnly = true;				// 本位币金额
		eval('div_SettleSubjectsettlesubject' + i + '_sh_sh').setEditable(false);	// 结算科目
		eval('div_SettleSubjectcostcenter' + i + '_sh_sh').setEditable(false);		// 成本中心
		eval('div_SettleSubjectdepid' + i + '_sh_sh').setEditable(false);			// 业务范围
		eval('div_SettleSubjectorderno' + i + '_sh_sh').setEditable(false);			// 销售订单
		Ext.getDom('SettleSubject.rowno' + i).readOnly = true;						// 行项目
		eval('dict_div_SettleSubjectantiaccount' + i + '_dict').setEditable(false);	// 反记账
	}
}
 ...
 * @创建作者：邱杰烜
 * @创建日期：2011-05-18
 * 设置纯资金往来Form下的输入框不可编辑 
 */
function setFundFlowCmpsUneditable(flag){
	for(var i=1; i<=flag; i++){
		eval('dict_div_FundFlowdebtcredit' + i + '_dict').setEditable(false);		// 借贷
		Ext.getDom('FundFlow.amount' + i).readOnly = true;							// 金额
		Ext.getDom('FundFlow.standardamount' + i).readOnly = true;					// 本位币金额
		eval('dict_div_FundFlowspecialaccount' + i + '_dict').setEditable(false);	// 特殊总账
		eval('div_FundFlowcustomer' + i + '_sh_sh').setEditable(false);				// 客户
		eval('div_FundFlowdepid' + i + '_sh_sh').setEditable(false);				// 业务范围
		eval('dict_div_FundFlowantiaccount' + i + '_dict').setEditable(false);		// 反记账
	}
}
