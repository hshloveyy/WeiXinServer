...
//    });
    ...
    var suretybond_cIndex = Collect_grid.getColumnModel().findColumnIndex('SURETYBOND_C');
    Collect_grid.getColumnModel().setRenderer(suretybond_cIndex,function(value,css,record,rowIndex,colIndex,store){
        return value.replace(re,"$1,");
    });
    ...
    var arrPS = [ ["'0'",'新增'],
                 ...
                 ["'3'",'审批通过'] ];
    var arr2P = [ [ "*", "*" ] ];
    var t_bstypeField = new Ext.form.MultiSelectField({
        applyTo : 't_bsMultiselect',
        hiddenName : 'businessstate',
        contextArray : arrPS,
        fieldLabel : '11',
        id : 't_bstypeField',
        defaltValueArray : arr2P,
...
function reload_Collect_grid2(urlParmeters) {
    var paraUrl = encodeURI(context
            + "/xdss3/collect/collectController.spr?action=_suretyBondContractDetailQuery")
            + "&" + urlParmeters;
...
function _expExcel()
{
    var para = Form.serialize('mainForm');
    para = encodeURI(para);
    window.location.href(contextPath+'/xdss3/collect/collectController.spr?action=_suretyBondContractDetailToExcel&'+para);
}
