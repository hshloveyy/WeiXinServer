/**
 * Author(s):java业务平台代码生成工具 Date: 2010年06月23日 12点02分18秒 Copyright Notice:版权声明
 * 福建讯盟软件有限公司, 版权所有 违者必究
 * 
 * @(#) Description: <功能>主对象票清付款(BillClearPayment)增加页面用户可编程扩展JS文件
 */
...
//	Ext.getDom('BillClearPayment.accountdate').style.display = "none";
//	Ext.getDom('BillClearPayment.voucherdate').style.display = "none";
...
function winBillClearItemPayCallBack(jsonArrayData)
{
	...
	{
		// alert(jsonArrayData[i].VENDORTITLEID + ":" +
		// jsonArrayData[i].INVOICE);
...
function _addBillClearItemPay()
{
	...
	if (pre_addBillClearItemPay())
	{
		...
		//searchBillClearItemPayHelpWin.defaultCondition="LIFNR='" + supplier +  "' and SHKZG='H' " ;
		// 方法执行体
...
function _addBillInPayment()
{
	if (pre_addBillInPayment())
	{
		...
		//searchPaymentItemWin.defaultCondition="SUPPLIER='" + supplier +  "' and (BUSINESSSTATE ='4' or BUSINESSSTATE = '7' ) " ;
		// 方法执行体
...
Ext.onReady(function()
        {
            ...
            div_supplier_sh_sh.on('change', function(e, n, o)
                    {
                        searchBillClearItemPayHelpWin.defaultCondition = " INVOICE is not null and INVOICE!=' ' and LIFNR='" + n + "' and ISCLEARED='0'";
