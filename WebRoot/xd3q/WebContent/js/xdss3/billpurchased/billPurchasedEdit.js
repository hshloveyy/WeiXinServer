/**
  ...
  * @(#)
  * Description:  
  * <功能>主对象出口押汇(BillPurchased)编辑页面用户可编程扩展JS文件
 */
...
//		Ext.getDom('BillPurchased.text').readOnly=true; // 抬头文本(还押汇用途)
	...
	function afterEditForBillPurBankItem (e) {
    	...
    	var billpurrate = parseFloat(Ext.getDom('BillPurchased.billpurrate').value);
    	var bankrate = record.get("bankrate");
    	if ( (e.field == 'realmoney' || e.field == 'bankrate' || e.field == 'applyamount') && Utils.isNumber(billpurrate)) {
    		Ext.getDom('BillPurchased.billpurrate').value = parseFloat(bankrate);
    		billpurrate = parseFloat(Ext.getDom('BillPurchased.billpurrate').value);
    		if ( Utils.isNumber(billpurrate * record.get("realmoney")) ) {
    			record.set('realmoney1', round((billpurrate * record.get("realmoney")),2));
    		}
    		if ( Utils.isNumber(billpurrate * record.get("applyamount")) ) {
    			record.set('applyamount2', round((billpurrate * record.get("applyamount")),2));
    		}
    		record.set('bankrate', billpurrate);
    	}
	...
	function afterEditForBillPurReBankItem (e) {
		var record = e.record;// 被编辑的记录
    	var billpurrate2 = parseFloat(Ext.getDom('BillPurchased.billpurrate').value);
    	var billpurrate =parseFloat(record.get('rebillpurrate'));
    	if ( (e.field == 'realmoney' || e.field == 'rebillpurrate' || e.field == 'applyamount')&& Utils.isNumber(billpurrate)) {
    		...
    		record.set('applyamount2', round((billpurrate2 * record.get("applyamount")),2));
    	}
	...
	// 还押汇银行2 汇率自动计算
	BillPurReBankTwo_grid.on("afterEdit", afterEditForBillPurBankTwo, BillPurReBankTwo_grid);
	function afterEditForBillPurBankTwo (e) {
		var record = e.record;// 被编辑的记录
    	//var rebillpurrate2 = parseFloat(record.get("rebillpurrate2"));
    	var rebillpurrate2 = parseFloat(Ext.getDom('BillPurchased.billpurrate').value);
    	var applyamount3 = parseFloat(record.get("applyamount3"));
    	var applyamount4 = parseFloat(record.get("applyamount4"));
    	if ( e.field == 'applyamount3' || e.field == 'applyamount4' || e.field == 'rebillpurrate2') {
    		if ( Utils.isNumber(applyamount3 * rebillpurrate2) ) {
    			record.set('applyamount4', round((applyamount3 * rebillpurrate2),2));
    		}
    	}
		BillPurReBankTwo_grid.recalculation();
	}
		// 还押汇银行2 改银行限定条件
	BillPurReBankTwo_grid.on("beforeEdit", beforeEditForBillPurBankTwo, BillPurReBankTwo_grid);
	function beforeEditForBillPurBankTwo (e) {
    	if ( e.field == 'bankacc3' ) {
			var gridlist = BillPurReBankTwo_grid.getColumnModel().columns;
	...
	Ext.getDom('BillPurchased.billpurrate').onchange = function (){
//		alert(this.value);
    	var billpurrate = parseFloat(Ext.getDom('BillPurchased.billpurrate').value);
    	for(var i=0;i<BillPurBankItem_grid.getStore().getCount();i++){
			var record = BillPurBankItem_grid.getStore().getAt(i);
    		if ( Utils.isNumber(billpurrate * record.get("realmoney")) ) {
    			record.set('realmoney1', round((billpurrate * record.get("realmoney")),2));
    		}
    		if ( Utils.isNumber(billpurrate * record.get("applyamount")) ) {
	...
	// 若业务状态为2，则把预确认按钮隐藏
	if(Ext.getDom('BillPurchased.businessstate').value=='2'){
 ...
 * 增加预确认功能
 */
function  _prepConfirmBillPurchased(){
 var param = '';
    new AjaxEngine(contextPath + '/xdss3/billpurchased/billPurchasedController.spr?action=_prepConfirmBillPurchased&businessid='+Ext.getDom('BillPurchased.billpurid').value, 
     {method:"post", parameters: param, onComplete: callBackHandle,callback:_prepConfirmBillPurchasedCallBackHandle});
}
function _prepConfirmBillPurchasedCallBackHandle(){
...
function commonValidate(){
	...
	for(var i=0;i<BillPurBankItem_grid.getStore().getCount();i++){
		...
		var applyamount = BillPurBankItem_grid.getStore().getAt(i).get('applyamount');
		if (applyamount=='' || applyamount == ' ') {
			_getMainFrame().showInfo('[押汇银行－实际押汇金额]存在为空数据，请添加！');
			return false;
		}
		var applyamount2 = BillPurBankItem_grid.getStore().getAt(i).get('applyamount2');
		if (applyamount2=='' || applyamount2 == ' ') {
			_getMainFrame().showInfo('[押汇银行－实际押汇金额本位币]存在为空数据，请添加！');
			return false;
		}
		var realmoney = BillPurBankItem_grid.getStore().getAt(i).get('realmoney');
		if (realmoney=='' || realmoney == ' ') {
			_getMainFrame().showInfo('[押汇银行－实际收款金额]存在为空数据，请添加！');
			return false;
		}
		var realmoney1 = BillPurBankItem_grid.getStore().getAt(i).get('realmoney1');
		if (realmoney1=='' || realmoney1 == ' ') {
			_getMainFrame().showInfo('[押汇银行－实际收款金额本位币]存在为空数据，请添加！');
			return false;
		}
	}
	for(var i=0;i<BillPurReBankItem_grid.getStore().getCount();i++){
		var bankacc = BillPurReBankItem_grid.getStore().getAt(i).get('bankacc');
		if (bankacc=='' || bankacc == ' ') {
			_getMainFrame().showInfo('[还押汇银行－还押汇银行帐号]存在为空数据，请添加！');
			return false;
		}
		...
		var applyamount = BillPurReBankItem_grid.getStore().getAt(i).get('applyamount');
		if (applyamount=='' || applyamount == ' ') {
//			_getMainFrame().showInfo('[还押汇银行－实际还押汇金额]存在为空数据，请添加！');
//			return false;
		}
		var applyamount2 = BillPurReBankItem_grid.getStore().getAt(i).get('applyamount2');
		if (applyamount2=='' || applyamount2 == ' ') {
//			_getMainFrame().showInfo('[还押汇银行－实际还押汇金额本位币]存在为空数据，请添加！');
//			return false;
		}
	}
	...
	if (ps == '业务填写相关信息') {
		...
		var applyamount = Ext.getDom("BillPurchased.payrealdate").value;
		if (payrealdate=='') {
			_getMainFrame().showInfo('请添加[押汇收款日]！');
			return false;;
		}
...
function voucherCallBackHandle(transport)
{
	...
	BillPurReBankTwo_grid.getStore().reload();
	BillPurReBankTwo_grid.getStore().commitChanges();
			...
			onComplete:function(transport){
				...
				for(var i=0;i<creditTypes.length;i++){
					...
					var p = new Ext.data.Record({
						...
						applyamount2:round( tApplyamount.value * billpurrate.value, 2),
...
function _addReBankBillPurReBankItem()
{
	...
	var gridlist = BillPurReBankItem_grid.getColumnModel().columns;
	for(var i=0;i<gridlist.length;i++){
		var col = gridlist[i];
		if (col.dataIndex =="bankacc" )
		{
			col.editor.field.defaultCondition = " BANK_ACCOUNT<>' '  and Isusing='1' ";
		} // end if
	} // end for
