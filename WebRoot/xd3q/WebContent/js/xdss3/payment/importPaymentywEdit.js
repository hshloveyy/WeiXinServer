...
function _checkDocumentaryCredit(){
	...
	if(ps=='填写办理押汇信息'){
		...
		if(dRealDate==''){
			_getMainFrame().showInfo("[实际付款日期(押汇)]不能为空！");
			return false;
...
function _checkImportPayBankItem(){
	...
	if((ps.indexOf('出纳付押汇到期款')>=0 || 
		...
		ps=='资金部出纳付款' ||
		ps=='出纳付款' ||
		ps=='资金部出纳付押汇到期款') && (ImportPayBankItem_store.getCount()<=0)){
		_getMainFrame().showInfo("进口付款银行不能为空！");
		return false;
	}**/
	if((ps=='出纳确认办理押汇' || 
		ps=='资金部出纳付款' ||
...
function _presubmitProcessImportPayment(){
	...
	if((document.getElementById('ImportPayment.processstate').value == '资金部出纳付款'||document.getElementById('ImportPayment.processstate').value == '出纳付款') && factcurrency == ''){
...
function _addPlickImportPaymentItem(){
	...
	searchPickHelpWin.defaultCondition = "DEPT_ID='" + div_dept_id_sh_sh.getValue() + "' " + "and PROJECT_STATE!='9' AND IS_AVAILABLE='1' " + "and trim(paymentid) is null or (businessstate='-1' and pick_list_id not in ( select pick_list_no from ypaymentitem left join ypayment on ypaymentitem.paymentid=ypayment.paymentid where businessstate<>'-1'))";
		...
		if (isExists == false){
			...
			if(pt.value == '15'  ||  pt.value == '16'){
				...
				if (dict_div_isrepresentpay_dict.getValue() == '1'){
					...
					if (ps == '部门会计审核押汇到期付款' || ps.indexOf('出纳付押汇到期款')>=0 || ps=='财务会计审核押汇到期付款并做帐') {
		...
		callback:function(){ImportDocuBankItem_grid.getStore().reload({
				callback : function(){
					if(transport.responseText){
						...
						if(paytype == '2'){ 
							if(isoverrepay == 0){
								if(document.getElementById('ImportPayment.processstate').value == '出纳确认办理押汇'){
									var str = getPayBankAndDocuBankID();
									var as = str.split('&');
									busids = as[0];
									bus_type = as[1];
								}
								if(document.getElementById('ImportPayment.processstate').value == '出纳付押汇到期款'||document.getElementById('ImportPayment.processstate').value == '资金部出纳付押汇到期款'){
						...
						}else if((paytype == '1' || paytype == '3') && (document.getElementById('ImportPayment.processstate').value == '资金部出纳付款'||document.getElementById('ImportPayment.processstate').value == '出纳付款')){
