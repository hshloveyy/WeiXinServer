/**
  ...
  * Date: 2013年11月19日 10点15分32秒
  * Copyright Notice:版权声明 福建讯盟软件有限公司, 版权所有 违者必究
  * @(#)
  * Description:  
  * <功能>主对象国内信用证(HomeCreditPayment)编辑页面用户可编程扩展JS文件
 */
...
/**
  ...
  * @(#)
  * Description:  
  * <功能>主对象付款(HomeCreditPayment)编辑页面用户可编程扩展JS文件
 */

var ps = Ext.getDom('HomeCreditPayment.processstate').value;
/**
 * 记录当前流程状态所属界面
 * 0：申请界面  1：财务界面
 * 2：资金界面  3：其他界面
...
//	calendar_HomeCreditPayment_documentarypaydt.setEditable(false);		// 押汇到期付款日（日期型）
	calendar_HomeCreditPayment_payrealdate.setEditable(false);			// 实际付款日期(押汇)（日期型）
	...
	if(dict_div_pay_type_dict.getValue()=='2'){
		if(ps=='填写办理押汇信息'){
//			calendar_HomeCreditPayment_documentarydate.setEditable(true);		// 押汇日（日期型）
			Ext.getDom('HomeCreditPayment.doctaryinterest').readOnly = false;	// 押汇利率
			Ext.getDom('HomeCreditPayment.documentaryrate').readOnly = false;	// 押汇汇率
//			calendar_HomeCreditPayment_documentarypaydt.setEditable(true);		// 押汇到期付款日（日期型）
		}else if(ps.indexOf('出纳付押汇到期款')>=0){
			calendar_HomeCreditPayment_payrealdate.setEditable(true);			// 实际付款日期(押汇)（日期型）
			Ext.getDom('HomeCreditPayment.redocaryamount').readOnly = false;	// 还押汇金额
			Ext.getDom('HomeCreditPayment.redocaryrate').readOnly = false;		// 还押汇汇率
			// 在该结点时自动将"申请金额"自动带到"还押汇金额"中
			//Ext.getDom('HomeCreditPayment.redocaryamount').value = Ext.getDom('HomeCreditPayment.applyamount').value;
		}
		if (ps == '财务会计审核办理押汇并做帐') {
			Ext.getDom('HomeCreditPayment.documentaryrate').readOnly = false;	// 押汇汇率
			Ext.getDom('HomeCreditPayment.redocaryrate').readOnly = false;		// 还押汇汇率
	...
	function calcPayAmount(e){
		...
		var redocaryrate = Ext.getDom('HomeCreditPayment.redocaryrate').value;				// 还押汇汇率
	 ...
	 * 押汇银行页签
	 */
	HomeCreditDocuBank_grid.on('afteredit', calcDocuPayAmount, HomeCreditDocuBank_grid);
	function calcDocuPayAmount(e){
		var cRecord = e.record;
		var cField  = e.field;
		var docuRate = Ext.getDom('HomeCreditPayment.documentaryrate').value;				// 押汇汇率
		var reDocuRate = Ext.getDom('HomeCreditPayment.redocaryrate').value;				// 还押汇汇率
	 ...
	 * 还押汇银行页签
	 */
	HomeCreditRebank_grid.on('afteredit', calcBillPayReBankAmount, HomeCreditRebank_grid);
	...
	*/
	/**
	HomeCreditPayCbill_grid.on('afteredit', calcClearAmount, HomeCreditPayCbill_grid);
	...
	function calcClearAmount(e){
		...
		for (var i = 0; i < HomeCreditPayCbill_grid.getStore().getCount(); i++)
		{
			var rec = HomeCreditPayCbill_grid.getStore().getAt(i);
			sumclearamount += parseFloat(rec.get('clearamount2'));			
		}
		HomeCreditPayCbill_grid.setTitle('清账金额合计：'+ round(sumclearamount,2));
	}
	HomeCreditPayCbill_grid.getStore().on('load', calcClearAmount2, HomeCreditPayCbill_grid);
	...
	function calcClearAmount2(e,recs,o){
		...
		HomeCreditPayCbill_grid.setTitle('清账金额合计：'+ round(sumclearamount2,2));
	}
	**/
	/*
	 * "押汇汇率"变更时触发"押汇银行页签"下的付款金额本位币的计算（若有"还押汇汇率"存在，则不触发）
	 */
	Ext.getDom('HomeCreditPayment.documentaryrate').onblur = function(){
		_calcDocuPayAmount();
	}
	/*
	 * "还押汇汇率"变更时触发"押汇银行页签"下的付款金额本位币的计算
	 */
	Ext.getDom('HomeCreditPayment.redocaryrate').onblur = function(){
		_calcDocuPayAmount();
	}
	/*
	...
	function _calcDocuPayAmount(){
		...
		if (ps=='财务会计审核押汇到期付款并做帐') {
			...
			for(var i=0;i<HomeCreditDocuBank_store.getCount();i++){
				var docPayAmount = HomeCreditDocuBank_store.getAt(i).get('docuarypayamount');	// 押汇付款金额
				HomeCreditDocuBank_store.getAt(i).set('docuarypayamoun2', round((docPayAmount * docuRate),2));
			}
		} else {
			for(var i=0;i<HomeCreditDocuBank_store.getCount();i++){
				var docPayAmount = HomeCreditDocuBank_store.getAt(i).get('docuarypayamount');	// 押汇付款金额
				HomeCreditDocuBank_store.getAt(i).set('docuarypayamoun2', round((docPayAmount * rate),2));
			}
		}
	}
	/*
	...
	var HomeCreditPayment_redocaryamount = document.getElementById("HomeCreditPayment.redocaryamount");
	
	HomeCreditPayment_redocaryamount.onblur = function redocaryamount(){    
		document.getElementById('HomeCreditPayment.factamount').value = document.getElementById('HomeCreditPayment.redocaryamount').value;    
	}
...
function mainInit(){
	...
	if(pt.value == '14'){
		Ext.getCmp("tabs").hideTabStripItem('homeCreditPayCbillTab');
	...
	if (strRoleType == "1"){
		...
		var HomeCreditDocuBank_grid_cm = HomeCreditDocuBank_grid.getColumnModel();
		HomeCreditDocuBank_grid_cm.setHidden(HomeCreditDocuBank_grid_cm.findColumnIndex('cashflowitem'),true);
		HomeCreditDocuBank_grid_cm.setHidden(HomeCreditDocuBank_grid_cm.findColumnIndex('docuarypayamoun2'),true);
	...
	/***********************************************************************/
		...
		document.getElementById('HomeCreditPayment.applyamount').readOnly=true;
		if(ps.trim()=='' || ps=='LC/DP/DA承兑付款申请' || ps=='修改'){
			Ext.getCmp('_bookofaccount').hide();			// 隐藏（按钮）现金日记帐
			Ext.getCmp('_simulate').hide();					// 隐藏（按钮）模拟凭证		
			tb.hideTabStripItem('homeCreditBankItemTab');	// 隐藏（页签）付款银行	
			tb.hideTabStripItem('homeCreditDocuBankTab');	// 隐藏（页签）押汇/海外代付银行	
			tb.hideTabStripItem('homeCreditRebankTab');
			tb.hideTabStripItem('settleSubjectTab');		// 隐藏（页签）付款结算科目	
			tb.hideTabStripItem('fundFlowTab');		// 隐藏（页签）付款纯资金	
			tb.hideTabStripItem('homeCreditRelatPayTab');	// 隐藏（页签）相关单据	
			document.getElementById('HomeCreditPayment.applyamount').readOnly=false;
		}else if(ps=='财务会计审核付款并做帐' || 
				ps=='财务会计审核办理押汇并做帐' 
				||  ps=='财务会计审核押汇到期付款并做帐'
				){
			stateFlag = 1;
			...
			tb.hideTabStripItem('homeCreditRelatPayTab');	// 隐藏（页签）相关单据
			// 若为海外代付
			if(Ext.getDom('HomeCreditPayment.isoverrepay').value=='1' && ps!='财务会计审核押汇到期付款并做帐'){
			...
			Ext.getDom('workflowLeaveTransitionName').onchange = function(){
				if(Ext.getDom('HomeCreditPayment.isoverrepay').value!='1'){
...
//			calendar_HomeCreditPayment_payrealdate.setEditable(true);			// 实际付款日期(押汇)（日期型）
		...
		if(ps.indexOf('出纳付押汇到期款')>=0){
			...
			document.getElementById('HomeCreditPayment.redocaryamount').readOnly="readOnly";
		}
...
function _predeletesHomeCreditBankItem(){
	return true;
}

function _postdeletesHomeCreditBankItem(){
...
function _checkBillPayReBankInfo(){
	...
	var applyamount = document.getElementById('HomeCreditPayment.redocaryamount').value;
	for (var k = 0;k<HomeCreditRebank_grid.getStore().getCount();k++){
		if (HomeCreditRebank_grid.getStore().getAt(k).get('realmoney') == '' || HomeCreditRebank_grid.getStore().getAt(k).get('realmoney') == '0'){
			isAssignBankValue = true;
			showText = '还押汇银行为的实际还款金额为空或为零!';
			break;
		}else{
			assignBankValue2 = parseFloat(assignBankValue2) + parseFloat(HomeCreditRebank_grid.getStore().getAt(k).get('applyamount'));
		}
		if(HomeCreditRebank_grid.getStore().getAt(k).get('businesstype') == '未做账'){
			if(HomeCreditRebank_grid.getStore().getAt(k).get('applyamount') != '0'){
				i++;
				if(HomeCreditRebank_grid.getStore().getAt(k).get('applyamount') != HomeCreditRebank_grid.getStore().getAt(k).get('realmoney')){
					_getMainFrame().showInfo("还押汇银行的还押汇金额不等于还押汇银行的实际付款金额！");
					return false;
				}
			}
			assignBankValue3 = parseFloat(assignBankValue3) + parseFloat(HomeCreditRebank_grid.getStore().getAt(k).get('applyamount'));
		}
	}
	if(i>1){
...
function _dealPrepayamount(){
	...
	for (var j = 0;j<HomeCreditPayItem_store.getCount();j++){
		...
		//record.set('assignamount2',parseFloat(ItemAssignValue) * parseFloat(document.getElementById('HomeCreditPayment.closeexchangerat').value));
		//record.set('goodsamount',parseFloat(ItemAssignValue));
	}
	
	//处理结算科目和纯资金往来
	if (Utils.isEmpty(document.getElementById('SettleSubject.amount1').value) == false &&
		dict_div_SettleSubjectdebtcredit1_dict.getValue() != ''){
		if (dict_div_SettleSubjectdebtcredit1_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('SettleSubject.amount1').value);
		}
		
		if (dict_div_SettleSubjectdebtcredit1_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('SettleSubject.amount1').value);
		}
	}
	
	if (Utils.isEmpty(document.getElementById('SettleSubject.amount2').value) == false &&
		dict_div_SettleSubjectdebtcredit2_dict.getValue() != ''){
		if (dict_div_SettleSubjectdebtcredit2_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('SettleSubject.amount2').value);
		}
		
		if (dict_div_SettleSubjectdebtcredit2_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('SettleSubject.amount2').value);
		}
	}
	
	if (Utils.isEmpty(document.getElementById('SettleSubject.amount3').value) == false &&
		dict_div_SettleSubjectdebtcredit3_dict.getValue() != ''){
		if (dict_div_SettleSubjectdebtcredit3_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('SettleSubject.amount3').value);
		}
		
		if (dict_div_SettleSubjectdebtcredit3_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('SettleSubject.amount3').value);
		}
	}
	
	if (Utils.isEmpty(document.getElementById('SettleSubject.amount4').value) == false &&
		dict_div_SettleSubjectdebtcredit4_dict.getValue() != ''){
		if (dict_div_SettleSubjectdebtcredit4_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('SettleSubject.amount4').value);
		}
		
		if (dict_div_SettleSubjectdebtcredit4_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('SettleSubject.amount4').value);
		}
	}

	if (Utils.isEmpty(document.getElementById('FundFlow.amount1').value) == false &&
		dict_div_FundFlowdebtcredit1_dict.getValue() != ''){
		if (dict_div_FundFlowdebtcredit1_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('FundFlow.amount1').value);
		}
		
		if (dict_div_FundFlowdebtcredit1_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('FundFlow.amount1').value);
		}
	}
	
	if (Utils.isEmpty(document.getElementById('FundFlow.amount2').value) == false &&
		dict_div_FundFlowdebtcredit2_dict.getValue() != ''){
		if (dict_div_FundFlowdebtcredit2_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('FundFlow.amount2').value);
		}
		
		if (dict_div_FundFlowdebtcredit2_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('FundFlow.amount2').value);
		}
	}
	
	if (Utils.isEmpty(document.getElementById('FundFlow.amount3').value) == false &&
		dict_div_FundFlowdebtcredit3_dict.getValue() != ''){
		if (dict_div_FundFlowdebtcredit3_dict.getValue() == 'S'){
			allItemAssignValue = allItemAssignValue + parseFloat(document.getElementById('FundFlow.amount3').value);
		}
		
		if (dict_div_FundFlowdebtcredit3_dict.getValue() == 'H'){
			allItemAssignValue = allItemAssignValue - parseFloat(document.getElementById('FundFlow.amount3').value3);
		}
	}
	//处理结算科目和纯资金往来
...
function _checkCredit(){
	...
	var paymentid = document.getElementById('HomeCreditPayment.paymentid').value;			
...
function _checkDocumentaryCredit(){
	...
	var dLimit = Ext.getDom('HomeCreditPayment.documentarylimit').value;	// 押汇期限
	var dDate = calendar_HomeCreditPayment_documentarydate.getValue();		// 押汇日（日期型）
	var dInterest = Ext.getDom('HomeCreditPayment.doctaryinterest').value;	// 押汇利率
	var dRate = Ext.getDom('HomeCreditPayment.documentaryrate').value;		// 押汇汇率
	var dPayDate = calendar_HomeCreditPayment_documentarypaydt.getValue();	// 押汇到期付款日（日期型）
	var reDocAmount = Ext.getDom('HomeCreditPayment.redocaryamount').value;	// 获取还押汇金额
	var reDocRate = Ext.getDom('HomeCreditPayment.redocaryrate').value;		// 获取还押汇汇率
	var dRealDate = calendar_HomeCreditPayment_payrealdate.getValue();		// 实际付款日期(押汇)（日期型）
	if(ps=='填写办理押汇信息'){
		if(dCurrnency==''){
		...
		if(dRealDate==''){
			...
			return false;
		}else if(Utils.isEmpty(reDocAmount)){
 ...
 * 取消
 */
function _precancelHomeCreditPayment(){
	return true ;
}
 ...
 * 取消
 */
function _postcancelHomeCreditPayment(){}
 ...
 */
/***********************************************************************/
function _checkHomeCreditBankItem(){
	// 判断以下资金节点下，"付款银行"是否为空
	if((ps=='出纳确认办理押汇' || 
		ps=='资金部出纳付款' ||
		ps=='出纳付款') && (HomeCreditBankItem_store.getCount()<=0)){
		_getMainFrame().showInfo("付款银行不能为空！");
		return false;
	}
	if((ps.indexOf('出纳付押汇到期款')>=0 	) && (HomeCreditRebank_store.getCount()<=0)){
		_getMainFrame().showInfo("押汇还押汇银行不能为空！");
		return false;
	}
	// 判断以下资金节点下，"押汇银行"是否为空
	if((ps=='出纳确认办理押汇') && (HomeCreditDocuBank_store.getCount()<=0)){
...
function _setPayer(){
	...
	if((ps=='资金部出纳付款' && ns=='确认') || (ps=='资金部出纳付押汇到期款' && ns=='提交机要室')||
	   ((ps=='出纳付款'||ps=='出纳付押汇到期款') && ns=='提交财务经理确认')){
		Ext.getDom('HomeCreditPayment.payer').value = ca;
	}
}
/***********************************************************************/
...
function _presubmitProcessHomeCreditPayment(){
	...
	//申请金额以实际付款金额不一致
	if(document.getElementById('HomeCreditPayment.applyamount').value!=document.getElementById('HomeCreditPayment.factamount').value){
	        if(!confirm("申请付款金额已实际付款金额不一致，是否确认提交！")){
				return false;
			}
	}
	if(!_checkHomeCreditBankItem()){
		return false;
	}
	/*
	 * 检查押汇信息
	 ...
	 * 检查实际付款币别
	 */
	var factcurrency = Ext.getDom('HomeCreditPayment.factcurrency').value;
	var ps = document.getElementById('HomeCreditPayment.processstate').value;
	if( (ps== '资金部出纳付款'||ps=='出纳付款') && factcurrency == ''){
		_getMainFrame().showInfo("实际付款币别不能为空！");
		return false;
	}
	//出纳付款必须填写实际付款日
	var payrealdate = Ext.getDom('HomeCreditPayment.payrealdate').value;
...
function winPickCallBack(jsonArrayData){
	...
	for(var i=0;i<jsonArrayData.length;i++){
		...
		document.getElementById('HomeCreditPayment.paydate').value = jsonArrayData[0].PAY_DATE;
	...
	plick_list_no = HomeCreditPayItem_grid.getStore().getAt(0).get('pick_list_no_text');

	//从到单表T_PICK_LIST_INFO中将到单的部分信息带出来：分配金额、申请金额（一般是到单金额 ）；币别；预计到货日；（即期/承兑到期）付款日；核销单号；实际付款金额；实际币别
	if(plick_list_no != ''){
		var conn = Ext.lib.Ajax.getConnectionObject().conn; 
		conn.open("POST", contextPath + '/xdss3/payment/homeCreditPaymentController.spr?action=getPlickListInfoByno&plicklistno=' + plick_list_no,false);
		conn.send(null);
...
function _submitSimulate(){
	...
	var Url = '';
	var billRecordCount = HomeCreditPayCbill_store.getCount();	
		
			//判断如果是押汇
			if (dict_div_pay_type_dict.getValue() == '2'){
				if (document.getElementById('HomeCreditPayment.processstate').value == '财务会计审核办理押汇并做帐'){
					Url = contextPath+'/xdss3/payment/homeCreditPaymentController.spr?action=_simulate&simulatetype=8';
				}
				
				if (document.getElementById('HomeCreditPayment.processstate').value == '财务会计审核押汇到期付款并做帐'){
					Url = contextPath+'/xdss3/payment/homeCreditPaymentController.spr?action=_simulate&simulatetype=9';
				}
			}else{
				//判断是不是一般付款和兑
				if (dict_div_pay_type_dict.getValue() == '1' || dict_div_pay_type_dict.getValue() == '3'){
					//国内信用证
					if (dict_div_paymenttype_dict.getValue() == '06'){
						if (document.getElementById('HomeCreditPayment.processstate').value == '财务会计审核付款并做帐'){
							Url = contextPath+'/xdss3/payment/homeCreditPaymentController.spr?action=_simulate&simulatetype=2';
						}
						
						if (document.getElementById('HomeCreditPayment.processstate').value == '财务审核银承或国内信用证到期付款并做帐'){
							Url = contextPath+'/xdss3/payment/homeCreditPaymentController.spr?action=_simulate&simulatetype=3';
						}
					}
				}
			}
...
//	if(isReassign=='Y'){
//		Url = contextPath+'/xdss3/payment/homeCreditPaymentController.spr?action=_simulate&simulatetype=1';
//	}
...
function _simulateHomeCreditPayment(){
	...
	if (document.getElementById('HomeCreditPayment.processstate').value == '资金部出纳付押汇到期款' ||
	    document.getElementById('HomeCreditPayment.processstate').value == '出纳付押汇到期款' ||
		document.getElementById('HomeCreditPayment.processstate').value == '资金部人员填写押汇信息' ||
		document.getElementById('HomeCreditPayment.processstate').value == '资金部出纳付承兑到期款' ||
		document.getElementById('HomeCreditPayment.processstate').value == '资金部出纳付款'||
		document.getElementById('HomeCreditPayment.processstate').value == '出纳付款'){
		if (HomeCreditBankItem_store.getCount() <=0){
	...
	if (document.getElementById('HomeCreditPayment.processstate').value == '资金部人员办理押汇'){
		if (HomeCreditDocuBank_store.getCount() <=0){
	...
	if (document.getElementById('HomeCreditPayment.voucherdate').value == ''){
		_getMainFrame().showInfo("模拟凭证之前请输入凭证日期!");
		return false;
	}
	
	if (document.getElementById('HomeCreditPayment.accountdate').value == ''){
		_getMainFrame().showInfo("模拟凭证之前请输入记账日期!");
		return false;
	}
	var accountdate = $('HomeCreditPayment.accountdate').value;
	for(var k = 0;k<HomeCreditPayCbill_store.getCount();k++){
		var cb_accountdate = HomeCreditPayCbill_store.getAt(k).get('accountdate');	
	...
	if (document.getElementById('HomeCreditPayment.closeexchangerat').value != document.getElementById('HomeCreditPayment.exchangerate').value){
		_getMainFrame().Ext.MessageBox.show({
			title:'系统提示',
...
function vouchercallBackHandle(transport){
	...
	if(transport.responseText){
		...
		HomeCreditPayItem_grid.getStore().reload();
		HomeCreditPayItem_grid.getStore().commitChanges();
	
	    HomeCreditBankItem_grid.getStore().reload();
	    HomeCreditBankItem_grid.getStore().commitChanges();
	
		HomeCreditDocuBank_grid.getStore().reload();
	    HomeCreditDocuBank_grid.getStore().commitChanges();
		
		HomeCreditRebank_grid.getStore().reload();
	    HomeCreditRebank_grid.getStore().commitChanges();
	    
		HomeCreditPayCbill_grid.getStore().commitChanges();
		HomeCreditPayCbill_grid.getStore().reload();
	
		HomeCreditRelatPay_grid.getStore().reload();
		HomeCreditRelatPay_grid.getStore().commitChanges();
		...
		_getMainFrame().maintabs.addPanel('凭证预览','', contextPath+'/xdss3/voucher/voucherController.spr?action=_manage&businessid='+Ext.get('HomeCreditPayment.paymentid').dom.value,closeVoucherCallBack,true);
...
function closeVoucherCallBack(flag)
{
	...
	if(flag){
		if(isReassign == 'Y'){
			_submitForReassignHomeCreditPayment(flag);
		}else{
			_submitProcessHomeCreditPayment(flag);
		}
	}
}
...
function _bookofaccountHomeCreditPayment(){
	...
	param = param +  "&"+ Form.serialize('fundFlowForm');		
	param = param + ""+ getHomeCreditRelatPayGridData();
    var url = contextPath+'/xdss3/payment/homeCreditPaymentController.spr?action=cashJournal';
...
function cashJournal(transport){
	HomeCreditBankItem_grid.getStore().reload({
		callback:function(){HomeCreditDocuBank_grid.getStore().reload({
				callback : function(){
					if(transport.responseText){
						var busids = '';
						var paytype = document.getElementById('HomeCreditPayment.pay_type').value;
						var isoverrepay = document.getElementById('HomeCreditPayment.isoverrepay').value;
						var ps = document.getElementById('HomeCreditPayment.processstate').value;
						var bus_type = '';
						if(paytype == '2'){ 
							if(isoverrepay == 0){
						...
						var journalType = '2';
						if($('HomeCreditPayment.processstate').value == '综合二部审核'){
							journalType = '1';
						}
...
function getPayBankAndDocuBankID(){
	var busids = '';
	var bus_type = '';
	for(var j = 0;j < HomeCreditBankItem_grid.getStore().getCount();j++){
		var paybankitemid = HomeCreditBankItem_grid.getStore().getAt(j).get('paybankitemid');
		busids += paybankitemid + ',';
		bus_type += '2' + ',';
	}
	for(var j = 0;j < HomeCreditDocuBank_grid.getStore().getCount();j++){
		var docuaryitemid = HomeCreditDocuBank_grid.getStore().getAt(j).get('docuaryitemid');
...
function getPayBankID(){
	...
	for(var j = 0;j < HomeCreditRebank_grid.getStore().getCount();j++){
		var bankitemid = HomeCreditRebank_grid.getStore().getAt(j).get('bankitemid');
		var businesstype = HomeCreditRebank_grid.getStore().getAt(j).get('businesstype');
		if(businesstype == "未做账"){
			busids += bankitemid + ',';
			bus_type += '2' + ',';
		}
 ...
 * 打印
 */
function  _printHomeCreditPayment(){
    window.open(contextPath +'/xdss3/payment/homeCreditPaymentController.spr?action=_print&paymentId='+document.getElementById('HomeCreditPayment.paymentid').value,'_blank','location=no,resizable=yes');
}
...
function _addReBankHomeCreditRebank()
{
	...
	if (HomeCreditDocuBank_grid.getStore().getCount() < 1) {
	...
	var applyamount = 0;
	/*
	for(var i=0;i<HomeCreditDocuBank_grid.getStore().getCount();i++){
		totalReadlMoney = parseFloat(totalReadlMoney) + parseFloat(HomeCreditDocuBank_grid.getStore().getAt(i).get('docuarypayamount'));
	}
	//累计还押汇金额
	redocaryamount2 = document.getElementById('HomeCreditPayment.redocaryamount2').value;	
	
	for(var i=0;i<HomeCreditRebank_grid.getStore().getCount();i++){
		applyamount = parseFloat(applyamount) + parseFloat(HomeCreditRebank_grid.getStore().getAt(i).get('applyamount'));
	}
	applyamount = round(totalReadlMoney - applyamount, 2);
	*/
	applyamount = document.getElementById('HomeCreditPayment.redocaryamount').value;
	if ( applyamount < 0){
		applyamount = 0;
	}
	/*
	var billpurrate = Ext.getDom("HomeCreditPayment.redocaryrate");
	var rebillpurrate = 0;
	if (HomeCreditRebank_grid.getStore().getCount() > 0 ) {
		rebillpurrate = HomeCreditRebank_grid.getStore().getAt(0).get('rebillpurrate');
	} else {
	...
	*/
	...
	//var supplieramount = HomeCreditDocuBank_grid.getStore().getAt(0).get('supplieramount');
	var tCurrency = Ext.getDom("HomeCreditPayment.currency2").value;
	var tCurrency_text = div_currency2_sh_sh.lastSelectionText;
	var bankacc = HomeCreditDocuBank_grid.getStore().getAt(0).get('docuarybankacco');
	var bankacc_text = HomeCreditDocuBank_grid.getStore().getAt(0).get('docuarybankname');
	var maturity = Ext.getDom("HomeCreditPayment.documentarypaydt");
	...
	HomeCreditRebank_grid.stopEditing();
	HomeCreditRebank_grid.getStore().insert(0, p);
	HomeCreditRebank_grid.startEditing(0, 0);
	
	HomeCreditRebank_grid.recalculation(); //重计算总计
}

function _deletesHomeCreditRebank()
{
		// 还押汇银行行项，存有ID时则不能修改
	if (HomeCreditRebank_grid.selModel.hasSelection() > 0) {
		var records = HomeCreditRebank_grid.selModel.getSelections();
		for (var i = 0; i < records.length; i++) {
		...
		_getMainFrame().Ext.MessageBox.show({
				...
				fn:function(buttonid){
					...
					var records = HomeCreditRebank_grid.selModel.getSelections();
					for (var i=0;i<records.length;i++){
						HomeCreditRebank_grid.getStore().remove(records[i]);
					}
				}
				}
			});
...
function _deletesHomeCreditPayCbill(){
		...
		if (HomeCreditPayCbill_grid.selModel.hasSelection() > 0){
			_getMainFrame().Ext.MessageBox.show({
				title:Txt.cue,
			    msg: '您选择了【国内证清票批量删除】操作，是否确定继续该操作？',
				buttons: {yes:Txt.ok, no:Txt.cancel},
				icon: Ext.MessageBox.QUESTION,
				fn:function(buttonid){
				    if (buttonid == 'yes'){
					var records = HomeCreditPayCbill_grid.selModel.getSelections();
					for (var i=0;i<records.length;i++){
						HomeCreditPayCbill_grid.getStore().remove(records[i]);
					}
				}
				}
			});
		}else{
			_getMainFrame().showInfo('您选择了【国内证清票批量删除】操作，是否确定继续该操作？');
		}
	
}
function _deletesHomeCreditBankItem(){
	
		if (HomeCreditBankItem_grid.selModel.hasSelection() > 0){
			_getMainFrame().Ext.MessageBox.show({
				title:Txt.cue,
			    msg: '您选择了【国内证付款银行批量删除】操作，是否确定继续该操作？',
				buttons: {yes:Txt.ok, no:Txt.cancel},
				icon: Ext.MessageBox.QUESTION,
				fn:function(buttonid){
				    if (buttonid == 'yes'){
					var records = HomeCreditBankItem_grid.selModel.getSelections();
					for (var i=0;i<records.length;i++){
						HomeCreditBankItem_grid.getStore().remove(records[i]);
					}
				}
				}
			});
		}else{
			_getMainFrame().showInfo('您选择了【国内证付款银行批量删除】操作，是否确定继续该操作？');
		}
