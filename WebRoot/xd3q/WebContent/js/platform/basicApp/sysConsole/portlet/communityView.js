...
function _editPortlet(id,url)
{
	...
	if(!id||id==''||id==' '||id=='undefined'){
		var records = Portlet_grid.getSelectionModel().getSelections();     
		var recordData = Ext.util.JSON.encode(records[0].data);
		url = url+"portlet="+ recordData;//'&subObject={"objectName":"OrderItems","operType":"modify","values":[{}';
	}
	else{
		url = url+"portletid="+id;
	}
	_getMainFrame().ExtModalWindowUtil.show("社区管理行项目编辑",	url,'','',{width:660,height:300});
}
