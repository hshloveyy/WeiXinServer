...
Ext.ux.UploadDialog.FileRecord = Ext.data.Record.create([
  {name: 'attachmentid'},  
  {name: 'filename',mapping:"attachmentname"},
  {name: 'state', type: 'int'},
  {name: 'note'},
  {name: 'input_element'}
]);
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  createGrid : function()
  {
    var store = new Ext.data.Store({
      proxy: new Ext.data.DWRProxy(AttachmentManager.getAttachList, true),
      //proxy: new Ext.data.MemoryProxy([]),
      reader: new Ext.data.ListRangeReader({totalProperty: 'totalSize',id: 'attachmentid'}, Ext.ux.UploadDialog.FileRecord),      
  ...
  fillToolbar : function()
  {
    ...
    */
    ...
    tb.x_buttons.download = tb.addButton({
      text: this.i18n.download_btn_text,
      tooltip: this.i18n.download_btn_tip,
      iconCls: 'ext-ux-uploaddialog-downloadbtn',
      handler: this.onDownloadButtonClick,
      scope: this
    });
  ...
  {
    var store = this.grid_panel.getStore();
    store.reload({params:this.base_params || this.baseParams || this.params});
  },
  ...
  prepareNextUploadTask : function()
  {
    ...
    store.each(function(r) {
      if (!record && r.get('state') == Ext.ux.UploadDialog.FileRecord.STATE_QUEUE) {
        ...
        if (r.get('input_element') != '') r.get('input_element').dom.disabled = true;
      }
    });
  ...
  downloadFiles : function(file_records)
  {
    var store = this.grid_panel.getStore();
    for (var i = 0, len = file_records.length; i < len; i++) {
      var r = file_records[i];
      
      if (r.get('attachmentid') != ''){
	    Ext.Ajax.request({
	      url : this.url+'?method=download',
	      params : {id:r.get('attachmentid')},
	      //method : 'POST',
	      form : this.form,
	      isUpload : true,
  ...
  removeFiles : function(file_records)
  {
    ...
    for (var i = 0, len = file_records.length; i < len; i++) {
      ...
      if (r.get('attachmentid') != ''){
        var attachmentid = r.get('attachmentid');
        Ext.Ajax.request({
          url : this.url+'?method=remove',
          params : {id:r.get('attachmentid')},
          //method : 'POST',
          form : this.form,
          isUpload : true,
          success : store.remove(r),
          //failure : this.onAjaxFailure,
          scope : this,
          record: r
        });
        ...
        store.remove(r);
        if (r.get('input_element') != '') r.get('input_element').remove();
      }
    }
  },
...
p.i18n = {
  ...
  download_btn_text: 'Dwonload',
  upload_btn_start_text: 'Upload',
  upload_btn_stop_text: 'Abort',
  download_btn_tip: 'Dwonload queued files to the client.',
