/**
  ...
  * @(#)
  * Description:  
  * <功能>主对象管理费用预算(DeptCharge)编辑页面JS文件
 */
 ...
 * 删除管理费用预算
 */
function _deleteDeptCharge()
{
if(_predeleteDeptCharge()){
	_getMainFrame().Ext.MessageBox.show({
		title:Txt.cue,
		msg: Txt.deptCharge_Delete_ConfirmOperation,
		buttons: {yes:Txt.ok, no:Txt.cancel},
		icon: Ext.MessageBox.QUESTION,
		fn:function(result){
			if (result == 'yes'){
				var param = '&deptchargeid='+deptchargeid;
				new AjaxEngine(contextPath + '/XDSS/sample/deptcharge/deptChargeController.spr?action=_delete', 
				{method:"post", parameters: param, onComplete: callBackHandle,callback:_deleteCallBackHandle});
			}
		 }
	});
  }
  _postdeleteDeptCharge();
}
 ...
 * 新增管理费用预算
 */
function _createDeptCharge()
{
	if(_precreateDeptCharge()){
		//增加页签
		_getMainFrame().maintabs.addPanel(Txt.deptCharge_Create,'',contextPath + '/XDSS/sample/deptcharge/deptChargeController.spr?action=_create');
     }
     _postcreateDeptCharge();
}
          
/**
 * 保存 
 */
function _saveOrUpdateDeptCharge()
{
if(_presaveOrUpdateDeptCharge()){
...
function customCallBackHandle(transport)
{
	...
	//document.getElementById("DeptCharge.deptchargeid").value = id;
 ...
 * 取消
 */
function _cancelDeptCharge()
{
  if(_precancelDeptCharge()){
	new AjaxEngine(contextPath + '/XDSS/sample/deptcharge/deptChargeController.spr?action=_cancel&deptchargeid='+deptchargeid, 
			   {method:"post", parameters: '', onComplete: callBackHandle,callback:_cancelDeptChargeCallBack});
   }
   _postcancelDeptCharge();
}
function _cancelDeptChargeCallBack()
{
	_getMainFrame().maintabs.remove(_getMainFrame().maintabs.getActiveTab());
}
          

function _copyCreateDeptCharge()
{
   if(_precopyCreateDeptCharge()){
		_getMainFrame().maintabs.addPanel(Txt.deptCharge_CopyCreate,'',contextPath + '/XDSS/sample/deptcharge/deptChargeController.spr?action=_copyCreate&deptchargeid='+deptchargeid);
   }
   _postcopyCreateDeptCharge();
}
