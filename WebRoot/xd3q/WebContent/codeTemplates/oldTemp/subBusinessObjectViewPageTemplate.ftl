<%-- 
  ...
  - @(#)
  - Description:  
  - <功能>子业务对象${beanAttribute.boDescription}(${beanAttribute.boName})查看页面
--%>
...
<title>${beanAttribute.boDescription}查看页面</title>
...
<#if column.formColumnNo==1 || column.uiType=="12">
	<tr>
		...
		<#elseif column.uiType=="03">
			<input type="password" class="inputText" id="${beanAttribute.boName}.${column.propertyName}" name="${beanAttribute.boName}.${column.propertyName}" value="${"$"}${"{"}${beanAttribute.boInstanceName}.${column.propertyName}${"}"}" <#if column.visibility!="X"> style="visibility: hidden;" <#else> <fisc:authentication sourceName="${beanAttribute.boName}.${column.propertyName}" taskId="${"$"}${"{"}workflowTaskId${"}"}"/></#if>>									
		<#-- 备注框 -->
		...
		<#else>
		    ...
		    <input type="text" class="inputText" id="${beanAttribute.boName}.${column.propertyName}" name="${beanAttribute.boName}.${column.propertyName}" value="${"$"}${"{"}${beanAttribute.boInstanceName}.${parentBo.boName?uncap_first}.${column.propertyName}${"}"}" <fisc:authentication sourceName="${beanAttribute.boName}.${column.propertyName}" taskId="${"$"}${"{"}workflowTaskId${"}"}"/>>
		    <#else>
			<input type="text" class="inputText" id="${beanAttribute.boName}.${column.propertyName}" name="${beanAttribute.boName}.${column.propertyName}" value="${"$"}${"{"}${beanAttribute.boInstanceName}.${column.propertyName}${"}"}" <fisc:authentication sourceName="${beanAttribute.boName}.${column.propertyName}" taskId="${"$"}${"{"}workflowTaskId${"}"}"/>>
			</#if>
		</#if>
		</td>
