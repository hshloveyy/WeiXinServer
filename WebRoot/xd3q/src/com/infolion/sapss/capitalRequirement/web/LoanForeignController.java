...
import com.infolion.sapss.capitalRequirement.service.LoanForeignHibernateService;
import com.infolion.sapss.capitalRequirement.service.LoanForeignJdbcService;

public class LoanForeignController extends BaseMultiActionController {
	@Autowired
	private LoanForeignHibernateService loanForeignHibernateService;
	@Autowired
	private LoanForeignJdbcService loanForeignJdbcService;

	public void setLoanForeignHibernateService(
			LoanForeignHibernateService loanForeignHibernateService) {
		this.loanForeignHibernateService = loanForeignHibernateService;
	}

	public void setLoanForeignJdbcService(
			LoanForeignJdbcService loanForeignJdbcService) {
		this.loanForeignJdbcService = loanForeignJdbcService;
	}
	...
	public ModelAndView toManager(HttpServletRequest request,
		...
		return new ModelAndView("sapss/capitalRequirement/loanForeignManager");
	}
	...
	public void find(HttpServletRequest req, HttpServletResponse resp)
		...
		sb
				.append("select tainfo.*,(case when (to_char(sysdate,'yyyy-mm-dd')> end_date and is_pay = 0) then 1 else 0  end) as IS_OVERDUE from T_ZJXQ_LOAN_Foreign tainfo where is_available='1' ");
		...
		String grid_columns = "LOAN_ID,COMPANY_CODE,AMOUNT_FOREIGN,CURRENCY,AMOUNT_CNY,INSTITUTION,BALANCE_FOREIGN,EX_RATE,BEGIN_DATE,END_DATE,IS_PAY,IS_OVERDUE,CREATOR_NAME,CREATOR_ID";
		String grid_size = "10";
	...
	public ModelAndView create(HttpServletRequest request,
		...
		UserContext userContext = UserContextHolder.getLocalUserContext()
				.getUserContext();
		TZJXQLoanForeignInfo info = new TZJXQLoanForeignInfo();
		...
		this.loanForeignHibernateService.saveLoanForeignInfo(info);
		request.setAttribute("main", info);
		request.setAttribute("from", "create");
		request.setAttribute("loanID", loanID);
		request.setAttribute("save_button_enable", true);
		return new ModelAndView("sapss/capitalRequirement/loanForeignCreate");
	}

	public void save(HttpServletRequest request, HttpServletResponse response,
			TZJXQLoanForeignInfo info) throws IllegalAccessException,
			InvocationTargetException, IOException {
		double db1 = new Double(info.getAmountForeign()).doubleValue();
		double db2 = new Double(info.getExRate()).doubleValue();
		db1 = db1 * db2;
		info.setAmountCNY(String.valueOf(db1));
		this.loanForeignHibernateService.updateLoanForeignInfo(info);
		this.loanForeignHibernateService.calBalance(info);
	...
	public ModelAndView modify(HttpServletRequest request,
		...
		TZJXQLoanForeignInfo info = this.loanForeignHibernateService
				.getLoanForeignInfo(loanID);
		request.setAttribute("main", info);
		if (from.equals("view")) {
	...
	public void delete(HttpServletRequest req, HttpServletResponse resp)
		...
		boolean bb = this.loanForeignHibernateService.deleteLoanInfo(loanID);
	...
	public void findRepay(HttpServletRequest request,
		...
		StringBuffer sb = new StringBuffer();
		sb
				.append("select tdinfo.* from T_ZJXQ_LOAN_Foreign_REPAY tdinfo where is_available='1' ");
		sb.append(" and loan_ID = '" + loanID + "'");
		sb.append(" order by create_time");
		String grid_sql = sb.toString();
		String grid_columns = "REPAY_ID,LOAN_ID,AMOUNT_FOREIGN,REPAY_DATE,EX_RATE,AMOUNT_CNY,REMARK";
		String grid_size = "10";
	...
	public void createRepay(HttpServletRequest request,
		...
		UserContext userContext = UserContextHolder.getLocalUserContext()
				.getUserContext();
		TZJXQLoanForeignRepayInfo repay = new TZJXQLoanForeignRepayInfo();
		String loanID = request.getParameter("loanID");
		...
		this.loanForeignHibernateService.saveLoanForeignRepay(repay);
	...
	public void saveRepay(HttpServletRequest request,
			HttpServletResponse response, TZJXQLoanForeignRepayInfo info)
		...
		response.setCharacterEncoding("GBK");
		response.setContentType("text/html; charset=GBK");
		this.loanForeignJdbcService.updateLoanForeignRepayInfo(repayID,
				colName, colValue);
	}
	...
	public void deleteRepay(HttpServletRequest req, HttpServletResponse response)
		...
		this.loanForeignHibernateService.deleteLoanForeignRepayInfo(idStr);
		JSONObject jsonObject = new JSONObject();
