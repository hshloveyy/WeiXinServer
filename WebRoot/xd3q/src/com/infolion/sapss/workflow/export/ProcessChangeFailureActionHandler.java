...
public class ProcessChangeFailureActionHandler implements ActionHandler
{
	public void execute(ExecutionContext context) throws Exception
	{
		...
		if ("1".equals(type))
		{
			exportProcessService.processBaleLoanFaile(businessRecordId,
					processId);
		}
		else if ("2".equals(type))
		{
			exportProcessService.processDiscountFaile(businessRecordId,
					processId);
		}
		else if ("3".equals(type))
		{
			exportProcessService.processNegotiationFaile(businessRecordId,
					processId);
		}
