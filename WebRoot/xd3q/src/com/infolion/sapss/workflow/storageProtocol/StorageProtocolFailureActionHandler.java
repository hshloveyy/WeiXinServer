/*
 * @(#)ScrappedProcessFailureActionHandler.java
...
public class StorageProtocolFailureActionHandler implements ActionHandler {
	...
	public void execute(ExecutionContext txt) throws Exception {
		...
		if(obj!=null){
			...
			ts.setInfoId(infoId);
			dao.updateState(ts, "4",true);
			long processId = txt.getProcessInstance().getId();
			WorkflowService.updateProcessInstanceEndState(processId, "仓储协议审批不通过");
		}
