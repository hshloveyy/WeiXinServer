...
import com.infolion.platform.workflow.engine.JBPMStoreFileService;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		byte[] bytes = (byte[]) jbpmTemplate.execute(new JbpmCallback()
		{
			...
			public Object doInJbpm(JbpmContext jbpmContext) throws JbpmException 
			{
				...
				try
				{
					String processimage = ReceiptShipConstants.ins().chooseProcessimage(processName);
					bytes = storeFileService.getBytes(processName, processimage);
