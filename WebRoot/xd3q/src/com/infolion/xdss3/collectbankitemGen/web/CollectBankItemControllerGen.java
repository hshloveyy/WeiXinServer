/*
 * @(#)CollectBankItemControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.collectbankitemGen.web;
 ...
 * @since 1.0
 */
public class CollectBankItemControllerGen extends AbstractGenController
{
	private final String boName = "CollectBankItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("collectBankItem",collectBankItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000323");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/collectbankitem/collectBankItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		CollectBankItem collectBankItem = new CollectBankItem ();
	    collectBankItem = (CollectBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectBankItem.class , false , request.getMethod(), false);
		request.setAttribute("collectBankItem", collectBankItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000323");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String colbankitemid = request.getParameter("colbankitemid");
		collectBankItemService._delete(colbankitemid,getBusinessObject());
	...
	{
		String collectBankItemIds = request.getParameter("collectBankItemIds");
		collectBankItemService._deletes(collectBankItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("collectBankItem", collectBankItem);  
		return new ModelAndView("xdss3/collectbankitem/collectBankItemView");
	}
	...
	{
		// 绑定主对象值
		CollectBankItem collectBankItem = (CollectBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectBankItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.collectBankItemService._saveOrUpdate(collectBankItem
,getBusinessObject());
}
this.collectBankItemService._submitProcess(collectBankItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/collectbankitem/collectBankItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		CollectBankItem collectBankItem  = new CollectBankItem();
		String colbankitemid = request.getParameter("colbankitemid");
		collectBankItem.setColbankitemid(colbankitemid);
		LockService.unLockBOData(collectBankItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		CollectBankItem collectBankItem = (CollectBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectBankItem.class , true , request.getMethod(), true);
this.collectBankItemService._saveOrUpdate(collectBankItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(collectBankItem.getColbankitemid());
		...
		{	
		jo.put("colbankitemid", collectBankItem.getColbankitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("collectBankItem", collectBankItem);
		return new ModelAndView("xdss3/collectbankitem/collectBankItemEdit");
	}
