/*
 * @(#)UnClearCustomBillController.java
 * 版权声明 福建讯盟软件有限公司, 版权所有 违者必究
 *
 *修订记录:
 *1)更改者：java业务平台代码生成工具
 *  时　间：2010年07月16日 11点02分06秒
 *  描　述：创建
 */
...
import com.infolion.xdss3.singleClearGen.web.UnClearCustomBillControllerGen;
...
public class UnClearCustomBillController extends UnClearCustomBillControllerGen
{    
    @Autowired
    ...
    {
        return new ModelAndView("xdss3/unclearquery/unClearCustomBillQuery");
    }
    ...
    public void queryGrid(HttpServletRequest request,
            ...
            throws Exception{
        BusinessObject businessObject = BusinessObjectService.getBusinessObject("UnClearCustomBill");
        String strAuthSql = "";
        try {
            ...
            // 替换权限默认的前缀表名
            strAuthSql = strAuthSql.replace("YUNCLEARCUSTBILL", "t");
        } catch (Exception ex) {
            throw new SQLException("类不存在：" + ex.getMessage());
        }
        
        StringBuffer sql = new StringBuffer("SELECT A.*  FROM YUNCLEARCUSTBILL A, YCUSTOMSICLEAR B ");
        sql.append("  WHERE TRIM(A.CONTRACT_NO) IS NULL   AND TRIM(A.PROJECT_NO) IS NULL AND A.CUSTOMSCLEARID=B.CUSTOMSCLEARID ");
        ...
        gridQueryCondition.setHandlerClass("com.infolion.xdss3.unclearQuery.domain.UnClearCustomBillQueryGrid");
