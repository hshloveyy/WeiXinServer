 ...
 *  描　述：创建
 */
package com.infolion.xdss3.rateGen.web;
 ...
 * @since 1.0
 */
public class RateControllerGen extends AbstractGenController
{
	private final String boName = "Rate";
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		Rate rate = this.rateService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		rateService._delete(rateid,getBusinessObject());
	...
	public void _deletes(HttpServletRequest request, HttpServletResponse response)
	{
		...
		rateService._deletes(rateIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
...
rate = this.rateService._get(id);        }
		...
		request.setAttribute("rate", rate);  
		return new ModelAndView("xdss3/rate/rateView");
	}
	...
	{
		// 绑定主对象值
		Rate rate = (Rate) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), Rate.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.rateService._saveOrUpdate(rate
,getBusinessObject());
}
this.rateService._submitProcess(rate		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/rate/rateManage");
	}
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		Rate rate = (Rate) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), Rate.class , true , request.getMethod(), true);
this.rateService._saveOrUpdate(rate,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(rate.getRateid());
		...
		{	
		jo.put("rateid", rate.getRateid());
String creator = rate.getCreator();
...
jo.put("createtime", rate.getCreatetime());
String lastmodifyer = rate.getLastmodifyer();
...
jo.put("lastmodifytime", rate.getLastmodifytime());		this.operateSuccessfullyWithString(response,jo.toString());
	   }
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
rate = this.rateService._getForEdit(id);        }
        else
        {
           rate = this.rateService._getForEdit(id);
        }
		...
		request.setAttribute("rate", rate);
		return new ModelAndView("xdss3/rate/rateEdit");
	}
