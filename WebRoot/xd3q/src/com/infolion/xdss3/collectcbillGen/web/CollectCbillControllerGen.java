/*
 * @(#)CollectCbillControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.collectcbillGen.web;
 ...
 * @since 1.0
 */
public class CollectCbillControllerGen extends AbstractGenController
{
	private final String boName = "CollectCbill";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("collectCbill",collectCbill);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000322");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/collectcbill/collectCbillAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		CollectCbill collectCbill = new CollectCbill ();
	    collectCbill = (CollectCbill) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectCbill.class , false , request.getMethod(), false);
		request.setAttribute("collectCbill", collectCbill);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000322");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String collectcbillid = request.getParameter("collectcbillid");
		collectCbillService._delete(collectcbillid,getBusinessObject());
	...
	{
		String collectCbillIds = request.getParameter("collectCbillIds");
		collectCbillService._deletes(collectCbillIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("collectCbill", collectCbill);  
		return new ModelAndView("xdss3/collectcbill/collectCbillView");
	}
	...
	{
		// 绑定主对象值
		CollectCbill collectCbill = (CollectCbill) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectCbill.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.collectCbillService._saveOrUpdate(collectCbill
,getBusinessObject());
}
this.collectCbillService._submitProcess(collectCbill		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/collectcbill/collectCbillManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		CollectCbill collectCbill  = new CollectCbill();
		String collectcbillid = request.getParameter("collectcbillid");
		collectCbill.setCollectcbillid(collectcbillid);
		LockService.unLockBOData(collectCbill);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		CollectCbill collectCbill = (CollectCbill) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectCbill.class , true , request.getMethod(), true);
this.collectCbillService._saveOrUpdate(collectCbill,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(collectCbill.getCollectcbillid());
		...
		{	
		jo.put("collectcbillid", collectCbill.getCollectcbillid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("collectCbill", collectCbill);
		return new ModelAndView("xdss3/collectcbill/collectCbillEdit");
	}
