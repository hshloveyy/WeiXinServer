/*
 * @(#)BillInPaymentControllerGen.java
 ...
 * @since 1.0
 */
public class BillInPaymentControllerGen extends AbstractGenController
{
	private final String boName = "BillInPayment";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billInPayment",billInPayment);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000301");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/billClear/billInPaymentAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		BillInPayment billInPayment = new BillInPayment ();
	    billInPayment = (BillInPayment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillInPayment.class , false , request.getMethod(), false);
		request.setAttribute("billInPayment", billInPayment);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000301");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String billnpaymentid = request.getParameter("billnpaymentid");
		billInPaymentService._delete(billnpaymentid,getBusinessObject());
	...
	{
		String billInPaymentIds = request.getParameter("billInPaymentIds");
		billInPaymentService._deletes(billInPaymentIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billInPayment", billInPayment);  
		return new ModelAndView("xdss3/billClear/billInPaymentView");
	}
	...
	{
		// 绑定主对象值
		BillInPayment billInPayment = (BillInPayment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillInPayment.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!StringUtils.isNullBlank(type) && type.equalsIgnoreCase("view"))
{
this.billInPaymentService._saveOrUpdate(billInPayment
,getBusinessObject());
}
this.billInPaymentService._submitProcess(billInPayment		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/billClear/billInPaymentManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		BillInPayment billInPayment  = new BillInPayment();
		String billnpaymentid = request.getParameter("billnpaymentid");
		billInPayment.setBillnpaymentid(billnpaymentid);
		LockService.unLockBOData(billInPayment);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		BillInPayment billInPayment = (BillInPayment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillInPayment.class , true , request.getMethod(), true);
this.billInPaymentService._saveOrUpdate(billInPayment,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(billInPayment.getBillnpaymentid());
		...
		{	
		jo.put("billnpaymentid", billInPayment.getBillnpaymentid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("billInPayment", billInPayment);
		return new ModelAndView("xdss3/billClear/billInPaymentEdit");
	}
