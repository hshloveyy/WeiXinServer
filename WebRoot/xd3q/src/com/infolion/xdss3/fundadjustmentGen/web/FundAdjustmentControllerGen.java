/*
 * @(#)FundAdjustmentControllerGen.java
 * 版权声明 福建讯盟软件有限公司, 版权所有 违者必究
 *
 *修订记录:
 *1)更改者：java业务平台代码生成工具
 *  时　间：2011年11月03日 14点45分25秒
 *  描　述：创建
 */
package com.infolion.xdss3.fundadjustmentGen.web;
 ...
 * @since 1.0
 */
public class FundAdjustmentControllerGen extends AbstractGenController
{
	private final String boName = "FundAdjustment";
	...
	@Autowired
	protected FundAdjustmentService fundAdjustmentService;
	
	public void setFundAdjustmentService(FundAdjustmentService fundAdjustmentService)
	{
		this.fundAdjustmentService = fundAdjustmentService;
	}
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
        ...
        fundAdjustment.setLocktime(DateUtils.getDateStr((new Date()), DateUtils.DB_STORE_DATE));
		request.setAttribute("fundAdjustment",fundAdjustment);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000358");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/fundadjustment/fundAdjustmentAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		FundAdjustment fundAdjustment = this.fundAdjustmentService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		fundAdjustmentService._delete(fundid,getBusinessObject());
	...
	{
		String fundAdjustmentIds = request.getParameter("fundAdjustmentIds");
		fundAdjustmentService._deletes(fundAdjustmentIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
fundAdjustment = this.fundAdjustmentService._get(id);        }
        else
        {
           fundAdjustment = this.fundAdjustmentService._get(id);
        }
		...
		request.setAttribute("fundAdjustment", fundAdjustment);  
		return new ModelAndView("xdss3/fundadjustment/fundAdjustmentView");
	}
	...
	{
		// 绑定主对象值
		FundAdjustment fundAdjustment = (FundAdjustment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), FundAdjustment.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.fundAdjustmentService._saveOrUpdate(fundAdjustment
,getBusinessObject());
}
this.fundAdjustmentService._submitProcess(fundAdjustment		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/fundadjustment/fundAdjustmentManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		FundAdjustment fundAdjustment  = new FundAdjustment();
		String fundid = request.getParameter("fundid");
		fundAdjustment.setFundid(fundid);
		LockService.unLockBOData(fundAdjustment);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		FundAdjustment fundAdjustment = (FundAdjustment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), FundAdjustment.class , true , request.getMethod(), true);
this.fundAdjustmentService._saveOrUpdate(fundAdjustment,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(fundAdjustment.getFundid());
		...
		{	
jo.put("fundno", fundAdjustment.getFundno());		jo.put("fundid", fundAdjustment.getFundid());
String creator = fundAdjustment.getCreator();
...
jo.put("createtime", fundAdjustment.getCreatetime());
String lastmodifyer = fundAdjustment.getLastmodifyer();
...
jo.put("lastmodifytime", fundAdjustment.getLastmodifytime());		this.operateSuccessfullyWithString(response,jo.toString());
	   }
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
fundAdjustment = this.fundAdjustmentService._getForEdit(id);        }
        else
        {
           fundAdjustment = this.fundAdjustmentService._getForEdit(id);
        }
		...
		request.setAttribute("fundAdjustment", fundAdjustment);
		return new ModelAndView("xdss3/fundadjustment/fundAdjustmentEdit");
	}
