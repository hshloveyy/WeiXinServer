/*
 * @(#)UnClearCollectControllerGen.java
 ...
 * @since 1.0
 */
public class UnClearCollectControllerGen extends AbstractGenController
{
	private final String boName = "UnClearCollect";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("unClearCollect",unClearCollect);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000331");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/singleClear/unClearCollectAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		UnClearCollect unClearCollect = new UnClearCollect ();
	    unClearCollect = (UnClearCollect) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), UnClearCollect.class , false , request.getMethod(), false);
		request.setAttribute("unClearCollect", unClearCollect);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000331");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		unClearCollectService._delete(unclearcollectid,getBusinessObject());
	...
	{
		String unClearCollectIds = request.getParameter("unClearCollectIds");
		unClearCollectService._deletes(unClearCollectIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("unClearCollect", unClearCollect);  
		return new ModelAndView("xdss3/singleClear/unClearCollectView");
	}
	...
	{
		// 绑定主对象值
		UnClearCollect unClearCollect = (UnClearCollect) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), UnClearCollect.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!StringUtils.isNullBlank(type) && type.equalsIgnoreCase("view"))
{
this.unClearCollectService._saveOrUpdate(unClearCollect
,getBusinessObject());
}
this.unClearCollectService._submitProcess(unClearCollect		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/singleClear/unClearCollectManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		UnClearCollect unClearCollect  = new UnClearCollect();
		String unclearcollectid = request.getParameter("unclearcollectid");
		unClearCollect.setUnclearcollectid(unclearcollectid);
		LockService.unLockBOData(unClearCollect);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		UnClearCollect unClearCollect = (UnClearCollect) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), UnClearCollect.class , true , request.getMethod(), true);
this.unClearCollectService._saveOrUpdate(unClearCollect,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(unClearCollect.getUnclearcollectid());
		...
		{	
		jo.put("unclearcollectid", unClearCollect.getUnclearcollectid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("unClearCollect", unClearCollect);
		return new ModelAndView("xdss3/singleClear/unClearCollectEdit");
	}
