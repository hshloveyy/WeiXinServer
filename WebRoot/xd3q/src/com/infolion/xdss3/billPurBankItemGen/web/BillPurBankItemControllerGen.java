/*
 * @(#)BillPurBankItemControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.billPurBankItemGen.web;
 ...
 * @since 1.0
 */
public class BillPurBankItemControllerGen extends AbstractGenController
{
	private final String boName = "BillPurBankItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billPurBankItem",billPurBankItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000348");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/billPurBankItem/billPurBankItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		BillPurBankItem billPurBankItem = new BillPurBankItem ();
	    billPurBankItem = (BillPurBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillPurBankItem.class , false , request.getMethod(), false);
		request.setAttribute("billPurBankItem", billPurBankItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000348");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		billPurBankItemService._delete(bankitemid,getBusinessObject());
	...
	{
		String billPurBankItemIds = request.getParameter("billPurBankItemIds");
		billPurBankItemService._deletes(billPurBankItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billPurBankItem", billPurBankItem);  
		return new ModelAndView("xdss3/billPurBankItem/billPurBankItemView");
	}
	...
	{
		// 绑定主对象值
		BillPurBankItem billPurBankItem = (BillPurBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillPurBankItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.billPurBankItemService._saveOrUpdate(billPurBankItem
,getBusinessObject());
}
this.billPurBankItemService._submitProcess(billPurBankItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/billPurBankItem/billPurBankItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		BillPurBankItem billPurBankItem  = new BillPurBankItem();
		String bankitemid = request.getParameter("bankitemid");
		billPurBankItem.setBankitemid(bankitemid);
		LockService.unLockBOData(billPurBankItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		BillPurBankItem billPurBankItem = (BillPurBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillPurBankItem.class , true , request.getMethod(), true);
this.billPurBankItemService._saveOrUpdate(billPurBankItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(billPurBankItem.getBankitemid());
		...
		{	
		jo.put("bankitemid", billPurBankItem.getBankitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("billPurBankItem", billPurBankItem);
		return new ModelAndView("xdss3/billPurBankItem/billPurBankItemEdit");
	}
