/*
 * @(#)BillPurBillItemControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.billPurBillItemGen.web;
 ...
 * @since 1.0
 */
public class BillPurBillItemControllerGen extends AbstractGenController
{
	private final String boName = "BillPurBillItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billPurBillItem",billPurBillItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000350");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/billPurBillItem/billPurBillItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		BillPurBillItem billPurBillItem = new BillPurBillItem ();
	    billPurBillItem = (BillPurBillItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillPurBillItem.class , false , request.getMethod(), false);
		request.setAttribute("billPurBillItem", billPurBillItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000350");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		billPurBillItemService._delete(billitemid,getBusinessObject());
	...
	{
		String billPurBillItemIds = request.getParameter("billPurBillItemIds");
		billPurBillItemService._deletes(billPurBillItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billPurBillItem", billPurBillItem);  
		return new ModelAndView("xdss3/billPurBillItem/billPurBillItemView");
	}
	...
	{
		// 绑定主对象值
		BillPurBillItem billPurBillItem = (BillPurBillItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillPurBillItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.billPurBillItemService._saveOrUpdate(billPurBillItem
,getBusinessObject());
}
this.billPurBillItemService._submitProcess(billPurBillItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/billPurBillItem/billPurBillItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		BillPurBillItem billPurBillItem  = new BillPurBillItem();
		String billitemid = request.getParameter("billitemid");
		billPurBillItem.setBillitemid(billitemid);
		LockService.unLockBOData(billPurBillItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		BillPurBillItem billPurBillItem = (BillPurBillItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillPurBillItem.class , true , request.getMethod(), true);
this.billPurBillItemService._saveOrUpdate(billPurBillItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(billPurBillItem.getBillitemid());
		...
		{	
		jo.put("billitemid", billPurBillItem.getBillitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("billPurBillItem", billPurBillItem);
		return new ModelAndView("xdss3/billPurBillItem/billPurBillItemEdit");
	}
