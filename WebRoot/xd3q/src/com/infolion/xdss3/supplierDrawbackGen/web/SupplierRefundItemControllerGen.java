/*
 * @(#)SupplierRefundItemControllerGen.java
 * 版权声明 福建讯盟软件有限公司, 版权所有 违者必究
 *
 *修订记录:
 *1)更改者：java业务平台代码生成工具
 *  时　间：2010年07月20日 09点59分20秒
 *  描　述：创建
 */
...
/**
 * <pre>
 * 供应商退款行项目(SupplierRefundItem)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class SupplierRefundItemControllerGen extends AbstractGenController
{
	private final String boName = "SupplierRefundItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("supplierRefundItem",supplierRefundItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000296");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/supplierDrawback/supplierRefundItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		SupplierRefundItem supplierRefundItem = new SupplierRefundItem ();
	    supplierRefundItem = (SupplierRefundItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SupplierRefundItem.class , false , request.getMethod(), false);
		request.setAttribute("supplierRefundItem", supplierRefundItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000296");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String refundmentitemid = request.getParameter("refundmentitemid");
		supplierRefundItemService._delete(refundmentitemid,getBusinessObject());
	...
	{
		String supplierRefundItemIds = request.getParameter("supplierRefundItemIds");
		supplierRefundItemService._deletes(supplierRefundItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("supplierRefundItem", supplierRefundItem);  
		return new ModelAndView("xdss3/supplierDrawback/supplierRefundItemView");
	}
	...
	{
		// 绑定主对象值
		SupplierRefundItem supplierRefundItem = (SupplierRefundItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SupplierRefundItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!StringUtils.isNullBlank(type) && type.equalsIgnoreCase("view"))
{
this.supplierRefundItemService._saveOrUpdate(supplierRefundItem
,getBusinessObject());
}
this.supplierRefundItemService._submitProcess(supplierRefundItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/supplierDrawback/supplierRefundItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		SupplierRefundItem supplierRefundItem  = new SupplierRefundItem();
		String refundmentitemid = request.getParameter("refundmentitemid");
		supplierRefundItem.setRefundmentitemid(refundmentitemid);
		LockService.unLockBOData(supplierRefundItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		SupplierRefundItem supplierRefundItem = (SupplierRefundItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SupplierRefundItem.class , true , request.getMethod(), true);
this.supplierRefundItemService._saveOrUpdate(supplierRefundItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(supplierRefundItem.getRefundmentitemid());
		...
		{	
		jo.put("refundmentitemid", supplierRefundItem.getRefundmentitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("supplierRefundItem", supplierRefundItem);
		return new ModelAndView("xdss3/supplierDrawback/supplierRefundItemEdit");
	}
