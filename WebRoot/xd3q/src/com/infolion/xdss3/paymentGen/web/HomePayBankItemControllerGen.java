/*
 * @(#)HomePayBankItemControllerGen.java
 ...
 * @since 1.0
 */
public class HomePayBankItemControllerGen extends AbstractGenController
{
	private final String boName = "HomePayBankItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homePayBankItem",homePayBankItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000318");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/homePayBankItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		HomePayBankItem homePayBankItem = new HomePayBankItem ();
	    homePayBankItem = (HomePayBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomePayBankItem.class , false , request.getMethod(), false);
		request.setAttribute("homePayBankItem", homePayBankItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000318");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		homePayBankItemService._delete(paybankitemid,getBusinessObject());
	...
	{
		String homePayBankItemIds = request.getParameter("homePayBankItemIds");
		homePayBankItemService._deletes(homePayBankItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homePayBankItem", homePayBankItem);  
		return new ModelAndView("xdss3/payment/homePayBankItemView");
	}
	...
	{
		// 绑定主对象值
		HomePayBankItem homePayBankItem = (HomePayBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomePayBankItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.homePayBankItemService._saveOrUpdate(homePayBankItem
,getBusinessObject());
}
this.homePayBankItemService._submitProcess(homePayBankItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/homePayBankItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		HomePayBankItem homePayBankItem  = new HomePayBankItem();
		String paybankitemid = request.getParameter("paybankitemid");
		homePayBankItem.setPaybankitemid(paybankitemid);
		LockService.unLockBOData(homePayBankItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		HomePayBankItem homePayBankItem = (HomePayBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomePayBankItem.class , true , request.getMethod(), true);
this.homePayBankItemService._saveOrUpdate(homePayBankItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(homePayBankItem.getPaybankitemid());
		...
		{	
		jo.put("paybankitemid", homePayBankItem.getPaybankitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("homePayBankItem", homePayBankItem);
		return new ModelAndView("xdss3/payment/homePayBankItemEdit");
	}
