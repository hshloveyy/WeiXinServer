/*
 * @(#)ImportPayBankItemControllerGen.java
 ...
 * @since 1.0
 */
public class ImportPayBankItemControllerGen extends AbstractGenController
{
	private final String boName = "ImportPayBankItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importPayBankItem",importPayBankItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000311");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/importPayBankItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		ImportPayBankItem importPayBankItem = new ImportPayBankItem ();
	    importPayBankItem = (ImportPayBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportPayBankItem.class , false , request.getMethod(), false);
		request.setAttribute("importPayBankItem", importPayBankItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000311");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		importPayBankItemService._delete(paybankitemid,getBusinessObject());
	...
	{
		String importPayBankItemIds = request.getParameter("importPayBankItemIds");
		importPayBankItemService._deletes(importPayBankItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importPayBankItem", importPayBankItem);  
		return new ModelAndView("xdss3/payment/importPayBankItemView");
	}
	...
	{
		// 绑定主对象值
		ImportPayBankItem importPayBankItem = (ImportPayBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportPayBankItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.importPayBankItemService._saveOrUpdate(importPayBankItem
,getBusinessObject());
}
this.importPayBankItemService._submitProcess(importPayBankItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/importPayBankItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ImportPayBankItem importPayBankItem  = new ImportPayBankItem();
		String paybankitemid = request.getParameter("paybankitemid");
		importPayBankItem.setPaybankitemid(paybankitemid);
		LockService.unLockBOData(importPayBankItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ImportPayBankItem importPayBankItem = (ImportPayBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportPayBankItem.class , true , request.getMethod(), true);
this.importPayBankItemService._saveOrUpdate(importPayBankItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(importPayBankItem.getPaybankitemid());
		...
		{	
		jo.put("paybankitemid", importPayBankItem.getPaybankitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("importPayBankItem", importPayBankItem);
		return new ModelAndView("xdss3/payment/importPayBankItemEdit");
	}
