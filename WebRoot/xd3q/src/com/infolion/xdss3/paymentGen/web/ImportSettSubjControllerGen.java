/*
 * @(#)ImportSettSubjControllerGen.java
 ...
 * @since 1.0
 */
public class ImportSettSubjControllerGen extends AbstractGenController
{
	private final String boName = "ImportSettSubj";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importSettSubj",importSettSubj);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000313");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/importSettSubjAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		ImportSettSubj importSettSubj = new ImportSettSubj ();
	    importSettSubj = (ImportSettSubj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportSettSubj.class , false , request.getMethod(), false);
		request.setAttribute("importSettSubj", importSettSubj);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000313");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		importSettSubjService._delete(settlesubjectid,getBusinessObject());
	...
	{
		String importSettSubjIds = request.getParameter("importSettSubjIds");
		importSettSubjService._deletes(importSettSubjIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importSettSubj", importSettSubj);  
		return new ModelAndView("xdss3/payment/importSettSubjView");
	}
	...
	{
		// 绑定主对象值
		ImportSettSubj importSettSubj = (ImportSettSubj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportSettSubj.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.importSettSubjService._saveOrUpdate(importSettSubj
,getBusinessObject());
}
this.importSettSubjService._submitProcess(importSettSubj		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/importSettSubjManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ImportSettSubj importSettSubj  = new ImportSettSubj();
		String settlesubjectid = request.getParameter("settlesubjectid");
		importSettSubj.setSettlesubjectid(settlesubjectid);
		LockService.unLockBOData(importSettSubj);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ImportSettSubj importSettSubj = (ImportSettSubj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportSettSubj.class , true , request.getMethod(), true);
this.importSettSubjService._saveOrUpdate(importSettSubj,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(importSettSubj.getSettlesubjectid());
		...
		{	
		jo.put("settlesubjectid", importSettSubj.getSettlesubjectid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("importSettSubj", importSettSubj);
		return new ModelAndView("xdss3/payment/importSettSubjEdit");
	}
