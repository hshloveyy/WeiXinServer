/*
 * @(#)HomeFundFlowControllerGen.java
 ...
 * @since 1.0
 */
public class HomeFundFlowControllerGen extends AbstractGenController
{
	private final String boName = "HomeFundFlow";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homeFundFlow",homeFundFlow);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000321");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/homeFundFlowAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		HomeFundFlow homeFundFlow = new HomeFundFlow ();
	    homeFundFlow = (HomeFundFlow) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeFundFlow.class , false , request.getMethod(), false);
		request.setAttribute("homeFundFlow", homeFundFlow);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000321");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		homeFundFlowService._delete(fundflowid,getBusinessObject());
	...
	{
		String homeFundFlowIds = request.getParameter("homeFundFlowIds");
		homeFundFlowService._deletes(homeFundFlowIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homeFundFlow", homeFundFlow);  
		return new ModelAndView("xdss3/payment/homeFundFlowView");
	}
	...
	{
		// 绑定主对象值
		HomeFundFlow homeFundFlow = (HomeFundFlow) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeFundFlow.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.homeFundFlowService._saveOrUpdate(homeFundFlow
,getBusinessObject());
}
this.homeFundFlowService._submitProcess(homeFundFlow		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/homeFundFlowManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		HomeFundFlow homeFundFlow  = new HomeFundFlow();
		String fundflowid = request.getParameter("fundflowid");
		homeFundFlow.setFundflowid(fundflowid);
		LockService.unLockBOData(homeFundFlow);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		HomeFundFlow homeFundFlow = (HomeFundFlow) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeFundFlow.class , true , request.getMethod(), true);
this.homeFundFlowService._saveOrUpdate(homeFundFlow,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(homeFundFlow.getFundflowid());
		...
		{	
		jo.put("fundflowid", homeFundFlow.getFundflowid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("homeFundFlow", homeFundFlow);
		return new ModelAndView("xdss3/payment/homeFundFlowEdit");
	}
