/*
 * @(#)ImportFundFlowControllerGen.java
 ...
 * @since 1.0
 */
public class ImportFundFlowControllerGen extends AbstractGenController
{
	private final String boName = "ImportFundFlow";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importFundFlow",importFundFlow);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000314");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/importFundFlowAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		ImportFundFlow importFundFlow = new ImportFundFlow ();
	    importFundFlow = (ImportFundFlow) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportFundFlow.class , false , request.getMethod(), false);
		request.setAttribute("importFundFlow", importFundFlow);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000314");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String fundflowid = request.getParameter("fundflowid");
		importFundFlowService._delete(fundflowid,getBusinessObject());
	...
	{
		String importFundFlowIds = request.getParameter("importFundFlowIds");
		importFundFlowService._deletes(importFundFlowIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importFundFlow", importFundFlow);  
		return new ModelAndView("xdss3/payment/importFundFlowView");
	}
	...
	{
		// 绑定主对象值
		ImportFundFlow importFundFlow = (ImportFundFlow) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportFundFlow.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.importFundFlowService._saveOrUpdate(importFundFlow
,getBusinessObject());
}
this.importFundFlowService._submitProcess(importFundFlow		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/importFundFlowManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ImportFundFlow importFundFlow  = new ImportFundFlow();
		String fundflowid = request.getParameter("fundflowid");
		importFundFlow.setFundflowid(fundflowid);
		LockService.unLockBOData(importFundFlow);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ImportFundFlow importFundFlow = (ImportFundFlow) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportFundFlow.class , true , request.getMethod(), true);
this.importFundFlowService._saveOrUpdate(importFundFlow,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(importFundFlow.getFundflowid());
		...
		{	
		jo.put("fundflowid", importFundFlow.getFundflowid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("importFundFlow", importFundFlow);
		return new ModelAndView("xdss3/payment/importFundFlowEdit");
	}
