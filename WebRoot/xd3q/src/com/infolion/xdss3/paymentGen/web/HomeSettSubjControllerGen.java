/*
 * @(#)HomeSettSubjControllerGen.java
 ...
 * @since 1.0
 */
public class HomeSettSubjControllerGen extends AbstractGenController
{
	private final String boName = "HomeSettSubj";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homeSettSubj",homeSettSubj);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000320");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/homeSettSubjAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		HomeSettSubj homeSettSubj = new HomeSettSubj ();
	    homeSettSubj = (HomeSettSubj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeSettSubj.class , false , request.getMethod(), false);
		request.setAttribute("homeSettSubj", homeSettSubj);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000320");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		homeSettSubjService._delete(settlesubjectid,getBusinessObject());
	...
	{
		String homeSettSubjIds = request.getParameter("homeSettSubjIds");
		homeSettSubjService._deletes(homeSettSubjIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homeSettSubj", homeSettSubj);  
		return new ModelAndView("xdss3/payment/homeSettSubjView");
	}
	...
	{
		// 绑定主对象值
		HomeSettSubj homeSettSubj = (HomeSettSubj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeSettSubj.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.homeSettSubjService._saveOrUpdate(homeSettSubj
,getBusinessObject());
}
this.homeSettSubjService._submitProcess(homeSettSubj		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/homeSettSubjManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		HomeSettSubj homeSettSubj  = new HomeSettSubj();
		String settlesubjectid = request.getParameter("settlesubjectid");
		homeSettSubj.setSettlesubjectid(settlesubjectid);
		LockService.unLockBOData(homeSettSubj);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		HomeSettSubj homeSettSubj = (HomeSettSubj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeSettSubj.class , true , request.getMethod(), true);
this.homeSettSubjService._saveOrUpdate(homeSettSubj,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(homeSettSubj.getSettlesubjectid());
		...
		{	
		jo.put("settlesubjectid", homeSettSubj.getSettlesubjectid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("homeSettSubj", homeSettSubj);
		return new ModelAndView("xdss3/payment/homeSettSubjEdit");
	}
