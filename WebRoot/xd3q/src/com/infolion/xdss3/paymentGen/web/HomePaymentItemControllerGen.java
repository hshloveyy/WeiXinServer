/*
 * @(#)HomePaymentItemControllerGen.java
 ...
 * @since 1.0
 */
public class HomePaymentItemControllerGen extends AbstractGenController
{
	private final String boName = "HomePaymentItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homePaymentItem",homePaymentItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000316");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/homePaymentItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		HomePaymentItem homePaymentItem = new HomePaymentItem ();
	    homePaymentItem = (HomePaymentItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomePaymentItem.class , false , request.getMethod(), false);
		request.setAttribute("homePaymentItem", homePaymentItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000316");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		homePaymentItemService._delete(paymentitemid,getBusinessObject());
	...
	{
		String homePaymentItemIds = request.getParameter("homePaymentItemIds");
		homePaymentItemService._deletes(homePaymentItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homePaymentItem", homePaymentItem);  
		return new ModelAndView("xdss3/payment/homePaymentItemView");
	}
	...
	{
		// 绑定主对象值
		HomePaymentItem homePaymentItem = (HomePaymentItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomePaymentItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.homePaymentItemService._saveOrUpdate(homePaymentItem
,getBusinessObject());
}
this.homePaymentItemService._submitProcess(homePaymentItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/homePaymentItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		HomePaymentItem homePaymentItem  = new HomePaymentItem();
		String paymentitemid = request.getParameter("paymentitemid");
		homePaymentItem.setPaymentitemid(paymentitemid);
		LockService.unLockBOData(homePaymentItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		HomePaymentItem homePaymentItem = (HomePaymentItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomePaymentItem.class , true , request.getMethod(), true);
this.homePaymentItemService._saveOrUpdate(homePaymentItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(homePaymentItem.getPaymentitemid());
		...
		{	
		jo.put("paymentitemid", homePaymentItem.getPaymentitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("homePaymentItem", homePaymentItem);
		return new ModelAndView("xdss3/payment/homePaymentItemEdit");
	}
