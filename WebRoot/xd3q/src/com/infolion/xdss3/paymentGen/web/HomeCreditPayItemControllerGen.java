/*
 * @(#)HomeCreditPayItemControllerGen.java
...
/**
 * <pre>
 * 国内证付款行项目(HomeCreditPayItem)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class HomeCreditPayItemControllerGen extends AbstractGenController
{
	private final String boName = "HomeCreditPayItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homeCreditPayItem",homeCreditPayItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000371");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/homeCreditPayItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		String id = request.getParameter("paymentitemid");
		HomeCreditPayItem homeCreditPayItem = this.homeCreditPayItemService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		homeCreditPayItemService._delete(paymentitemid,getBusinessObject());
	...
	{
		String homeCreditPayItemIds = request.getParameter("homeCreditPayItemIds");
		homeCreditPayItemService._deletes(homeCreditPayItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
homeCreditPayItem = this.homeCreditPayItemService._get(id);        }
        else
        {
           homeCreditPayItem = this.homeCreditPayItemService._get(id);
        }
		...
		request.setAttribute("homeCreditPayItem", homeCreditPayItem);  
		return new ModelAndView("xdss3/payment/homeCreditPayItemView");
	}
	...
	{
		// 绑定主对象值
		HomeCreditPayItem homeCreditPayItem = (HomeCreditPayItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeCreditPayItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.homeCreditPayItemService._saveOrUpdate(homeCreditPayItem
,getBusinessObject());
}
this.homeCreditPayItemService._submitProcess(homeCreditPayItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/homeCreditPayItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		HomeCreditPayItem homeCreditPayItem  = new HomeCreditPayItem();
		String paymentitemid = request.getParameter("paymentitemid");
		homeCreditPayItem.setPaymentitemid(paymentitemid);
		LockService.unLockBOData(homeCreditPayItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		HomeCreditPayItem homeCreditPayItem = (HomeCreditPayItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeCreditPayItem.class , true , request.getMethod(), true);
this.homeCreditPayItemService._saveOrUpdate(homeCreditPayItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(homeCreditPayItem.getPaymentitemid());
		...
		{	
		jo.put("paymentitemid", homeCreditPayItem.getPaymentitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
homeCreditPayItem = this.homeCreditPayItemService._getForEdit(id);        }
        else
        {
           homeCreditPayItem = this.homeCreditPayItemService._getForEdit(id);
        }
		...
		request.setAttribute("homeCreditPayItem", homeCreditPayItem);
		return new ModelAndView("xdss3/payment/homeCreditPayItemEdit");
	}
