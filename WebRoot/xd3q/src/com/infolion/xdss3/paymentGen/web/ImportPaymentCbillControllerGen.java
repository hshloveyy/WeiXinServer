/*
 * @(#)ImportPaymentCbillControllerGen.java
 ...
 * @since 1.0
 */
public class ImportPaymentCbillControllerGen extends AbstractGenController
{
	private final String boName = "ImportPaymentCbill";
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
	    ...
	    importPaymentCbill = (ImportPaymentCbill) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportPaymentCbill.class , false, request.getMethod(), false);
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("importPaymentCbill", importPaymentCbill);
		return new ModelAndView("xdss3/payment/importPaymentCbillEdit");
	}
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("importPaymentCbill",importPaymentCbill);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000310");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/importPaymentCbillAdd");
	}
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		importPaymentCbillService._delete(paymentcbillid,getBusinessObject());
	...
	{
		String importPaymentCbillIds = request.getParameter("importPaymentCbillIds");
		importPaymentCbillService._deletes(importPaymentCbillIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importPaymentCbill", importPaymentCbill);  
		return new ModelAndView("xdss3/payment/importPaymentCbillView");
	}
	...
	{
		// 绑定主对象值
		ImportPaymentCbill importPaymentCbill = (ImportPaymentCbill) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportPaymentCbill.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.importPaymentCbillService._saveOrUpdate(importPaymentCbill
,getBusinessObject());
}
this.importPaymentCbillService._submitProcess(importPaymentCbill		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/importPaymentCbillManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ImportPaymentCbill importPaymentCbill  = new ImportPaymentCbill();
		String paymentcbillid = request.getParameter("paymentcbillid");
		importPaymentCbill.setPaymentcbillid(paymentcbillid);
		LockService.unLockBOData(importPaymentCbill);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ImportPaymentCbill importPaymentCbill = (ImportPaymentCbill) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportPaymentCbill.class , true , request.getMethod(), true);
this.importPaymentCbillService._saveOrUpdate(importPaymentCbill,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(importPaymentCbill.getPaymentcbillid());
		...
		{	
		jo.put("paymentcbillid", importPaymentCbill.getPaymentcbillid());
