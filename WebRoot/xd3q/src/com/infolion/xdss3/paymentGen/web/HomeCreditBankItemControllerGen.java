/*
 * @(#)HomeCreditBankItemControllerGen.java
 ...
 * @since 1.0
 */
public class HomeCreditBankItemControllerGen extends AbstractGenController
{
	private final String boName = "HomeCreditBankItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("homeCreditBankItem",homeCreditBankItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000373");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/homeCreditBankItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		String id = request.getParameter("paybankitemid");
		HomeCreditBankItem homeCreditBankItem = this.homeCreditBankItemService._getEntityCopy(id);
	...
	{
		String paybankitemid = request.getParameter("paybankitemid");
		homeCreditBankItemService._delete(paybankitemid,getBusinessObject());
	...
	{
		String homeCreditBankItemIds = request.getParameter("homeCreditBankItemIds");
		homeCreditBankItemService._deletes(homeCreditBankItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
homeCreditBankItem = this.homeCreditBankItemService._get(id);        }
        else
        {
           homeCreditBankItem = this.homeCreditBankItemService._get(id);
        }
		...
		request.setAttribute("homeCreditBankItem", homeCreditBankItem);  
		return new ModelAndView("xdss3/payment/homeCreditBankItemView");
	}
	...
	{
		// 绑定主对象值
		HomeCreditBankItem homeCreditBankItem = (HomeCreditBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeCreditBankItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.homeCreditBankItemService._saveOrUpdate(homeCreditBankItem
,getBusinessObject());
}
this.homeCreditBankItemService._submitProcess(homeCreditBankItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/homeCreditBankItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		HomeCreditBankItem homeCreditBankItem  = new HomeCreditBankItem();
		String paybankitemid = request.getParameter("paybankitemid");
		homeCreditBankItem.setPaybankitemid(paybankitemid);
		LockService.unLockBOData(homeCreditBankItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		HomeCreditBankItem homeCreditBankItem = (HomeCreditBankItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), HomeCreditBankItem.class , true , request.getMethod(), true);
this.homeCreditBankItemService._saveOrUpdate(homeCreditBankItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(homeCreditBankItem.getPaybankitemid());
		...
		{	
		jo.put("paybankitemid", homeCreditBankItem.getPaybankitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
homeCreditBankItem = this.homeCreditBankItemService._getForEdit(id);        }
        else
        {
           homeCreditBankItem = this.homeCreditBankItemService._getForEdit(id);
        }
		...
		request.setAttribute("homeCreditBankItem", homeCreditBankItem);
		return new ModelAndView("xdss3/payment/homeCreditBankItemEdit");
	}
