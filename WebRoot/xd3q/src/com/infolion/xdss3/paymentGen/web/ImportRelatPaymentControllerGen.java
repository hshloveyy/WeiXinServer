/*
 * @(#)ImportRelatPaymentControllerGen.java
 ...
 * @since 1.0
 */
public class ImportRelatPaymentControllerGen extends AbstractGenController
{
	private final String boName = "ImportRelatPayment";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importRelatPayment",importRelatPayment);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000312");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/payment/importRelatPaymentAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		ImportRelatPayment importRelatPayment = new ImportRelatPayment ();
	    importRelatPayment = (ImportRelatPayment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportRelatPayment.class , false , request.getMethod(), false);
		request.setAttribute("importRelatPayment", importRelatPayment);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000312");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		importRelatPaymentService._delete(relatedpaymentid,getBusinessObject());
	...
	{
		String importRelatPaymentIds = request.getParameter("importRelatPaymentIds");
		importRelatPaymentService._deletes(importRelatPaymentIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("importRelatPayment", importRelatPayment);  
		return new ModelAndView("xdss3/payment/importRelatPaymentView");
	}
	...
	{
		// 绑定主对象值
		ImportRelatPayment importRelatPayment = (ImportRelatPayment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportRelatPayment.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.importRelatPaymentService._saveOrUpdate(importRelatPayment
,getBusinessObject());
}
this.importRelatPaymentService._submitProcess(importRelatPayment		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/payment/importRelatPaymentManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ImportRelatPayment importRelatPayment  = new ImportRelatPayment();
		String relatedpaymentid = request.getParameter("relatedpaymentid");
		importRelatPayment.setRelatedpaymentid(relatedpaymentid);
		LockService.unLockBOData(importRelatPayment);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ImportRelatPayment importRelatPayment = (ImportRelatPayment) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ImportRelatPayment.class , true , request.getMethod(), true);
this.importRelatPaymentService._saveOrUpdate(importRelatPayment,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(importRelatPayment.getRelatedpaymentid());
		...
		{	
		jo.put("relatedpaymentid", importRelatPayment.getRelatedpaymentid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("importRelatPayment", importRelatPayment);
		return new ModelAndView("xdss3/payment/importRelatPaymentEdit");
	}
