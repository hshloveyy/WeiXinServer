/*
 * @(#)SupplyRemSumControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.supplyremainsumGen.web;
 ...
 * @since 1.0
 */
public class SupplyRemSumControllerGen extends AbstractGenController
{
	private final String boName = "SupplyRemSum";
	...
	@Autowired
	protected SupplyRemSumService supplyRemSumService;
	
	public void setSupplyRemSumService(SupplyRemSumService supplyRemSumService)
	{
		this.supplyRemSumService = supplyRemSumService;
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		String id = request.getParameter("srsid");
		...
		{	
supplyRemSum = this.supplyRemSumService._getForEdit(id);        }
        else
        {
           supplyRemSum = this.supplyRemSumService._getForEdit(id);
        }
       
		boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000338");
		...
		request.setAttribute("supplyRemSum", supplyRemSum);
		return new ModelAndView("xdss3/supplyremainsum/supplyRemSumEdit");
	}
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/supplyremainsum/supplyRemSumAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		String id = request.getParameter("srsid");
		SupplyRemSum supplyRemSum = this.supplyRemSumService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		supplyRemSumService._delete(srsid,getBusinessObject());
	...
	{
		String supplyRemSumIds = request.getParameter("supplyRemSumIds");
		supplyRemSumService._deletes(supplyRemSumIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{
supplyRemSum = this.supplyRemSumService._get(id);        }
        else
        {
           supplyRemSum = this.supplyRemSumService._get(id);
        }
		...
		request.setAttribute("supplyRemSum", supplyRemSum);  
		return new ModelAndView("xdss3/supplyremainsum/supplyRemSumView");
	}
	...
	{
		// 绑定主对象值
		SupplyRemSum supplyRemSum = (SupplyRemSum) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SupplyRemSum.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.supplyRemSumService._saveOrUpdate(supplyRemSum
,getBusinessObject());
}
this.supplyRemSumService._submitProcess(supplyRemSum		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		SupplyRemSum supplyRemSum  = new SupplyRemSum();
		String srsid = request.getParameter("srsid");
		supplyRemSum.setSrsid(srsid);
		LockService.unLockBOData(supplyRemSum);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		SupplyRemSum supplyRemSum = (SupplyRemSum) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SupplyRemSum.class , true , request.getMethod(), true);
this.supplyRemSumService._saveOrUpdate(supplyRemSum,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(supplyRemSum.getSrsid());
		...
		{	
		jo.put("srsid", supplyRemSum.getSrsid());
