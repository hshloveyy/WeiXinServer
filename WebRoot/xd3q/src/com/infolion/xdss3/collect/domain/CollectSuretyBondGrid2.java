...
public class CollectSuretyBondGrid2 extends GridTagHandler {
	...
	public void buildGrid() {
		...
		for (int i = 0; i < collectColumn.length; i++) {
			...
			if(collectColumnTitle[i].equals("单据类别")){
			    column.setVisibility(false);
			}else if(collectColumnTitle[i].equals("操作")){
				column.setRenderer("_manager");
			}else if(collectColumnTitle[i].equals("收/退款单号")){
			    column.setRenderer("_viewoldcollectinfo");
			}else if(collectColumnTitle[i].equals("申请收/退款金额")){
                column.setRenderer("_signJudge");
            }else if(collectColumnTitle[i].equals("折算金额/保证金转货款金额")){
                column.setRenderer("_signJudge");
