/*
 * @(#)AdvanceWarnCondControllerGen.java
 ...
 * @since 1.0
 */
public class AdvanceWarnCondControllerGen extends AbstractGenController
{
	private final String boName = "AdvanceWarnCond";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("advanceWarnCond",advanceWarnCond);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000245");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/advanceWarn/advanceWarnCondAdd");
	}
	...
	{
		AdvanceWarnCond advanceWarnCond = new AdvanceWarnCond ();
	    advanceWarnCond = (AdvanceWarnCond) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), AdvanceWarnCond.class , false , request.getMethod(), false);
		request.setAttribute("advanceWarnCond", advanceWarnCond);
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/advanceWarn/advanceWarnCondAdd");
	}
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		advanceWarnCondService._delete(conditionid,getBusinessObject());
	...
	{
		String advanceWarnCondIds = request.getParameter("advanceWarnCondIds");
		advanceWarnCondService._deletes(advanceWarnCondIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("advanceWarnCond", advanceWarnCond);  
		return new ModelAndView("xdss3/advanceWarn/advanceWarnCondView");
	}
	...
	{
		// 绑定主对象值
		AdvanceWarnCond advanceWarnCond = (AdvanceWarnCond) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), AdvanceWarnCond.class , true , request.getMethod(), true);
this.advanceWarnCondService._submitProcess(advanceWarnCond		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/advanceWarn/advanceWarnCondManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		AdvanceWarnCond advanceWarnCond  = new AdvanceWarnCond();
		String conditionid = request.getParameter("conditionid");
		advanceWarnCond.setConditionid(conditionid);
		LockService.unLockBOData(advanceWarnCond);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		AdvanceWarnCond advanceWarnCond = (AdvanceWarnCond) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), AdvanceWarnCond.class , true , request.getMethod(), true);
this.advanceWarnCondService._saveOrUpdate(advanceWarnCond,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(advanceWarnCond.getConditionid());
		...
		{	
		jo.put("conditionid", advanceWarnCond.getConditionid());
String creator = advanceWarnCond.getCreator();
...
jo.put("createtime", advanceWarnCond.getCreatetime());
String lastmodifyer = advanceWarnCond.getLastmodifyer();
...
jo.put("lastmodifytime", advanceWarnCond.getLastmodifytime());		this.operateSuccessfullyWithString(response,jo.toString());
	   }
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("advanceWarnCond", advanceWarnCond);
		return new ModelAndView("xdss3/advanceWarn/advanceWarnCondEdit");
	}
