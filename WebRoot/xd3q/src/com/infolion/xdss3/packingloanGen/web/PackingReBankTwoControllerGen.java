/*
 * @(#)PackingReBankTwoControllerGen.java
 ...
 * @since 1.0
 */
public class PackingReBankTwoControllerGen extends AbstractGenController
{
	private final String boName = "PackingReBankTwo";
	...
	@Autowired
	protected PackingReBankTwoService packingReBankTwoService;
	
	public void setPackingReBankTwoService(PackingReBankTwoService packingReBankTwoService)
	{
		this.packingReBankTwoService = packingReBankTwoService;
	}
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("packingReBankTwo",packingReBankTwo);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000356");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/packingloan/packingReBankTwoAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		PackingReBankTwo packingReBankTwo = new PackingReBankTwo ();
	    packingReBankTwo = (PackingReBankTwo) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), PackingReBankTwo.class , false , request.getMethod(), false);
		request.setAttribute("packingReBankTwo", packingReBankTwo);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000356");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		packingReBankTwoService._delete(bankitemid,getBusinessObject());
	...
	{
		String packingReBankTwoIds = request.getParameter("packingReBankTwoIds");
		packingReBankTwoService._deletes(packingReBankTwoIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("packingReBankTwo", packingReBankTwo);  
		return new ModelAndView("xdss3/packingloan/packingReBankTwoView");
	}
	...
	{
		// 绑定主对象值
		PackingReBankTwo packingReBankTwo = (PackingReBankTwo) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), PackingReBankTwo.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.packingReBankTwoService._saveOrUpdate(packingReBankTwo
,getBusinessObject());
}
this.packingReBankTwoService._submitProcess(packingReBankTwo		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/packingloan/packingReBankTwoManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		PackingReBankTwo packingReBankTwo  = new PackingReBankTwo();
		String bankitemid = request.getParameter("bankitemid");
		packingReBankTwo.setBankitemid(bankitemid);
		LockService.unLockBOData(packingReBankTwo);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		PackingReBankTwo packingReBankTwo = (PackingReBankTwo) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), PackingReBankTwo.class , true , request.getMethod(), true);
this.packingReBankTwoService._saveOrUpdate(packingReBankTwo,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(packingReBankTwo.getBankitemid());
		...
		{	
		jo.put("bankitemid", packingReBankTwo.getBankitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("packingReBankTwo", packingReBankTwo);
		return new ModelAndView("xdss3/packingloan/packingReBankTwoEdit");
	}
