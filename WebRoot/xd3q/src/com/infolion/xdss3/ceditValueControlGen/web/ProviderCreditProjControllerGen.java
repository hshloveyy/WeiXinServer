/*
 * @(#)ProviderCreditProjControllerGen.java
 * 版权声明 福建讯盟软件有限公司, 版权所有 违者必究
 *
 *修订记录:
 *1)更改者：java业务平台代码生成工具
 *  时　间：2010年05月20日 12点34分52秒
 *  描　述：创建
 */
...
/**
 * <pre>
 * 供应商授限立项配置(ProviderCreditProj)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class ProviderCreditProjControllerGen extends AbstractGenController
{
	private final String boName = "ProviderCreditProj";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("providerCreditProj",providerCreditProj);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000241");
	    ...
	    return new ModelAndView("xdss3/ceditValueControl/providerCreditProjAdd");
	}
	...
	{
		ProviderCreditProj providerCreditProj = new ProviderCreditProj ();
	    providerCreditProj = (ProviderCreditProj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ProviderCreditProj.class , false , request.getMethod(), false);
		request.setAttribute("providerCreditProj", providerCreditProj);
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/ceditValueControl/providerCreditProjAdd");
	}
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		providerCreditProjService._delete(configprojectid,getBusinessObject());
	...
	{
		String providerCreditProjIds = request.getParameter("providerCreditProjIds");
		providerCreditProjService._deletes(providerCreditProjIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("providerCreditProj", providerCreditProj);  
		return new ModelAndView("xdss3/ceditValueControl/providerCreditProjView");
	}
	...
	{
		// 绑定主对象值
		ProviderCreditProj providerCreditProj = (ProviderCreditProj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ProviderCreditProj.class , true , request.getMethod(), true);
this.providerCreditProjService._submitProcess(providerCreditProj		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/ceditValueControl/providerCreditProjManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ProviderCreditProj providerCreditProj  = new ProviderCreditProj();
		String configprojectid = request.getParameter("configprojectid");
		providerCreditProj.setConfigprojectid(configprojectid);
		LockService.unLockBOData(providerCreditProj);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ProviderCreditProj providerCreditProj = (ProviderCreditProj) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ProviderCreditProj.class , true , request.getMethod(), true);
this.providerCreditProjService._saveOrUpdate(providerCreditProj,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(providerCreditProj.getConfigprojectid());
		...
		{	
		jo.put("configprojectid", providerCreditProj.getConfigprojectid());
String creator = providerCreditProj.getCreator();
...
jo.put("createtime", providerCreditProj.getCreatetime());
String lastmodifyer = providerCreditProj.getLastmodifyer();
...
jo.put("lastmodifytime", providerCreditProj.getLastmodifytime());		this.operateSuccessfullyWithString(response,jo.toString());
	   }
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
        ...
        String userId = UserContextHolder.getLocalUserContext().getUserContext().getSysUser().getUserId();
        request.setAttribute("userId", userId);
		return new ModelAndView("xdss3/ceditValueControl/providerCreditProjEdit");
	}
