 ...
 * @since 1.0
 */
public class ProfitLossControllerGen extends AbstractGenController
{
	private final String boName = "ProfitLoss";
	...
	public void setProfitLossService(ProfitLossService profitLossService)
	{
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("profitLoss",profitLoss);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000250");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/profitLoss/profitLossAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ProfitLoss profitLoss = this.profitLossService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		profitLossService._delete(profitlossid,getBusinessObject());
	...
	{
		String profitLossIds = request.getParameter("profitLossIds");
		profitLossService._deletes(profitLossIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
profitLoss = this.profitLossService._get(id);        }
        else
        {
           profitLoss = this.profitLossService._get(id);
        }
		...
		request.setAttribute("profitLoss", profitLoss);  
		return new ModelAndView("xdss3/profitLoss/profitLossView");
	}
	...
	{
		// 绑定主对象值
		ProfitLoss profitLoss = (ProfitLoss) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ProfitLoss.class , true , request.getMethod(), true);
this.profitLossService._submitProcess(profitLoss		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/profitLoss/profitLossManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		ProfitLoss profitLoss  = new ProfitLoss();
		String profitlossid = request.getParameter("profitlossid");
		profitLoss.setProfitlossid(profitlossid);
		LockService.unLockBOData(profitLoss);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		ProfitLoss profitLoss = (ProfitLoss) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), ProfitLoss.class , true , request.getMethod(), true);
this.profitLossService._saveOrUpdate(profitLoss,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(profitLoss.getProfitlossid());
		...
		{	
		jo.put("profitlossid", profitLoss.getProfitlossid());
String creator = profitLoss.getCreator();
...
jo.put("createtime", profitLoss.getCreatetime());
String lastmodifyer = profitLoss.getLastmodifyer();
...
jo.put("lastmodifytime", profitLoss.getLastmodifytime());		this.operateSuccessfullyWithString(response,jo.toString());
	   }
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
profitLoss = this.profitLossService._getForEdit(id);        }
        else
        {
           profitLoss = this.profitLossService._getForEdit(id);
        }
		...
		request.setAttribute("profitLoss", profitLoss);
		return new ModelAndView("xdss3/profitLoss/profitLossEdit");
	}
