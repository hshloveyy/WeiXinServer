/*
 * @(#)BillClearItemControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.billclearitemGen.web;
 ...
 * @since 1.0
 */
public class BillClearItemControllerGen extends AbstractGenController
{
	private final String boName = "BillClearItem";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billClearItem",billClearItem);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000284");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/billclearitem/billClearItemAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		BillClearItem billClearItem = new BillClearItem ();
	    billClearItem = (BillClearItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillClearItem.class , false , request.getMethod(), false);
		request.setAttribute("billClearItem", billClearItem);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000284");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		billClearItemService._delete(billclearitemid,getBusinessObject());
	...
	{
		String billClearItemIds = request.getParameter("billClearItemIds");
		billClearItemService._deletes(billClearItemIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("billClearItem", billClearItem);  
		return new ModelAndView("xdss3/billclearitem/billClearItemView");
	}
	...
	{
		// 绑定主对象值
		BillClearItem billClearItem = (BillClearItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillClearItem.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.billClearItemService._saveOrUpdate(billClearItem
,getBusinessObject());
}
this.billClearItemService._submitProcess(billClearItem		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/billclearitem/billClearItemManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		BillClearItem billClearItem  = new BillClearItem();
		String billclearitemid = request.getParameter("billclearitemid");
		billClearItem.setBillclearitemid(billclearitemid);
		LockService.unLockBOData(billClearItem);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		BillClearItem billClearItem = (BillClearItem) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), BillClearItem.class , true , request.getMethod(), true);
this.billClearItemService._saveOrUpdate(billClearItem,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(billClearItem.getBillclearitemid());
		...
		{	
		jo.put("billclearitemid", billClearItem.getBillclearitemid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("billClearItem", billClearItem);
		return new ModelAndView("xdss3/billclearitem/billClearItemEdit");
	}
