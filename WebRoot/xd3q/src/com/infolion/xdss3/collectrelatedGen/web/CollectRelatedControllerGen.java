/*
 * @(#)CollectRelatedControllerGen.java
 ...
 *  描　述：创建
 */
package com.infolion.xdss3.collectrelatedGen.web;
 ...
 * @since 1.0
 */
public class CollectRelatedControllerGen extends AbstractGenController
{
	private final String boName = "CollectRelated";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("collectRelated",collectRelated);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000324");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/collectrelated/collectRelatedAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		CollectRelated collectRelated = new CollectRelated ();
	    collectRelated = (CollectRelated) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectRelated.class , false , request.getMethod(), false);
		request.setAttribute("collectRelated", collectRelated);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000324");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String collectrelatedid = request.getParameter("collectrelatedid");
		collectRelatedService._delete(collectrelatedid,getBusinessObject());
	...
	{
		String collectRelatedIds = request.getParameter("collectRelatedIds");
		collectRelatedService._deletes(collectRelatedIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("collectRelated", collectRelated);  
		return new ModelAndView("xdss3/collectrelated/collectRelatedView");
	}
	...
	{
		// 绑定主对象值
		CollectRelated collectRelated = (CollectRelated) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectRelated.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.collectRelatedService._saveOrUpdate(collectRelated
,getBusinessObject());
}
this.collectRelatedService._submitProcess(collectRelated		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/collectrelated/collectRelatedManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		CollectRelated collectRelated  = new CollectRelated();
		String collectrelatedid = request.getParameter("collectrelatedid");
		collectRelated.setCollectrelatedid(collectrelatedid);
		LockService.unLockBOData(collectRelated);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		CollectRelated collectRelated = (CollectRelated) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), CollectRelated.class , true , request.getMethod(), true);
this.collectRelatedService._saveOrUpdate(collectRelated,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(collectRelated.getCollectrelatedid());
		...
		{	
		jo.put("collectrelatedid", collectRelated.getCollectrelatedid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("collectRelated", collectRelated);
		return new ModelAndView("xdss3/collectrelated/collectRelatedEdit");
	}
