 ...
 *  描　述：创建
 */
package com.infolion.xdss3.fundflowBccGen.web;
 ...
 * @since 1.0
 */
public class FundFlowBccControllerGen extends AbstractGenController
{
	private final String boName = "FundFlowBcc";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("fundFlowBcc",fundFlowBcc);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000308");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/fundflowBcc/fundFlowBccAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		FundFlowBcc fundFlowBcc = new FundFlowBcc ();
	    fundFlowBcc = (FundFlowBcc) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), FundFlowBcc.class , false , request.getMethod(), false);
		request.setAttribute("fundFlowBcc", fundFlowBcc);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000308");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		fundFlowBccService._delete(fundflowid,getBusinessObject());
	...
	{
		String fundFlowBccIds = request.getParameter("fundFlowBccIds");
		fundFlowBccService._deletes(fundFlowBccIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("fundFlowBcc", fundFlowBcc);  
		return new ModelAndView("xdss3/fundflowBcc/fundFlowBccView");
	}
	...
	{
		// 绑定主对象值
		FundFlowBcc fundFlowBcc = (FundFlowBcc) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), FundFlowBcc.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.fundFlowBccService._saveOrUpdate(fundFlowBcc
,getBusinessObject());
}
this.fundFlowBccService._submitProcess(fundFlowBcc		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/fundflowBcc/fundFlowBccManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		FundFlowBcc fundFlowBcc  = new FundFlowBcc();
		String fundflowid = request.getParameter("fundflowid");
		fundFlowBcc.setFundflowid(fundflowid);
		LockService.unLockBOData(fundFlowBcc);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		FundFlowBcc fundFlowBcc = (FundFlowBcc) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), FundFlowBcc.class , true , request.getMethod(), true);
this.fundFlowBccService._saveOrUpdate(fundFlowBcc,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(fundFlowBcc.getFundflowid());
		...
		{	
		jo.put("fundflowid", fundFlowBcc.getFundflowid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("fundFlowBcc", fundFlowBcc);
		return new ModelAndView("xdss3/fundflowBcc/fundFlowBccEdit");
	}
