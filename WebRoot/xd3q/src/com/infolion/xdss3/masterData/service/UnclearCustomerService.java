...
import com.infolion.xdss3.masterData.dao.CustomerDetailHibernateDao;
...
import com.infolion.xdss3.masterData.dao.UnclearDataJdbcDao;
 ...
 */
@Service
public class UnclearCustomerService extends BaseService
{

	protected static Log log = LogFactory.getFactory().getLog(UnclearCustomerService.class);
	 ...
	 */
	public void setCustomerTitleHibernateDao(CustomerTitleHibernateDao customerTitleHibernateDao)
	{
		this.customerTitleHibernateDao = customerTitleHibernateDao;
	}

	@Autowired
	protected CustomerDetailHibernateDao customerDetailHibernateDao;

	/**
	 * @param customerDetailHibernateDao
	 *            the customerDetailHibernateDao to set
	 */
	public void setCustomerDetailHibernateDao(CustomerDetailHibernateDao customerDetailHibernateDao)
	{
		this.customerDetailHibernateDao = customerDetailHibernateDao;
	}

	@Autowired
	private CertificateNoHibernateDao certificateNoHibernateDao;

	/**
	 * @param certificateNoHibernateDao
	 *            the certificateNoHibernateDao to set
	 */
	public void setCertificateNoHibernateDao(CertificateNoHibernateDao certificateNoHibernateDao)
	{
		this.certificateNoHibernateDao = certificateNoHibernateDao;
	}

	@Autowired
	private UnclearDataJdbcDao unclearDataJdbcDao;

	/**
	 * @param unlearDataJdbcDao
	 *            the unclearDataJdbcDao to set
	 */
	public void setUnlearDataJdbcDao(UnclearDataJdbcDao unclearDataJdbcDao)
	{
	...
	public MasterDataRfcTable _executeRfcGetMasterData(String syncDate, String zdate, String customer)
	{
		...
		try
		{
			IFunctionTemplate ftemplate = manager.repository.getFunctionTemplate(SyncMasterDataConstants.UNCLEARCUSTOMER_RFCFunctionName);
			if (ftemplate != null)
			{
			...
			{
				log.error("----------------取得未清客户主数据发生错误：目标系统上不存在RFC" + SyncMasterDataConstants.UNCLEARCUSTOMER_RFCFunctionName);
	...
	public void _ayncMasterData(MasterDataRfcTable masterDataRfcTable) throws Exception
	{
		...
		else
		{
			...
			for (Map map : masterDataRfcTable.getAddDataList())
			{
				...
				i = i + 1;
				// 判断该未清数据是否已清
				if(this.customerTitleJdbcDao.isCustomerTitleClear(customerTitle)){
				    customerTitle.setIscleared("1");	
				}else{
				...
				if(StringUtils.isNotBlank(customerTitle.getVgbel())){
				    ...
				    if(StringUtils.isNotBlank(contractNo)){
				        ...
				        customerTitle.setBname(contractNo.substring(0,6));
				    }
				}
				...
				if(isCustomerTitleExist){		// 若该数据已经在数据库中存在，则只更新CONTRACT_NO和VGBEL
			        String existTitleId = this.getTitleIdIfExist(customerTitle.getBukrs(), customerTitle.getBelnr(), 
			                                                     customerTitle.getGjahr(), customerTitle.getBuzei());
			        CustomerTitle ct = this.customerTitleHibernateDao.get(existTitleId);
			        if(StringUtils.isNotBlank(customerTitle.getVgbel())){
			        ...
			        ct.setBktxt(customerTitle.getBktxt());
			        ct.setIscleared(customerTitle.getIscleared());
...
//				}
			...
			if(masterDataRfcTable.isFirstTimeSyn()){ // 若为全量更新
			    ...
			    List<CustomerTitle> cts = this.customerTitleHibernateDao.find("from CustomerTitle");
...
//			}
	 ...
	 *            借方/贷方标识
	 */
	public List<CustomerTitle> getCustomerTitleListBySupplier(String custom, String subject, String shkzg)
	{
	 ...
	 * @return
	 */
	public List<CustomerTitle> getCustomerTitleListBySupplier(String custom, String subject,String currencytext ,String companyno,String depid, String shkzg)
	{
	 ...
	 */
	public boolean isTitleExistInSAP(CustomerTitle ct, String compareStr){
     ...
     * @创建作者：邱杰烜
     * @创建时间：2010-12-02
     * 调用客户未清抬头状态更新的存储过程 (单客户)
     */
	public void updateCustomerTitle(final String customer){
        this.customerTitleHibernateDao.getHibernateTemplate().execute(new HibernateCallback() {
     ...
     * @创建作者：邱杰烜
     * @创建时间：2010-12-02
     * 调用客户未清抬头状态更新的存储过程 (全客户)
     */
    public void updateCustomerTitle(){
        this.customerTitleHibernateDao.getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException,
                    SQLException {
                CallableStatement cs = session.connection().prepareCall("{call UPDATE_ALL_CUSTOMERTITLE}");    
                boolean isSuccess = cs.execute();
                return isSuccess; 
            }
        });
