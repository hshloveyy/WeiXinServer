...
public class VendorService extends BaseService
{
	...
	protected static Log log = LogFactory.getFactory().getLog(VendorService.class);
	...
	public MasterDataRfcTable _executeRfcGetMasterData(String syncDate, String zdate, String vendor)
	{
		...
		try
		{
			IFunctionTemplate ftemplate = manager.repository.getFunctionTemplate(SyncMasterDataConstants.UNCLEARVENDOR_RFCFunctionName);
			if (ftemplate != null)
			{
			...
			{
				log.error("----------------取得未清供应商主数据发生错误：目标系统上不存在RFC" + SyncMasterDataConstants.UNCLEARVENDOR_RFCFunctionName);
	...
	public void _ayncMasterData(MasterDataRfcTable masterDataRfcTable) throws Exception
	{
		...
		else
		{
			...
			for (Map map : masterDataRfcTable.getAddDataList())
			{
                ...
                if(StringUtils.isNotBlank(vendorTitle.getEbeln())){
                	String contractNo =this.vendorTitleJdbcDao.getContractNoByP(vendorTitle.getEbeln());
                	if(StringUtils.isNotBlank(contractNo)){
	                	vendorTitle.setVerkf(contractNo);
	                	vendorTitle.setLixiang(contractNo.substring(0, 6));
                	}
                }
...
//					+ masterDataRfcTable.getAddDetailDataList().size() > 0)
    ...
    public void _syncAllCustomerTitleState(MasterDataRfcTable masterDataRfcTable) throws Exception{
        ...
        if(masterDataRfcTable.getErrorMsgs() != null && masterDataRfcTable.getErrorMsgs().size() > 0){
             ...
             */
            List<VendorTitle> vts = this.vendorTitleHibernateDao.find("from VendorTitle");
            List<VendorTitle> vts4Update = new ArrayList<VendorTitle>();
	...
	{
		return this.vendorTitleJdbcDao.getSumClearAmount(invoice, businessstates);
	}
	...
	{
		return this.vendorTitleJdbcDao.getSumClearAmount(invoice, businessstates,billclearid);
	}
	 ...
	 * @return
	 */
	public BigDecimal getSumClearAmountByVendorTitleID(String vendorTitleID, String businessstates)
	{
		return this.vendorTitleJdbcDao.getSumClearAmountVendorTitleId(vendorTitleID, businessstates);
	}
	...
	{
		this.vendorTitleJdbcDao.updateVendorTitleIsClearedByInvoice(invoice, isCleared);
	}
	
	/**
     * @创建作者：邱杰烜
     * @创建时间：2010-12-02
     * 调用更新供应商未清抬头状态更新的存储过程(单供应商)
     */
	public void updateVendorTitle(final String supplier){
        this.vendorTitleHibernateDao.getHibernateTemplate().execute(new HibernateCallback() {
     ...
     * @创建作者：邱杰烜
     * @创建时间：2010-12-02
     * 调用更新供应商未清抬头状态更新的存储过程(全供应商)
     */
    public void updateVendorTitle(){
        this.vendorTitleHibernateDao.getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException,
                    SQLException {
                CallableStatement cs = session.connection().prepareCall("{call UPDATE_ALL_VENDORTITLE}");    
                boolean isSuccess = cs.execute();
                return isSuccess; 
            }
        });
     ...
     */
    public boolean isTitleExistInSAP(VendorTitle vt, String compareStr){
