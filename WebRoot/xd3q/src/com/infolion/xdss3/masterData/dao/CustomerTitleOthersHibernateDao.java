 ...
 */
@Repository
public class CustomerTitleOthersHibernateDao extends BaseHibernateDao<CustomerTitleOthers>
{	
	/**
	 * 获取指定客户未清数据凭证编号列表
	 * @param bukrs 
	...
	public List<CertificateNo> getUnclearCustomerCertificateNoByBukrs(String kunnr)
	{
		...
		List<CustomerTitleOthers> customerTitleList;
		if(kunnr == null || kunnr.equals(""))
		{
			String hql = "from CustomerTitleOthers where iscleared = '0'";
			customerTitleList = this.find(hql);
		}
		else
		{
			String hql = "from CustomerTitleOthers where iscleared = '0' and kunnr = ?";
	...
	{		
		String hql = "from CustomerTitleOthers where customertitleid in (:ids) and iscleared = '0' ";
	...
	{	
		String hql = "from CustomerTitleOthers where ihrez in (:ids) and iscleared = '0' ";
	...
	{	
		String hql = "from CustomerTitleOthers where bname in (:ids) and iscleared = '0' ";
		Query query = getSession().createQuery(hql); 
	...
	{	
		String hql = "from CustomerTitleOthers where kunnr = (:kunnr) and ihrez in (:ids) and iscleared = '0' ";
	...
	{	
		String hql = "from CustomerTitleOthers where kunnr = (:kunnr) and bname in (:ids) and iscleared = '0' ";
		Query query = getSession().createQuery(hql); 
		query.setParameter("kunnr", kunnr);
		query.setParameterList("ids", projectnos.split(",")); 
		return query.list(); 
	...
	{	
		String hql = "from CustomerTitleOthers where ((ihrez in (:cns)) or (bname in (:pns))) and iscleared = '0' ";
		Query query = getSession().createQuery(hql);
