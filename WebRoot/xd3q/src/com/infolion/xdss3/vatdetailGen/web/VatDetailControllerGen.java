 ...
 *  描　述：创建
 */
package com.infolion.xdss3.vatdetailGen.web;
 ...
 * @since 1.0
 */
public class VatDetailControllerGen extends AbstractGenController
{
	private final String boName = "VatDetail";
	...
	@Autowired
	protected VatDetailService vatDetailService;
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vatDetail",vatDetail);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000360");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/vatdetail/vatDetailAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		VatDetail vatDetail = this.vatDetailService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		vatDetailService._delete(tid,getBusinessObject());
	...
	{
		String vatDetailIds = request.getParameter("vatDetailIds");
		vatDetailService._deletes(vatDetailIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
vatDetail = this.vatDetailService._get(id);        }
        else
        {
           vatDetail = this.vatDetailService._get(id);
        }
		...
		request.setAttribute("vatDetail", vatDetail);  
		return new ModelAndView("xdss3/vatdetail/vatDetailView");
	}
	...
	{
		// 绑定主对象值
		VatDetail vatDetail = (VatDetail) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), VatDetail.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.vatDetailService._saveOrUpdate(vatDetail
,getBusinessObject());
}
this.vatDetailService._submitProcess(vatDetail		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		VatDetail vatDetail  = new VatDetail();
		String tid = request.getParameter("tid");
		vatDetail.setTid(tid);
		LockService.unLockBOData(vatDetail);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		VatDetail vatDetail = (VatDetail) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), VatDetail.class , true , request.getMethod(), true);
this.vatDetailService._saveOrUpdate(vatDetail,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(vatDetail.getTid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
vatDetail = this.vatDetailService._getForEdit(id);        }
        else
        {
           vatDetail = this.vatDetailService._getForEdit(id);
        }
		...
		request.setAttribute("vatDetail", vatDetail);
		return new ModelAndView("xdss3/vatdetail/vatDetailEdit");
	}
