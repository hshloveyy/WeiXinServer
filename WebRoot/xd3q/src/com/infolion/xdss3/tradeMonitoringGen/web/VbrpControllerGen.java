/*
 * @(#)VbrpControllerGen.java
 * 版权声明 福建讯盟软件有限公司, 版权所有 违者必究
 *
 *修订记录:
 *1)更改者：java业务平台代码生成工具
 *  时　间：2010年06月06日 13点53分48秒
 *  描　述：创建
 */
 ...
 * @since 1.0
 */
public class VbrpControllerGen extends AbstractGenController
{
	private final String boName = "Vbrp";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vbrp",vbrp);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000277");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/tradeMonitoring/vbrpAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		Vbrp vbrp = new Vbrp ();
	    vbrp = (Vbrp) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), Vbrp.class , false , request.getMethod(), false);
		request.setAttribute("vbrp", vbrp);
		request.setAttribute("vt", getBusinessObject().getViewText());
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000277");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
	...
	{
		String vbrpid = request.getParameter("vbrpid");
		vbrpService._delete(vbrpid,getBusinessObject());
	...
	{
		String vbrpIds = request.getParameter("vbrpIds");
		vbrpService._deletes(vbrpIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vbrp", vbrp);  
		return new ModelAndView("xdss3/tradeMonitoring/vbrpView");
	}
	...
	{
		// 绑定主对象值
		Vbrp vbrp = (Vbrp) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), Vbrp.class , true , request.getMethod(), true);
this.vbrpService._submitProcess(vbrp		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/tradeMonitoring/vbrpManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		Vbrp vbrp  = new Vbrp();
		String vbrpid = request.getParameter("vbrpid");
		vbrp.setVbrpid(vbrpid);
		LockService.unLockBOData(vbrp);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		Vbrp vbrp = (Vbrp) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), Vbrp.class , true , request.getMethod(), true);
this.vbrpService._saveOrUpdate(vbrp,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(vbrp.getVbrpid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("vbrp", vbrp);
		return new ModelAndView("xdss3/tradeMonitoring/vbrpEdit");
	}
