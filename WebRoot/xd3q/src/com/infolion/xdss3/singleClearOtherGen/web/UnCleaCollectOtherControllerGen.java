/*
 * @(#)UnCleaCollectOtherControllerGen.java
...
/**
 * <pre>
 * 其他公司未清收款(贷方)(UnCleaCollectOther)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class UnCleaCollectOtherControllerGen extends AbstractGenController
{
	private final String boName = "UnCleaCollectOther";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("unCleaCollectOther",unCleaCollectOther);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000362");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/singleClearOther/unCleaCollectOtherAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		String id = request.getParameter("unclearcollectid");
		UnCleaCollectOther unCleaCollectOther = this.unCleaCollectOtherService._getEntityCopy(id);
	...
	{
		String unclearcollectid = request.getParameter("unclearcollectid");
		unCleaCollectOtherService._delete(unclearcollectid,getBusinessObject());
	...
	{
		String unCleaCollectOtherIds = request.getParameter("unCleaCollectOtherIds");
		unCleaCollectOtherService._deletes(unCleaCollectOtherIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
unCleaCollectOther = this.unCleaCollectOtherService._get(id);        }
        else
        {
           unCleaCollectOther = this.unCleaCollectOtherService._get(id);
        }
		...
		request.setAttribute("unCleaCollectOther", unCleaCollectOther);  
		return new ModelAndView("xdss3/singleClearOther/unCleaCollectOtherView");
	}
	...
	{
		// 绑定主对象值
		UnCleaCollectOther unCleaCollectOther = (UnCleaCollectOther) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), UnCleaCollectOther.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.unCleaCollectOtherService._saveOrUpdate(unCleaCollectOther
,getBusinessObject());
}
this.unCleaCollectOtherService._submitProcess(unCleaCollectOther		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/singleClearOther/unCleaCollectOtherManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		UnCleaCollectOther unCleaCollectOther  = new UnCleaCollectOther();
		String unclearcollectid = request.getParameter("unclearcollectid");
		unCleaCollectOther.setUnclearcollectid(unclearcollectid);
		LockService.unLockBOData(unCleaCollectOther);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		UnCleaCollectOther unCleaCollectOther = (UnCleaCollectOther) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), UnCleaCollectOther.class , true , request.getMethod(), true);
this.unCleaCollectOtherService._saveOrUpdate(unCleaCollectOther,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(unCleaCollectOther.getUnclearcollectid());
		...
		{	
		jo.put("unclearcollectid", unCleaCollectOther.getUnclearcollectid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
unCleaCollectOther = this.unCleaCollectOtherService._getForEdit(id);        }
        else
        {
           unCleaCollectOther = this.unCleaCollectOtherService._getForEdit(id);
        }
		...
		request.setAttribute("unCleaCollectOther", unCleaCollectOther);
		return new ModelAndView("xdss3/singleClearOther/unCleaCollectOtherEdit");
	}
