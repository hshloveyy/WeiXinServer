/*
 * @(#)SettleSubjectOtherControllerGen.java
...
/**
 * <pre>
 * 其他公司结算科目(SettleSubjectOther)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class SettleSubjectOtherControllerGen extends AbstractGenController
{
	private final String boName = "SettleSubjectOther";
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("settleSubjectOther",settleSubjectOther);
      	boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000365");
		request.setAttribute("haveBindWorkFlow", haveBindWorkFlow);
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("xdss3/singleClearOther/settleSubjectOtherAdd");
	}
	...
	public ModelAndView _copyCreate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNotBlank(calActivityId))
			request.setAttribute("calActivityId", calActivityId);
		String id = request.getParameter("settlesubjectid");
		SettleSubjectOther settleSubjectOther = this.settleSubjectOtherService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		settleSubjectOtherService._delete(settlesubjectid,getBusinessObject());
	...
	{
		String settleSubjectOtherIds = request.getParameter("settleSubjectOtherIds");
		settleSubjectOtherService._deletes(settleSubjectOtherIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
settleSubjectOther = this.settleSubjectOtherService._get(id);        }
        else
        {
           settleSubjectOther = this.settleSubjectOtherService._get(id);
        }
		...
		request.setAttribute("settleSubjectOther", settleSubjectOther);  
		return new ModelAndView("xdss3/singleClearOther/settleSubjectOtherView");
	}
	...
	{
		// 绑定主对象值
		SettleSubjectOther settleSubjectOther = (SettleSubjectOther) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SettleSubjectOther.class , true , request.getMethod(), true);
        //类型标识是从那个页面提交，view 表示从view页面提交流程。
        String type = request.getParameter("type");
if (!"view".equalsIgnoreCase(type))
{
this.settleSubjectOtherService._saveOrUpdate(settleSubjectOther
,getBusinessObject());
}
this.settleSubjectOtherService._submitProcess(settleSubjectOther		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("xdss3/singleClearOther/settleSubjectOtherManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		SettleSubjectOther settleSubjectOther  = new SettleSubjectOther();
		String settlesubjectid = request.getParameter("settlesubjectid");
		settleSubjectOther.setSettlesubjectid(settlesubjectid);
		LockService.unLockBOData(settleSubjectOther);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		SettleSubjectOther settleSubjectOther = (SettleSubjectOther) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), SettleSubjectOther.class , true , request.getMethod(), true);
this.settleSubjectOtherService._saveOrUpdate(settleSubjectOther,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(settleSubjectOther.getSettlesubjectid());
		...
		{	
		jo.put("settlesubjectid", settleSubjectOther.getSettlesubjectid());
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{	
settleSubjectOther = this.settleSubjectOtherService._getForEdit(id);        }
        else
        {
           settleSubjectOther = this.settleSubjectOtherService._getForEdit(id);
        }
		...
		request.setAttribute("settleSubjectOther", settleSubjectOther);
		return new ModelAndView("xdss3/singleClearOther/settleSubjectOtherEdit");
	}
