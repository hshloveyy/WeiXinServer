/*
 * @(#)PurchaseRowsControllerGen.java
...
/**
 * <pre>
 * 采购订单行项目信息(SAP)(PurchaseRows)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class PurchaseRowsControllerGen extends AbstractGenController
{
	private final String boName = "PurchaseRows";
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
	    ...
	    purchaseRows = (PurchaseRows) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), PurchaseRows.class , false, request.getMethod(), false);
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("purchaseRows", purchaseRows);
		return new ModelAndView("sample/purchase/purchaseRowsEdit");
	}
	...
	{
	    PurchaseRows purchaseRows = new PurchaseRows();
    	purchaseRows.setLpein("D");
    	purchaseRows.setConvnum2("1");
    	purchaseRows.setConvnum1("1");
		request.setAttribute("vt", getBusinessObject().getViewText());
		request.setAttribute("purchaseRows",purchaseRows);

	    return new ModelAndView("sample/purchase/purchaseRowsAdd");
	}
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		purchaseRowsService._delete(purchaserowsId,getBusinessObject());
	...
	{
		String purchaseRowsIds = request.getParameter("purchaseRowsIds");
		purchaseRowsService._deletes(purchaseRowsIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("purchaseRows", purchaseRows);  
		return new ModelAndView("sample/purchase/purchaseRowsView");
	}
	...
	{
		// 绑定主对象值
		PurchaseRows purchaseRows = (PurchaseRows) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), PurchaseRows.class , true , request.getMethod(), true);
this.purchaseRowsService._submitProcess(purchaseRows		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("sample/purchase/purchaseRowsManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		PurchaseRows purchaseRows  = new PurchaseRows();
		String purchaserowsId = request.getParameter("purchaserowsId");
		purchaseRows.setPurchaserowsId(purchaserowsId);
		LockService.unLockBOData(purchaseRows);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		PurchaseRows purchaseRows = (PurchaseRows) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), PurchaseRows.class , true , request.getMethod(), true);
this.purchaseRowsService._saveOrUpdate(purchaseRows,getBusinessObject());
		jo.put("purchaserowsId", purchaseRows.getPurchaserowsId());
String creator = purchaseRows.getCreator();
...
jo.put("createTime", purchaseRows.getCreateTime());
String lastmodifyer = purchaseRows.getLastmodifyer();
...
jo.put("lastmodifyTime", purchaseRows.getLastmodifyTime());		this.operateSuccessfullyWithString(response,jo.toString());
	}
