/*
 * @(#)TestProjectManageControllerGen.java
 * 版权声明 福建讯盟软件有限公司, 版权所有 违者必究
 *
 *修订记录:
 *1)更改者：java业务平台代码生成工具
 *  时　间：2010年04月22日 16点43分43秒
 *  描　述：创建
 */
...
import com.infolion.TEMP.Temp.service.TestProjectManageService;
...
/**
 * <pre>
 * 项目管理（测试）(TestProjectManage)控制器类
 * </pre>
 * 
 * <br>
 * JDK版本:1.5
 ...
 * @since 1.0
 */
public class TestProjectManageControllerGen extends AbstractGenController
{
	private final String boName = "TestProjectManage";
	...
	@Autowired
	protected TestProjectManageService testProjectManageService;
	
	public void setTestProjectManageService(TestProjectManageService testProjectManageService)
	{
		this.testProjectManageService = testProjectManageService;
	}
	...
	public ModelAndView _edit(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{	
testProjectManage = this.testProjectManageService._getForEdit(id);        }
        else
        {
           testProjectManage = this.testProjectManageService._getForEdit(id);
        }
       
		boolean haveBindWorkFlow = WorkflowService.getHaveBindWorkFlow("000000000218");
		...
		request.setAttribute("testProjectManage", testProjectManage);
		return new ModelAndView("TEMP/Temp/testProjectManageEdit");
	}
	...
	public ModelAndView _create(HttpServletRequest request, HttpServletResponse response)
	{
		...
		request.setAttribute("workflowTaskId", workflowTaskId);	
		request.setAttribute("workflowNodeDefId", workflowNodeDefId);
	    return new ModelAndView("TEMP/Temp/testProjectManageAdd");
	}
	...
	{
		String id = request.getParameter("projectid");
		TestProjectManage testProjectManage = this.testProjectManageService._getEntityCopy(id);
	...
	public void _delete(HttpServletRequest request, HttpServletResponse response)
	{
		...
		testProjectManageService._delete(projectid,getBusinessObject());
	...
	{
		String testProjectManageIds = request.getParameter("testProjectManageIds");
		testProjectManageService._deletes(testProjectManageIds,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	public ModelAndView _view(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (StringUtils.isNullBlank(id))
		{
testProjectManage = this.testProjectManageService._get(id);        }
        else
        {
           testProjectManage = this.testProjectManageService._get(id);
        }
		...
		request.setAttribute("testProjectManage", testProjectManage);  
		return new ModelAndView("TEMP/Temp/testProjectManageView");
	}
	...
	{
		// 绑定主对象值
		TestProjectManage testProjectManage = (TestProjectManage) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), TestProjectManage.class , true , request.getMethod(), true);
this.testProjectManageService._submitProcess(testProjectManage		,getBusinessObject());
		this.operateSuccessfully(response);
	}
	...
	{ 
		request.setAttribute("vt", getBusinessObject().getViewText());
		return new ModelAndView("TEMP/Temp/testProjectManageManage");
	}
	...
	public void _cancel(HttpServletRequest request, HttpServletResponse response)
	{
		...
		TestProjectManage testProjectManage  = new TestProjectManage();
		String projectid = request.getParameter("projectid");
		testProjectManage.setProjectid(projectid);
		LockService.unLockBOData(testProjectManage);
	...
	public void _saveOrUpdate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		// 绑定主对象值
		TestProjectManage testProjectManage = (TestProjectManage) ExBeanUtils.bindBusinessObjectData(request.getParameterMap(), TestProjectManage.class , true , request.getMethod(), true);
this.testProjectManageService._saveOrUpdate(testProjectManage,getBusinessObject());
		...
		if (StringUtils.isNotBlank(calActivityId))
		{
			...
			if (calActivity != null)
			{
				...
				calActivity.setBusid(testProjectManage.getProjectid());
		...
		{	
		jo.put("projectid", testProjectManage.getProjectid());
String creator = testProjectManage.getCreator();
...
jo.put("createtime", testProjectManage.getCreatetime());
String lastmodifyer = testProjectManage.getLastmodifyer();
...
jo.put("lastmodifytime", testProjectManage.getLastmodifytime());		this.operateSuccessfullyWithString(response,jo.toString());
	   }
	}
